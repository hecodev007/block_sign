				...
				if s.cfg.Enable {
					...
					if s.cfg.Type == "rabbitmq" {
						...
						if err == nil {
							...
							log.Println("mqPush err:", err.Error())
						}
					} else {
						...
						if err == nil {
							...
							log.Println("httpPush err:", err.Error())
						}
					}
					po.UpdateNotifyResult(notifyDB)
					if notifyDB.Result == 1 {
						log.Printf("push tx success %v", string(task.TxData))
						break
					}
					//重推超过3次的话,不再重推
					if notifyDB.Num > 3 {
...
func (s *PushServer) AddPushUserTask(height int64, pushdata []byte) {
	...
	if err != nil {
		log.Printf("don't have user watch height")
	}
