...
import (
	...
	dao "iotasync/models/po/doge"
	"iotasync/utils"
	rpc "iotasync/utils/doge"
	"encoding/json"
	"fmt"
...
retryGetBlockByHeight:
	...
	if err != nil {
		log.Printf("%v height:%v", err.Error(), height)
...
func parseBlockRawTX(RpcClient *rpc.RpcClient, tx *rpc.Transaction, blockhash string, height int64) (*TxInfo, error) {
	...
	for _, vout := range tx.Vout {
		...
		if address, err := vout.ScriptPubkey.GetAddress(); err == nil && len(address) == 1 {
	...
	for _, vin := range tx.Vin {
		...
		if vin.Txid == "" { //跳过挖矿交易
			log.Println(tx.Txid, "empty vin.txid")
			continue
		}
		...
		GetRawTransaction:
			...
			if err != nil {
				log.Println(err.Error(), tx.Txid, vin.Txid)
