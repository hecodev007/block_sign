...
const (
	...
	create_rawtransaction = "createrawtransaction"
	sign_rawtransaction   = "signrawtransactionwithkey"
...
func (rpc *RpcClient) GetBalance(address string) (balance string, err error) {

	err = rpc.CallWithAuth(get_balance, rpc.Credentials, &balance, address)
	return
}
...
type SatImportAddressResult struct {
	Result string      `json:"result"`
	Id     int64       `json:"id"`
	Error  interface{} `json:"error,omitempty"`
}

type SatListUnSpentResult struct {
	Result []UnSpend   `json:"result"`
	Id     int64       `json:"id"`
	Error  interface{} `json:"error,omitempty"`
}
...
type UnSpend struct {
	...
	RedeemScript  string
	ScriptPubKey  string
	Amount        decimal.Decimal //float64
	Confirmations int64
	Spendable     bool
	Solvable      bool
	Safe          bool
...
func (rpc *RpcClient) GetUnSpends(addr string) (utxo []UnSpend, err error) {
	addrs := []string{addr}
	err = rpc.CallWithAuth(list_unspend, rpc.Credentials, &utxo, 1, 999999, addrs)
	return
}

func (rpc *RpcClient) GetUnSpends2(addr []string) (*SatListUnSpentResult, error) {
	utxo := &SatListUnSpentResult{}
	utxo2 := []UnSpend{}
	err := rpc.CallWithAuth(list_unspend, rpc.Credentials, &utxo2, 1, 999999, addr)
	utxo.Result = utxo2
	return utxo, err
}

func (rpc *RpcClient) ImportAddress(addr, label string, rescan bool) (*SatImportAddressResult, error) {
	result := &SatImportAddressResult{}
	err := rpc.CallWithAuth(import_address, rpc.Credentials, result, addr, label, rescan)
	return result, err
}
...
func (rpc *RpcClient) CreateRawTransaction(txins interface{}, txouts interface{}) (hextx string, err error) {
	err = rpc.CallWithAuth(create_rawtransaction, rpc.Credentials, &hextx, txins, txouts)
	return
}

func (rpc *RpcClient) SignRawTransaction(hextx string, pri []string, pre interface{}) (ret SignRawTransactionResponse, err error) {
	err = rpc.CallWithAuth(sign_rawtransaction, rpc.Credentials, &ret, hextx, pri, pre)
	return
}
