...
import (
	...
	"github.com/group-coldwallet/btcsign/api/e"
	"github.com/group-coldwallet/btcsign/conf"
	"github.com/group-coldwallet/btcsign/crypto"
	...
	"github.com/group-coldwallet/btcsign/service/btcservice"
...
type APIV1 struct {
	btcService btcservice.BasicService
}

func NewAPIV1() api.API {
	return &APIV1{
		btcService: btcservice.NewBtcService(),
	}
}
...
func (a *APIV1) CreateTpl(c *gin.Context) {
	...
	if txinput.Fee < conf.GlobalConf.BtcCfg.MinFee {
		err = fmt.Errorf("Fee too low,min:%d ,now:%d", conf.GlobalConf.BtcCfg.MinFee, txinput.Fee)
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		return
	}

	if txinput.Fee > conf.GlobalConf.BtcCfg.MaxFee {
		err = fmt.Errorf("Fee too high,max:%d ,now:%d", conf.GlobalConf.BtcCfg.MaxFee, txinput.Fee)
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		return
	}
	...
	for i, v := range txinput.Txins {
		...
		tpl.TxIns = append(tpl.TxIns, bo.BtcTxInTpl{
			FromAddr:         v.Address,
			FromTxid:         v.Txid,
			FromAmount:       v.Amount,
...
func (a *APIV1) SignTx(c *gin.Context) {
	...
	hex, err := a.btcService.SignTx(tpl)
	if err != nil {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), vo.SignResult{
			MchInfo: tpl.MchInfo,
		})
...
func (a *APIV1) GetPrivkey(c *gin.Context) {
	...
	privkey, _ := global.GetValue2("", address)
	if privkey == "" {
...
func (a *APIV1) SendTx(c *gin.Context) {
	...
	sendUrl := conf.GlobalConf.BtcCfg.PushServers
	if len(sendUrl) == 0 {
...
func (a *APIV1) ImportAddr(c *gin.Context) {
	...
	//global.SetValue2(address, privkey)
...
func (a *APIV1) CreateAddrs(c *gin.Context) {
	...
	if params.Count > 100000 {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, "num too big，100000", nil)
		return
	}
	if params.Mch == "" {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, "empty mch", nil)
		return
	}
	filepath := conf.GlobalConf.BtcCfg.CreateAddrPath
	copyPath := conf.GlobalConf.BtcCfg.FilePath
	logrus.Info(a.btcService == nil)
	resultVo, err := a.btcService.CreateAddr(params, filepath, copyPath)
	...
	kmsReq := &secure.RequestPushKey{CoinCode: "btc", Mch: params.Mch}
	addrList := make([]string, 0)
	for k, v := range resultVo.Addrs {
		cryptoText, err := crypto.AesBase64Str(v, conf.Global.Secret.TransportSecureKey, true)
		if err != nil {
			api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, fmt.Sprintf("[%s]failed to aes crypto: %v", k, err), nil)
			return
		}
	...
	api.HttpResponseByMsg(c, http.StatusOK, e.SUCCESS, e.GetMsg(e.SUCCESS), ResponseInner{Address: addrList})
