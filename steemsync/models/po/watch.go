...
type AddressesInfo struct {
	...
	AddrType string `json:"addr_type,omitempty" gorm:"column:addr_type"`
...
func (u *AddressesInfo) MaxId() (int64, error) {
	has, err := db.UserConn.OrderBy("id desc").Get(u)
	if err != nil {
		return 0, err
	}
	if !has {
		return 0, errors.New(u.TableName() + " 表没数据")
	}
	return u.Id, nil
}
...
func FindAddressesInfos(cointype string, startId int64) (list []*AddressesInfo, err error) {
	//db.UserConn.ShowSQL(true)
	err = db.UserConn.Where("coin_type = ? and status = 'used' and id>?", cointype, startId).Find(&list)
	return
}
...
func (o *ContractInfo) MaxId() (int64, error) {
	has, err := db.UserConn.OrderBy("id desc").Get(o)
	if err != nil {
		return 0, err
	}
	if !has {
		return 0, errors.New(o.TableName() + " 表没数据")
	}
	return o.Id, nil
}
...
func FindContractInfos(coinType string, startId int64) (list []*ContractInfo, err error) {
	list = make([]*ContractInfo, 0)
	err = db.UserConn.Where("coin_type = ? and invaild = ? and id>?", coinType, 0, startId).Find(&list)
	return
}
