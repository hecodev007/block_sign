...
func (tx *SignedTransaction) Sign(privKeys [][]byte, chain *Chain) error {
	...
	//e, err := btcutil.DecodeWIF("5HwTWjvH52LW2pdkAvQo3gYsj1DhFRo81sn88TVFPPbzibKdChS")
	//pri := e.PrivKey.ToECDSA()
	//seckey := math.PaddedBigBytes(pri.D, pri.Params().BitSize/8)
	//
	//sig, _ := secp256k1.Sign(digest, seckey)
	// Sign.
	...
	//sigsHex := hex.EncodeToString(sig)
	//tx.Transaction.Signatures = []string{sigsHex}
