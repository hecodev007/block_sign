...
type WatchControl struct {
	...
	//Account            map[string]bool                  //*sync.Map
}
...
func NewWatchControl(name string, addressRecover, contractRecover int64) *WatchControl {
	...
	//for _, v := range conf.Cfg.Sync.Account {
	//	s.Account[v] = true
	//}
	...
	for _, addrInfo := range addrInfos {
		userAddrInfo := bo.UserAddressInfo{
			...
			//AddrType: addrInfo.AddrType, //account name
		}
		if v, ok := s.watchUsers[addrInfo.UserId]; ok {
			userAddrInfo.NotifyUrl = v.TrxNotifyUrl
		}

		if v, ok := s.WatchAddrs[addrInfo.Address]; !ok {
			s.WatchAddrs[addrInfo.Address] = []*bo.UserAddressInfo{&userAddrInfo}
		} else {
			v = append(v, &userAddrInfo)
			s.WatchAddrs[addrInfo.Address] = v
		}
	}
...
//func (s *WatchControl) IsWatchAccountExist(memo, account string) bool {
//
//	_, ok := s.WatchAddrs[memo]
//	if ok {
//		//if info[0].AddrType == account {
//		//	return true
//		//}
//		//log.Infof("wat address:%s, ressult:%t", addr, ok)
//	}
