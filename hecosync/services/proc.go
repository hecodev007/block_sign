...
func NewProcServer(processor common.Processor) *ProcServer {
...
func (s *ProcServer) mpload(k int64) (cacheTask, bool) {
	s.mplock.RLock()
	defer s.mplock.RUnlock()
	ret, ok := s.mempool[k]
	return ret, ok
}
func (s *ProcServer) mpset(k int64, v cacheTask) {
	s.mplock.Lock()
	defer s.mplock.Unlock()
	s.mempool[k] = v
}
func (s *ProcServer) mpdel(k int64) {
	s.mplock.Lock()
	defer s.mplock.Unlock()
	delete(s.mempool, k)
}
	...
	go func() {
		...
		for run {
			...
			default:
				...
				////加载end
				//删除缓存
				if _, ok := s.mpload(t.GetHeight()); ok {
					s.mpdel(t.GetHeight())
				} else {
...
func (s *ProcServer) AddReverseTask(t common.ProcTask) {
	...
	//第一次推送交易
	if v, ok := s.mpload(t.GetHeight()); !ok || v.GetBlockHash() != t.GetBlockHash() {
		hasWatchAddr, err := s.processor.ProcReverseTxs(t)
		if err != nil {
			log.Error(err)
			panic(err.Error())
		}
		//log.Info(t.GetHeight(), hasWatchAddr, t.GetConfirms())
		s.mpset(t.GetHeight(), cacheTask{ProcTask: t, Reconfirm: hasWatchAddr})
		//后面推送确认数
	} else if ok && v.Reconfirm {
		//log.Info(t.GetHeight(), v.Reconfirm, t.GetConfirms(), v.GetConfirms())

		s.processor.PushReverseConfirms(t)
		s.mpset(t.GetHeight(), cacheTask{ProcTask: t, Reconfirm: true})
		return
	} else {
		//log.Info(t.GetHeight(), ok)
		//if ok {
		//	log.Info(t.GetHeight(), v.Reconfirm, t.GetConfirms(), v.GetConfirms())
		//}
		//panic("")
...
func (s *ProcServer) RepushTx(params *bo.RePushRequest) error {
	return s.processor.RepushTx(params)
}
