...
func (bs *BaseService) ChainService(ctx context.Context) *ChainService {
	...
	if conf.Config.Gas.Special != "" {
		...
		for _, s := range gasArr {
			...
			if err != nil {
				log.Infof("gas.special %s 转换为int64失败 %s", contractGasArr[1], err.Error())
				continue
			}
...
loop:
	for {
		...
		case <-ctx.Done():
			log.Info("收到Done信号，停止timer，终止循环")
			timer.Stop()
			break loop
		}
	}
	log.Info("退出 loopSign")
}
...
func (s *ChainService) processSign() {
	...
	log.Infof("从redis待执行列表获取到订单:%s", order.OuterOrderNo)
	if err != nil {
		log.Errorf("从redis待执行列表(缓存)弹出订单失败:%v", err)
		return
	}
	...
	if err != nil {
		...
		// 签名成功
		log.Infof("订单:%s 签名完成", order.OuterOrderNo)
	}
	...
	log.Infof("订单:%s 处理完毕", order.OuterOrderNo)
}
...
func (s *ChainService) SignService(req *model.ReqSignParams) (interface{}, error) {
	...
	log.Infof("出账金额为：%s,gasPrice：%s,Nonce: %d", amount.String(), gasPrice.String(), nonce)
...
热钱包出账服务
*/
func (s *ChainService) TransferService(req interface{}) error {
	var tp model.TransferParams
	if err := s.BaseService.parseData(req, &tp); err != nil {
		return err
	}
	if &tp == nil {
		return errors.New("transfer params is null")
	}
	if tp.OuterOrderNo == "" {
		return errors.New("outer_order_no require")
	}
	//if tp.Mac == "" {
	//	return errors.New("mac require")
	//}
	if tp.FromAddress == "" || tp.ToAddress == "" || tp.Amount == "" {
		return fmt.Errorf("params is null,from=[%s],to=[%s],amount=[%s]", tp.FromAddress, tp.ToAddress, tp.Amount)
	}
...
func (s *ChainService) signAndSendRawTransaction(order *model.TransferParams) ([]byte, error) {
	...
	log.Infof("实际gas price : %d", gasPrice)
	amount = toAmount.BigInt()
	toAddress := common.HexToAddress(order.ToAddress)
	log.Infof("出账金额为： %s,gasPrice： %d,Nonce: %d", amount.String(), gasPrice, nonce)
	...
	if order.ContractAddress != "" {
		...
		if v, ok := s.contractGas[strings.ToLower(order.ContractAddress)]; ok {
			logrus.Infof("特殊合约(%s)的gasLimit 从配置中获取: %d", order.ContractAddress, v)
			gasLimit = v
		}
		// 合约 转账
		// safemoon oks safe
		...
		//	gasLimit = 1000000
		//	logrus.Infof("特殊交易，修改limit为: %d", gasLimit)
	...
	if order.OuterOrderNo != "" {
		log.Infof("OuterOrderNo 不为空 = %s", order.OuterOrderNo)
	...
	if order.OuterOrderNo != "" {
		if err = redis.Client.Set(redis.GetBroadcastOuterOrderNoKey(order.OuterOrderNo), order.OuterOrderNo, time.Hour*24); err != nil {
			log.Infof("广播订单存入redis失败: %v", err)
		} else {
			log.Infof("广播订单=%s 存入redis成功", order.OuterOrderNo)
		}
	}
	log.Infof("成功出账一笔交易： txId=%s", string(res))
	log.Info("广播结束 ")
	return res, nil
}
...
func (s *ChainService) getBuildTxParams(method string, params []interface{}) (*big.Int, error) {
	...
	if err != nil {
		log.Errorf("rpc send error,Err=%v", err)
...
func (s *ChainService) aegisNonce(address string) (uint64, error) {
	...
	if nonce < 0 {
		log.Errorf("%s 获取链上的nonce值错误", address)
...
func (s *ChainService) TransferCollectService(req interface{}) (interface{}, error) {
	...
	if tp.GasLimit > 0 {
		gasLimit = tp.GasLimit
		logrus.Infof("使用入参传过来的gasLimit %d", gasLimit)
	} else {
		if tp.ContractAddress != "" {
			if v, ok := s.addrTimeMap[strings.ToLower(tp.FromAddress)]; ok {
				if time.Now().Unix() < v {
					logrus.Infof("地址 %s 归集完不到30秒，本次不能归集", tp.FromAddress)
			...
			if v, ok := s.contractGas[strings.ToLower(tp.ContractAddress)]; ok {
				logrus.Infof("特殊合约(%s)的gasLimit 从配置中获取: %d", tp.ContractAddress, v)
				gasLimit = v
			}
		}
	}
	...
	if tp.GasPrice > 0 {
		gasPrice = big.NewInt(tp.GasPrice)
		logrus.Infof("使用入参的gasPrice %d", tp.GasPrice)
	} else {
		gasPrice = big.NewInt(s.confirmFeeBsc(tp.Fee))
		logrus.Infof("最终 from=%s  to=%s 使用手续费为 %d", tp.FromAddress, tp.ToAddress, gasPrice.Int64())
	}

	logrus.Infof("实际gas price : %s", gasPrice.String())
	amount = toAmount.BigInt()
	toAddress := common.HexToAddress(tp.ToAddress)
	...
	if tp.ContractAddress != "" {
		...
		logrus.Infof("EstimateGas估算的gas为 %d 实际使用的gas为 %d", estimateGas, gasLimit)
		if err != nil {
			return nil, err
		}
		if gasLimit < estimateGas {
			gasLimit = estimateGas
			logrus.Infof("使用EstimateGas估算的值 %d", gasLimit)
		}
