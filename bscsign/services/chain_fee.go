...
func (s *ChainService) confirmFee() *big.Int {
	...
	if err != nil {
		log.Errorf("从redis获取手续费出错: %s", err.Error())
	}
	...
	if value != "" {
		...
		if !ok {
			log.Errorf("从Redis获取的Fee转换成big.int出错")
			// 清除这个格式有误的缓存
			if errDel := redis.Client.Del(feeCacheKey); errDel != nil {
				log.Errorf("删除redis出错：%v", errDel)
			}
			return big.NewInt(conf.Config.ChainCfg.GasPrice)
		}
		log.Infof("使用从缓存读取的手续费 %s", feeInCache.String())
		return feeInCache
	}

	log.Infof("没有从缓存获取到数据，直接从链上获取")
...
func (s *ChainService) getFeeFromChain() *big.Int {
	...
	log.Infof("从链上获取的gasPrice为 %s", priceOnChain.String())
	log.Info("--从链上获取eth_gasPrice结束 ")
	return priceOnChain
}
...
func adjustFee(price *big.Int) *big.Int {
	...
	if price.Cmp(big.NewInt(0)) != 1 { // price <= 0
		log.Infof("从链上获取到的gasPrice值<=0，使用配置文件的值:%s", minLimit.String())
		return minLimit
	}
	...
	if price.Cmp(checkPoint) == -1 { // price < 100GWei
		...
		log.Infof("调整gasPrice价格 price=%s 增加百分之30", price.String())
	} else {
		thirty := big.NewInt(30)
		thirty.Mul(thirty, GWei)

		price.Add(price, thirty)
		log.Infof("调整gasPrice价格 price=%s 增加30GWei", price.String())
	}
	...
	log.Infof("调整gasPrice价格 最终计算得出的值为:%s", price.String())
	return price
}
