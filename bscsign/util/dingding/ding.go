...
import (
	...
	"github.com/bsc-sign/util/dingding/dingbot"
...
func InitDingBot(ctx context.Context) {
	if conf.Config.IMBot.DingErrorToken != "" {
		...
		log.Info("不启用钉钉ERROR机器人：没有配置dingErrorToken")
	}
	...
	if conf.Config.IMBot.DingWarnToken != "" {
		...
		log.Info("不启用钉钉WARN机器人：没有配置dingWarnToken")
	}
	...
	if conf.Config.IMBot.DingInfoToken != "" {
		...
		log.Info("不启用钉钉INFO机器人：没有配置dingInfoToken")
	}
}
...
func talk(level DingLevel, tag string, outerOrderNo string, sections ...string) {
	if Error == level && ErrorDingBot == nil {
		log.Infof("未初始化的机器人 %d", Error)
		return
	}
	if Warn == level && WarnDingBot == nil {
		log.Infof("未初始化的机器人 %d", Warn)
		return
	}
	if Info == level && InfoDingBot == nil {
		log.Infof("未初始化的机器人 %d", Info)
		return
	}
	...
	if err != nil {
		log.Errorf("notifyDingDing设置redis失败：%s", err.Error())
		return
	}
	if !setSuccess {
		return
	...
	log.Infof("ding talk setSuccess:%s", setSuccess)
	if setSuccess {
		switch level {
		case Warn:
