...
func init() {
	if conf.Cfg == nil {
		InitLogger(true, "info", "text", "tmplogs/info", "tmplogs/err")
	} else {
		InitLogger(conf.Cfg.Log.Console, conf.Cfg.Log.Level, conf.Cfg.Log.Formatter, conf.Cfg.Log.OutFile, conf.Cfg.Log.ErrFile)
	}
}
...
func InitLogger(mode bool, level, format, outfile, errfile string) {
	var (
