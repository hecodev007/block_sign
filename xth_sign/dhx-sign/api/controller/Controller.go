...
import (
	...
	"dhxSign/common/conf"
	"dhxSign/common/log"
	"dhxSign/common/validator"
	btc "dhxSign/utils/dhx"
	"encoding/json"
	"fmt"
	"net/http"
...
const (
	// 限制最小的转账金额
	minTransferAmountLimit = "10000000000000" // 1e13
)
...
func (this *Controller) Router(r *gin.Engine) {
	group := r.Group("/v1/" + conf.GetConfig().Name)
	{
		...
		group.POST("/transfer", gin.BasicAuth(gin.Accounts{"hsc": "7c0dc78d742b0bd805635d00a63f39fcd86945fb13575440"}), this.transfer)
		group.POST("/validAddress", this.validAddress)
		group.POST("/getBalance", this.GetBalance)
		group.POST("/broadcast", this.Broadcast)
...
func (this *Controller) Broadcast(ctx *gin.Context) {
	var params = new(validator.BroadcastParams)
	if err := ctx.ShouldBindJSON(params); err != nil {
		this.NewError(ctx, err.Error())
		return
	}
...
func (this *Controller) transfer(ctx *gin.Context) {
	...
	minAmountLimit, err := decimal.NewFromString(minTransferAmountLimit)
	if err != nil {
		this.NewError(ctx, "string format decimal error")
		return
	}

	if params.Amount.Cmp(minAmountLimit) == -1 {
		this.NewError(ctx, "invalid amount. must not be less than 0.00001 (10000000000000)")
		return
	}
	...
	accountInfo, err := client.GetAccountInfo(params.FromAddress)
	if err != nil {
		this.NewError(ctx, err.Error())
		return
	}
	fromBalance := decimal.NewFromBigInt(accountInfo.Data.Free.Int, 0)
	...
	// 调用方没有传入`nonce`值时使用主网获取的值
	if params.Nonce <= 0 {
		params.Nonce = uint64(accountInfo.Nonce)
	}
