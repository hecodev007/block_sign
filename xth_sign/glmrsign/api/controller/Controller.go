...
func (this *Controller) sign(ctx *gin.Context) {
	...
	if params.Nonce == 0 {
		renum := 0
	RetryGetTransactionCount:
		Nonce, err := client.GetTransactionCount(params.FromAddress, "latest")
		if err != nil {
			log.Info(err.Error())
			if renum < 3 {
				renum++
				goto RetryGetTransactionCount
			}
	...
	err = tx.Sign(big.NewInt(1284), prikey)
	if err != nil {
		this.NewError(ctx, err.Error())
		return
	}
...
func (this *Controller) transfer(ctx *gin.Context) {
	...
	if params.Nonce == 0 {
		renum := 0
	RetryGetTransactionCount:
		Nonce, err := client.GetTransactionCount(fromAddress, "latest")
		if err != nil {
			if renum < 3 {
				renum++
				goto RetryGetTransactionCount
...
func (this *Controller) getBalance(ctx *gin.Context) {
	...
	if params.Token == "" {
		...
		balance, err = client.GetBalance(params.Address, "latest")
