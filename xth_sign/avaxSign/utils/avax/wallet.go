...
type Wallet struct {
	...
	utxoSet  *avmwallet.UTXOSet    // Mapping from utxoIDs to UTXOs
...
func NewAvaxWallet(log logging.Logger, networkID uint32, chainID ids.ID, txFee uint64) (*Wallet, error) {
	c := codec.NewDefault()
	errs := wrappers.Errs{}
	errs.Add(
...
func (w *Wallet) AddUTXO(utxo *avax.UTXO) {
	...
	if !ok {
		fmt.Println("error utxo type:TransferableOut")
		return
	}
	...
	if _, _, err := w.keychain.Spend(out, stdmath.MaxUint64); err == nil {
		...
		fmt.Println(err.Error(), "amount", out.Amount())
	}
}
...
func (w *Wallet) CreateTx(assetID ids.ID, amount uint64, destAddr ids.ShortID) (*avm.Tx, error) {
	...
	for _, utxo := range w.utxoSet.UTXOs {
		...
		if amountSpent >= amount+w.txFee {
			break
		}
	}

	if amountSpent < amount+w.txFee {
		fmt.Println("amountSpent:", amountSpent, "spent:", amount+w.txFee)
	...
	if amountSpent > amount+w.txFee+56 {

		if w.ChangeAddr.IsZero() {
			return nil, errors.New("empty changeAddr")
		}
		outs = append(outs, &avax.TransferableOutput{
			Asset: avax.Asset{ID: assetID},
			Out: &secp256k1fx.TransferOutput{
				Amt: amountSpent - amount - w.txFee,
				OutputOwners: secp256k1fx.OutputOwners{
					Locktime:  0,
					Threshold: 1,
					Addrs:     []ids.ShortID{w.ChangeAddr},
				},
			},
		})
	}
...
func (w *Wallet) SetChangeAddr(changeAddr string) error {
	addr, err := AddressToShot(changeAddr)
	if err != nil {
		return err
	}
	w.ChangeAddr = *addr
