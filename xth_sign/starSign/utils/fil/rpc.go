...
func (rpc *RpcClient) BaseFee() (int64, error) {
	ret := new(SyncState)
	err := rpc.CallWithAuth("Filecoin.SyncState", rpc.Credentials, ret)
	if err != nil {
		return 0, err
	}
	if len(ret.ActiveSyncs) >= 2 && len(ret.ActiveSyncs[1].Target.Blocks) > 0 {
		return strconv.ParseInt(ret.ActiveSyncs[1].Target.Blocks[0].ParentBaseFee, 10, 64)
	}
	return 0, nil
}
