...
func (this *Controller) transfer(ctx *gin.Context) {
	...
	if fromBalance.LessThan(params.Amount){
		this.NewError(ctx, params.OrderId+" "+fmt.Sprintf("账户余额不足,出账(%v)<余额(%v)",params.Amount.String(),fromBalance.String()))
		return
	}
	params.GenesisHash = client.GetGenesisHash()
	params.SpecVersion = uint32(client.SpecVersion)
	...
	if err != nil || result == nil {
		this.NewError(ctx, params.OrderId+" "+"交易发送失败:"+err.Error())
		return
	}
	txid := result.(string)
	returns.Data = rawtx
