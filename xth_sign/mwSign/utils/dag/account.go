...
func GenAccount1() (address string, private string, err error) {
	ac := sdk.NewAccountManager("keystore")
	addr, err := ac.Create("")
	if err != nil {
		return "", "", err
	}
...
func GenAccount2() (address string, private string, err error) {
	...
	return string(utils.ToCfxGeneralAddress(addr)), hex.EncodeToString(crypto.FromECDSA(privateKeyECDSA)), nil
}

func BuildTx(params *validator.TelosSignParams) (tx *types.UnsignedTransaction, err error) {
	tx = &types.UnsignedTransaction{}
	tx.From = types.NewAddress(params.FromAddress)

	//平台币转账
	if params.Token == "" {
		tx.To = types.NewAddress(params.ToAddress)
		tx.Value = types.NewBigIntByRaw(params.Value.BigInt())
	} else { //合约转账
		tx.To = types.NewAddress(params.Token)
		erctoken, err := abi.JSON(strings.NewReader(TokenABI))
		if err != nil {
			return nil, err
		}
		data, err := erctoken.Pack("transfer", common.HexToAddress(params.ToAddress), params.Value.BigInt())
		if err != nil {
			return nil, err
		}
		tx.Data = data
...
func SignTx2(tx *types.UnsignedTransaction, private string) (txhash string, rawTx string, err error) {
	acc := sdk.NewAccountManager("dagkeydir")
