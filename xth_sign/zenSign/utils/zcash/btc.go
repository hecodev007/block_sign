...
func (rpc *RpcClient) GetBlockByHash(h interface{}) (block Block, err error) {
...
func (rpc *RpcClient) GetBlockByHeight(h int64) (block Block, err error) {
	block, err = rpc.GetBlockByHash(fmt.Sprintf("%v", h))
	return
}
...
func (rpc *RpcClient) GetBlockByHeight2(h int64) (block BlockWithTx, err error) {
	...
	//	return
	//}
	block, err = rpc.GetBlockWithTxByHash(fmt.Sprintf("%v", h))
	if err != nil {
		log.Info(err.Error())
		return
	}
