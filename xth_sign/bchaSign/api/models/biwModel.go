...
import (
	"bchaSign/common"
	"bchaSign/common/conf"
	"bchaSign/common/log"
	"bchaSign/common/validator"
	util "bchaSign/utils/bcha"
	"bchaSign/utils/keystore"
	"bytes"
	"encoding/hex"
	"errors"
	"fmt"
...
func (m *BiwModel) NewAccount(params *validator.CreateAddressParams) (adds []string, err error) {
	...
	for i := 1; i <= params.Num; i++ {
		Legacy_addr, address, private, err := util.GenAccount()
		if err != nil {
			return nil, err
		}
		...
		cvsKeysD = append(cvsKeysD, &keystore.CsvKey{Address: Legacy_addr, Key: address})
	}
