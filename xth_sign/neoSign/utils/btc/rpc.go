...
//需要权限认证的RPC请求。
func (rpc *RpcClient) NeoCallWithAuth(method, credentials string, target interface{}, params ...interface{}) error {
	success, err := rpc.neocall(method, credentials, params...)
	if err != nil {
		fmt.Println("CallWithAuth", method, err.Error())
		return err
	}
...
func (rpc *RpcClient) neocall(method, credentials string, params ...interface{}) (bool, error) {
	request := request{
		ID:      "curltest",
		JSONRPC: "1.0",
		Method:  method,
	...
	data, err := ioutil.ReadAll(response.Body)
	if err != nil {
		return false, fmt.Errorf("ReadAll err: %v", err)
	}

	//rpc.log.Println(fmt.Sprintf("%s\nResponse: %s\n", method, data))
	resp := new(NeoResponse)
	if err := json.Unmarshal(data, resp); err != nil {
		return false, fmt.Errorf("resp: %v , err: %v", resp, err)
	}
