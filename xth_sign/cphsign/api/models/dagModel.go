...
import (
	...
	util "cphsign/utils/cph"
	"cphsign/utils/keystore"
	"errors"
	"fmt"
)
...
func (m *DagModel) GetPrivate(mchName, pubkey string) (private []byte, err error) {
	...
	//}
	//get mch akey
	pubkey = util.ToCphAddress(pubkey)
...
func (m *DagModel) SignTx(params *validator.TelosSignParams) (txhash, rawTx string, err error) {

	pri, err := m.GetPrivate(params.MchName, params.FromAddress)
	if err != nil {
		return
	}
	txhash, rawTx, err = util.BuildTx(params, string(pri))
	if err != nil {
		return
	}
