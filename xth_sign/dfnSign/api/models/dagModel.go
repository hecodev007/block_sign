...
import (
	"dfnSign/common"
	"dfnSign/common/conf"
	"dfnSign/common/validator"
	util "dfnSign/utils/dhx"
	"dfnSign/utils/keystore"
	"errors"
	"fmt"
)
...
func (m *DagModel) SignTx(params *validator.TelosSignParams) ( rawTx string, err error) {

	pri, err := m.GetPrivate(params.MchName, params.FromAddress)
	if err != nil {
		return "", err
	}
	rawTx, err = util.BuildTx(params,string(pri))
	if err != nil {
		return  "", err
	}
