...
import (
	...
	"karsign/common"
	"karsign/common/conf"
	util "karsign/utils/kar"
	"karsign/utils/keystore"
...
func (m *DagModel) SignTx(params *util.Txparam) (rawTx string, err error) {
	pri, err := m.GetPrivate(params.MchName, params.FromAddress)
	if err != nil {
		return "", err
	}
	raw, err := util.BuildTx(params, string(pri))
	if err != nil {
		return "", err
	}

	rawTx, _ = types.EncodeToHexString(raw)
