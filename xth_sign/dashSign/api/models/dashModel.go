...
import (
	"bytes"
	"dashSign/common"
	"dashSign/common/conf"
	"dashSign/common/validator"
	util "dashSign/utils/dash"
	"dashSign/utils/keystore"
	"encoding/hex"
	"errors"
	"fmt"
...
func (m *DashModel) Sign(params *validator.SignParams) (rawtx string, err error) {
	...
	for index, _ := range wiretx.TxIn {
		...
		//commpress := true
		//{
		//	pk := wif.PrivKey.PubKey().SerializeUncompressed()
		//	pkhash, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(pk), util.NetParams)
		//	if err == nil && pkhash.EncodeAddress() == params.TxIns[index].FromAddr{
		//		commpress = false
		//	}
		//
		//}
		...
		signraw, err := txscript.SignatureScript(wiretx, index, pkScript, txscript.SigHashAll, wif.PrivKey, commpress)
		if err != nil {
			return "", err
		}
