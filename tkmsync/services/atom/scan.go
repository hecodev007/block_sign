	...
	if len(block.Transactions) > 0 {
		if s.conf.EnableGoroutine {
			...
			for _, txid := range block.Transactions {
				if tx, err := s.GetTx(txid, block.Timestamp); err == nil {
...
//批量解析交易
func (s *Scanner) batchParseTx(jobs <-chan string, results chan<- int, blockhash string, blocktime time.Time, task *AtomProcTask) {
	count := len(jobs)
	offset := 0
	for i := 0; i < count; i++ {
		select {
...
// 解析交易
func parseBlockTX(tx *atom.Transaction, blockhash string) (*TxInfo, error) {
