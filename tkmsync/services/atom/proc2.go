...
type Processor2 struct {
	*atom.AtomhttpClient
	...
	mempool map[string]*AtomProcTask
	conf    conf.SyncConfig
}

func NewProcessor2(conf conf.Config, node conf.NodeConfig, watch *services.WatchControl) common.Processor {
	return &Processor2{
		AtomhttpClient: atom.NewAtomhttpClient(node.Url),
		watch:          watch,

		mempool: make(map[string]*AtomProcTask),
		conf:    conf.Sync,
	}
}

func (s *Processor2) RepushTxByIsInternal(userId int64, txid string, isInternal bool) error {
	panic("implement me")
}
func (s *Processor2) SetPusher(p common.Pusher) {
...
func (s *Processor2) RemovePusher() {
	s.pusher = nil
}

func (s *Processor2) RepushTx(userid int64, txid string) error {
	...
	bestHeight, err := s.GetLatestBlockHeight()
	if err != nil {
		return err
	}
...
func (s *Processor2) Info() (string, int64, error) {
	dbheight, err := dao.GetMaxBlockIndex()
	return s.conf.Name, dbheight, err
}

func (s *Processor2) Init() error {
	return nil
}

func (s *Processor2) Clear() {
}

func (s *Processor2) CheckIrreverseBlock(hash string) error {
...
func (s *Processor2) ProcIrreverseTxs(tmps []interface{}, bestHeight int64) error {
	if len(tmps) <= 0 {
			...
			go func(w *sync.WaitGroup) {
				...
				if err := s.processTX(txinfo, bestHeight, true); err == nil {
					if num, err := dao.InsertBlockTX(txinfo.tx); num <= 0 || err != nil {
						log.Infof("block tx insert err: %v", err)
					}
				} else {
					log.Infof("err : %v", err)
				}
...
func (s *Processor2) ProcReverseTxs(tmps []interface{}, bestHeight int64) error {
	if len(tmps) <= 0 {
		return fmt.Errorf("ProcIrreverseTxs don't support len is zero")
	}
	if s.conf.EnableGoroutine {
...
func (s *Processor2) ProcIrreverseBlock(b interface{}) error {
...
func (s *Processor2) UpdateIrreverseConfirms() {
	//查找所有未确认的区块
	...
	if err == nil && bs != nil && len(bs) > 0 {
		...
		if err := dao.BatchUpdateConfirmations(ids, 1); err != nil {
			log.Infof("batch update confirmations err: %v", err)
		}
	}
}

func (s *Processor2) UpdateReverseConfirms(b interface{}) {
	s.confirmsPush(b.(*dao.BlockInfo))
}

func (s *Processor2) getBlockTxInfosFromDB(txid string) (*TxInfo, error) {
...
func (s *Processor2) getBlockTxInfosFromNode(txid string) (*TxInfo, error) {
	if txid == "" {
...
// 解析交易信息到db
func (s *Processor2) processTX(txInfo *TxInfo, bestHeight int64, isStore bool) error {

	if txInfo == nil {
		return fmt.Errorf("tx is null")
	}

	log.Infof("processTX 1: %v, tx num : %d", txInfo, len(txInfo.txmsgs))
		...
		if s.conf.FullBackup {
			if isStore {
				if _, err := dao.InsertBlockTXMsg(txmsg); err != nil {
					log.Infof("insert block txmsg err : %v", err)
				}
			}
			...
			if s.watch.IsWatchAddressExist(txmsg.FromAddress) || s.watch.IsWatchAddressExist(txmsg.ToAddress) {
				if isStore {
					if _, err := dao.InsertBlockTXMsg(txmsg); err != nil {
						log.Infof("insert block txdetail err : %v", err)
					}
				}
				if txmsg.Status == 1 {
					careMsgs = append(careMsgs, txmsg)
	...
	if !s.conf.FullBackup {
		if len(tmpWatchList) <= 0 {
			log.Infof("dont't have care of watch address ")
	...
	log.Infof("processTX 2: %v, tx num : %d", txInfo, len(txInfo.txmsgs))
	return nil
}

func (s *Processor2) processPush(blocktx *dao.BlockTX, txmsgs []*dao.BlockTXMsg, tmpWatchList map[string]bool, bestHeight int64) error {
	...
	for _, txMsg := range txmsgs {
		...
		if txMsg.Type != atom.TypeMsgSend {
			continue
		}
		...
		pushtx := bo.PushAccountTx{
			...
			To:     txMsg.ToAddress,
			Fee:    atom.GetCoinNum(blocktx.Fee, s.conf.Name).String(),
			Amount: atom.GetCoinNum(txMsg.Amount, s.conf.Name).String(),
		}
...
func (s *Processor2) confirmsPush(blockInfo *dao.BlockInfo) error {
