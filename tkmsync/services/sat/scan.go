...
//批量解析交易
func (s *Scanner) batchParseRawTx(jobs <-chan string, results chan<- int, blockhash string, height int64, task *SatProcTask) {
	count := len(jobs)
	offset := 0
	for i := 0; i < count; i++ {
		select {
...
//解析交易
func parseBlockRawTX(coinName string, tx *sat.Transaction, blockhash string, height int64) (*TxInfo, error) {
