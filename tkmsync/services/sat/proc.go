...
func NewProcessor(conf conf.Config, node conf.NodeConfig, watch *services.WatchControl) common.Processor {
	...
	return &Processor{
		RpcClient: sat.NewRpcClient(node.Url, node.RPCKey, node.RPCSecret),
		watch:     watch,
		wg:        &sync.WaitGroup{},
		lock:      &sync.Mutex{},
		conf:      conf.Sync,
...
func (s *Processor) ProcIrreverseTxs(tmps []interface{}, bestHeight int64) error {
	...
	if err := dao.BatchInsertBlockTXVouts(insertVins); err != nil {
		log.Printf("Batch Insert Vins err : %v", err)
	}

	if err := dao.BatchUpdateBlockTXVouts(updateVins); err != nil {
		log.Printf("Batch Update Vins err : %v", err)
	}

	if err := dao.BatchInsertBlockTXs(txs); err != nil {
		log.Printf("Batch Insert TXs err : %v", err)
	}
...
func (s *Processor) processTXVouts(tmp []interface{}, height int64) {
	...
	if err := dao.BatchInsertBlockTXVouts(insertVouts); err != nil {
		log.Printf("block %d Batch Insert Vouts err : %v", height, err)
	}
...
func (s *Processor) getBlockTxInfosFromNode(txid string) (*TxInfo, error) {
	...
	return parseBlockRawTX(s.conf.Name, &tx, tx.BlockHash, block.Height)
}
