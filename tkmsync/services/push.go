...
func NewPushServer(cfg conf.PushConfig, watch *WatchControl) (*PushServer, error) {
	s := &PushServer{
		...
		for run {
			...
			case task := <-s.pushTaskList:
				if s.cfg.Enable {
					...
					if err != nil {
						log.Printf("SelectNotifyResult err : %v", err)
						break
					}
					notifyDB.Result = 0
					...
					//log.Printf(" push task : %v ,err : %v , need repush", task, err)
					// repush, 添加到重试队列
...
func (s *PushServer) AddPushTask(height int64, txid string, watchlist map[string]bool, pushdata []byte) {
	...
	for k := range watchlist {
		...
		if err != nil {
			log.Printf("don't find user info")
			break
		}
		for _, wui := range was {
			if users[wui.UserID] == "" {
...
func (s *PushServer) AddPushUserTask(height int64, pushdata []byte) {
	...
	if err != nil {
		log.Println("don't have user watch height")
	}
...
func (s *PushServer) httpPush(pt *PushTask) (string, error) {
	...
	if s.cfg.Agent {
		...
		log.Printf("推送数据为： %v", string(pt.TxData))
		req, err = http.NewRequest("POST", s.cfg.Url, bytes.NewBuffer(ds))
		if err != nil {
			log.Printf("url %s ,err : %v", s.cfg.Url, err)
		...
		if err != nil {
			log.Printf("url %s ,err : %v", pt.PushUrl, err)
			return fmt.Sprintf("url %s ,err : %v", pt.PushUrl, err), err
		}
	}
	log.Printf("NewRequest %v ", req)
	resp, err := s.client.Do(req)
	if err != nil {
		log.Printf("post %v ,err : %v", req.URL, err)
	...
	log.Printf(" push tx %s ", string(pt.TxData))
...
//			log.Printf("producer shutdown err : %v", err)
//		}
//	}
//	if err := s.rmq.Shutdown("disconnect with finish"); err != nil {
//		log.Printf("disconnectMQ err : %v", err)
//	}
//}
