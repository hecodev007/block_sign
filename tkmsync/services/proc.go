...
func (s *ProcServer) Start() {
	if err := s.processor.Init(); err != nil {
		log.Printf("initProcess, err :%v", err)
		return
	}
	log.Println("proc server start ")
	...
	go func() {
		...
		for run {
			...
			case task := <-s.procTasks:
				...
				log.Printf("processTask : %d ,tx: %d , used time: %f 's", task.GetHeight(), len(task.GetTxs()), time.Since(starttime).Seconds())
				break
			}
		}

		s.processor.Clear()
		log.Println("proc server shutdown")
		s.done <- struct{}{}
	}()
}
