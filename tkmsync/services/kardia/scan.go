...
import (
	...
	"math/big"
	"rsksync/conf"
	dao "rsksync/models/po/kardia"
	"rsksync/utils"
	"rsksync/utils/icap"
	"rsksync/utils/kardia"
	"strings"
	"sync"
	"time"
)
...
func NewScanner(conf conf.Config, node conf.NodeConfig, watch *services.WatchControl) common.Scanner {
	...
	return &Scanner{
		RpcClient: kardia.NewRpcClient(node.Url),
		lock:      &sync.Mutex{},
		conf:      conf.Sync,
		watch:     watch,
	}
...
//批量解析交易
func (s *Scanner) batchParseTx(tx *kardia.Transaction, bestHeight, blockTimestamp int64, task *KardiaProcTask, w *sync.WaitGroup) {
	defer w.Done()
...
// 解析交易
func (s *Scanner) parseBlockTX(tx *kardia.Transaction, bestHeight, blockTimestamp int64) ([]*dao.BlockTX, error) {
	...
	if !s.IsContractTx(tx) {
		...
		from, err := icap.ConvertAddressToICAP(common2.HexToAddress(tx.From))
		if err != nil {
			return nil, err
		}

		to, err := icap.ConvertAddressToICAP(common2.HexToAddress(tx.To))
		if err != nil {
			return nil, err
		}
		...
		if strings.ToLower(tx.To) != "0xa7de087329bfcda5639247f96140f9dabe3deed1" &&
			...
			txReceipt, err := s.GetTransactionReceipt(tx.Hash)
			if err != nil {
				log.Printf("%s 交易获取异常", txReceipt.TransactionHash)
				return nil, err
			}
			if txReceipt.Status != "0x1" {
				log.Printf("%s 无效交易", txReceipt.TransactionHash)
			...
			//}
			log.Printf("mykey kai合约入账解析: %s，Logs：%d", tx.Hash, len(txReceipt.Logs))
			for _, lg := range txReceipt.Logs {
				...
				log.Printf("mykey 解析数据解析：sender：%s,receiver:%s,am:%s", sender, receiver, amountFloatStr)
				if err != nil {
					log.Printf("mykey 解析异常 err:%s", err.Error())
					continue
				}
				if txid != tx.Hash {
					log.Printf("mykey txid 不一致")
					continue
				}

				am, _ := decimal.NewFromString(amountFloatStr)
				if sender == "" || receiver == "" || am.IsZero() {
					log.Printf("mykey txid 解析数据解析不全：sender：%s,receiver:%s,am:%s", sender, receiver, am.String())
					continue
				}
				...
				res = append(res, blocktx)
				log.Printf("添加mykey kai，txid：%s", txid)
			}
		} else {
			if errPTR := s.parseTxReceipt(tx, blockTimestamp, &res); errPTR != nil {
				return nil, errPTR
...
func (s *Scanner) parseTxReceipt(tx *kardia.Transaction, blockTimestamp int64, res *[]*dao.BlockTX) error {
	txReceipt, err := s.GetTransactionReceipt(tx.Hash)
	if err != nil {
		return err
	}
