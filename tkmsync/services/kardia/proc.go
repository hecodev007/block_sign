...
func (s *Processor) processPush(tmpWatchList map[string]bool, bestHeight int64, blocktxs ...*dao.BlockTX) error {
	...
	fee := decimal.New(blocktxs[0].GasPrice, 0).
		...
		Shift(int32(0 - kardia.WEI)).String()
...
func (s *Processor) getBlockTxFromETHAPI(txid string) ([]*dao.BlockTX, map[string]bool, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("getInternalKaiTx err : %s ", err.Error())
	}
	...
	if internalData.Status != "1" && len(internalData.Result) == 0 && internalData.Message != "OK" {
		return nil, nil, errors.New("getInternalKaiTx err DATA")
	}
	for _, v := range internalData.Result {
		...
		from, err := icap.ConvertAddressToICAP(common2.HexToAddress(v.From))
		if err != nil {
			return nil, nil, err
		}

		to, err := icap.ConvertAddressToICAP(common2.HexToAddress(v.To))
		if err != nil {
			return nil, nil, err
		}

		if s.watch.IsWatchAddressExist(strings.ToLower(from)) {
			watchs[strings.ToLower(from)] = true
			has = true
		}
		if s.watch.IsWatchAddressExist(strings.ToLower(to)) {
			watchs[strings.ToLower(to)] = true
			has = true
		}
