...
type BlockInfo struct {
	...
	ChainID        string    `json:"chainid" gorm:"column:chainid"`
}
...
func BatchInsertBlockInfo(bs []*BlockInfo) error {
	...
	for _, b := range bs {
		vals = append(vals, b.Height, b.Hash, b.ChainID, b.FrontBlockHash, b.NextBlockHash,
			b.Timestamp, b.Transactions, b.Confirmations, b.CreateTime)
	}
	buf := bytes.NewBufferString(`INSERT INTO block_info(height,hash,chainid,previousblockhash,nextblockhash,timestamp,transactions,confirmations,createtime) VALUES `)
...
func InsertBlockInfo(b *BlockInfo) (int64, error) {
	...
	if b.Timestamp.Unix() < 0 {
		b.Timestamp = time.Date(1970, 1, 2, 0, 0, 0, 0, time.Local).UTC()
	}
	fmt.Println(b.Timestamp.Unix())
