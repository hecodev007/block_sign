...
func FindAddressesInfos(cointype string, startId int64) ([]AddressesInfo, error) {
	var as []AddressesInfo
	err := db.UserDB.DB.Where("coin_type = ? and status = 'used' and id > ?", cointype, startId).Find(&as).Error
	if err != nil {
		return nil, err
	}
...
func FindContractInfos(coinType string, startId int64) ([]ContractInfo, error) {
	var cs []ContractInfo
	err := db.UserDB.DB.Debug().Where("coin_type = ? and invaild = ? and id>?", coinType, 0, startId).Find(&cs).Error
	if err != nil {
		return nil, err
	}
