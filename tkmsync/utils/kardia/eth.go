...
func (rpc *RpcClient) BlockNumber() (int64, error) {
	...
	if strings.HasPrefix(response, "0x") {
		...
		return 0, errors.New("获取高度失败")
	}

	if s, err := strconv.ParseInt(response, 16, 32); err != nil {
		return 0, err
	} else {
		return s, nil
	}
...
func (rpc *RpcClient) GetBlockByNumber(number int64, withTransactions bool) (*Block, error) {
	hex := utils.Int64ToHex(number)
	return rpc.getBlock("eth_getBlockByNumber", withTransactions, hex, withTransactions)
}
...
type proxyBlockWithTransactionsTkm struct {
	Number           string             `json:"number"`
	Hash             string             `json:"hash"`
	ParentHash       string             `json:"parentHash"`
	Nonce            string             `json:"nonce"`
...
func (proxy *proxyBlockWithTransactionsTkm) toBlock() Block {
...
func (rpc *RpcClient) getBlock(method string, withTransactions bool, params ...interface{}) (*Block, error) {
	...
	if withTransactions {
		response = new(proxyBlockWithTransactionsTkm)
	} else {
