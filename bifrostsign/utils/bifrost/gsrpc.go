...
func (c *Client) GetAccountInfo(address string, meta *types.Metadata) (AccountInfo, error) {
	var accountInfo AccountInfo

	pubKey := GetPublicFromAddr(address, []byte{0x06})
...
func (c *Client)AccountInfo(address string)(balance int64,nonce int64,err error){
	acc ,err :=c.GetAccountInfo(address,c.Meta)
	if err != nil {
		return 0, 0, err
	}
	return int64(acc.Data.Free.Int64()),int64( acc.Nonce),nil
}
func (Api *Client)GetNonce(address string)(nonce uint64,err error){
