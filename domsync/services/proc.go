...
func (s *ProcServer) Start() {
	...
	log.Printf("proc server start :%d", s.StartHeight)
	...
	go func() {
		...
		for run {
			...
			default:
				...
				if !ok {
					//log.Printf("获取but失败等待", s.StartHeight, bullet)
				...
				if !ok {
					//log.Printf("获取高度失败等待", s.StartHeight, bullet)
					time.Sleep(time.Second * 1)
					break
				}
				...
				if t.GetHeight()%10 == 0 {
					log.Printf("处理不可逆交易%d,%d", t.GetHeight(), t.GetBestHeight())
				}
				...
				//log.Printf("processTask : %d , used time: %f 's", s.StartHeight, time.Since(startime).Seconds())
				break
			}
		}

		s.processor.Clear()
		log.Printf("proc server shutdown")
		s.done <- struct{}{}
	}()
}
...
func (s *ProcServer) AddReverseTask(t common.ProcTask) {
	log.Printf("处理可逆块：%d", t.GetHeight())
	...
	if v, ok := s.mpload(t.GetBlockHash()); !ok {
		...
		//log.Printf(hasWatchAddr, t.GetBlockHash())
...
func (s *ProcServer) AddIrrevTask(t common.ProcTask) {
	...
	//log.Printf("AddIrrevTask success:", t.GetHeight())
}
