...
func (s *ScanServer) Start() {
	...
	log.Printf("scan server start")
	go func() {
		run := true
		for run {
			select {
			case <-s.stop:
				log.Printf("scan server stop")
				run = false
				break
			default:
		...
		s.scanner.Clear()
		log.Printf("scan server shutdown")
		s.done <- struct{}{}
	}()
}
...
func (s *ScanServer) scanData() (err error) {
	...
	if s.irrevScanHeight == 0 {
		...
		if err != nil {
			log.Printf("GetCurrentBlockHeight %v", err)
			return err
		}
	...
	log.Printf("当前扫描区块:%d,最高区块 : %d ", s.irrevScanHeight, s.bestHeight)

	//需要执行的块个数
	gapCount := s.bestHeight - s.irrevScanHeight + 1
	if gapCount <= 0 {
		//如果达到最高高度就睡眠一定时间,是否是配置有问题
		log.Printf("already sacn reach best height: %d, current: %d", s.bestHeight, s.irrevScanHeight)
		//time.Sleep(time.Second * (3 * time.Duration(int64((gapCount)*-1))))
		return nil
	}

	//log.Printf("gapCount", gapCount, "s.conf.Confirmations", s.conf.Confirmations)
	//扫描不可逆区块
	if gapCount > s.conf.Confirmations {
		...
		for endHeight := s.bestHeight - s.conf.Confirmations; s.irrevScanHeight <= endHeight; s.irrevScanHeight++ {
			workPool.Incr()
			//			log.Printf("workPool start", workPool.Running, s.irrevScanHeight, endHeight)
			go func(height, bestHeight int64) {
				defer workPool.Dec()
			ScanIrreverseBlock:
				task, err := s.scanner.ScanIrreverseBlock(height, bestHeight)
				//defer log.Printf("workPool end", workPool.Running, height, bestHeight)
				if err != nil {
	...
	if s.conf.EnableMempool {
		...
		for height := s.irrevScanHeight; height <= s.bestHeight; height++ {
			...
			if err != nil {
				log.Printf("scanBlock err: %v height:%d bestheight:%d", err, height, s.bestHeight)
				continue
			}
			//log.Printf(s.irrevScanHeight, s.bestHeight, tmp.GetIrreversible(), tmp.GetConfirms(), tmp.GetIrreversible())
	...
	log.Printf("scanData:%d's block, with use time : %d s \n\n", gapCount, int64(time.Since(startime).Seconds()))
	//已达到最高度，减缓扫描频率
