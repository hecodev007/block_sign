...
import (
	...
	"github.com/group-coldwallet/ltcserver/api"
	"github.com/group-coldwallet/ltcserver/api/e"
	"github.com/group-coldwallet/ltcserver/conf"
	...
	"github.com/group-coldwallet/ltcserver/service/ltcservice"
	"io/ioutil"
	"net"
	"net/http"
	"net/url"
...
type API struct {
	ltcService ltcservice.BasicService
}

//实例化指定版本的service
func NewLtcAPI() *API {
	return &API{
		ltcService: new(ltcservice.LtcService),
	}
}
...
func (a *API) CreateTpl(c *gin.Context) {
	...
	if txinput.Fee < conf.GlobalConf.LtcCfg.MinFee {
		err = fmt.Errorf("Fee too low,min:%d ,now:%d", conf.GlobalConf.LtcCfg.MinFee, txinput.Fee)
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		return
	}

	if txinput.Fee > conf.GlobalConf.LtcCfg.MaxFee {
		err = fmt.Errorf("Fee too high,max:%d ,now:%d", conf.GlobalConf.LtcCfg.MaxFee, txinput.Fee)
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		return
	}
	...
	for i, v := range txinput.Txins {
		...
		tpl.TxIns = append(tpl.TxIns, bo.LtcTxInTpl{
			FromAddr:         v.Address,
			FromTxid:         v.Txid,
			FromAmount:       v.Amount,
	...
	for i, v := range txinput.Txouts {
		...
		toAmount += v.ToAmount
		tpl.TxOuts = append(tpl.TxOuts, bo.LtcTxOutTpl{
			ToAddr:   v.ToAddress,
			ToAmount: v.ToAmount,
		})
	}
...
func (a *API) SignTx(c *gin.Context) {
	...
	hex, err := a.ltcService.SignTx(tpl)
	if err != nil {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), vo.SignResult{
			MchInfo: tpl.MchInfo,
		})
...
func (a *API) SendTx(c *gin.Context) {
	...
	sendUrl := conf.GlobalConf.LtcCfg.PushServers
	if len(sendUrl) == 0 {
...
// @Summary 生成地址
// @Produce  json
// @param body body string true "data"
// @Success 200 {string} json "{"code":0,"message":"ok","data":{},"hash":"8978608dad8f150ea142e1c076f6564e"}"
// @Router /v1/addr [post]
func (a *API) CreateAddrs(c *gin.Context) {
	...
	filepath := conf.GlobalConf.LtcCfg.CreateAddrPath
	copyPath := conf.GlobalConf.LtcCfg.AddrPath
	resultVo, err := a.ltcService.CreateAddr(params, filepath, copyPath)
	if err != nil {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		return
	}
