...
type ZecServiceV2 struct {
}

func NewZecServiceV2() BasicService {
	return &ZecServiceV2{}
}
func (zec *ZecServiceV2) SignTx(tpl *bo.ZecTxTpl) (hex string, err error) {
	var (
		fromAmount int64
		toAmount   int64
		fee        int64
...
func (zec *ZecServiceV2) zecSignTxTpl(tpl *bo.ZecTxTpl) (hex string, err error) {

	if tpl.Hex == "" {
		return "", errors.New("zecSignTxTpl empty hex")
	}
	...
	//获取create结果
	//createResult, err := rylink.ZecRpcClient.Createtx(vins, vouts)
	//if err != nil {
	//	return "", err
	//}
	//if createResult.Error != nil {
	//	return "", fmt.Errorf("create error :%+v", createResult.Error)
	//}

	signTpl := rylink.Signtx{
		Rawtx:       tpl.Hex,
		Prevtxs:     prevtxs,
	...
	//结果进行签名
	signResult, err := rylink.ZecRpcClient.SignTx(signTpl)
	if err != nil {
		return "", err
	}
	if signResult.Error != nil {
		return "", fmt.Errorf("sign error :%+v", signResult.Error)
	}
	if !signResult.Result.Complete {
		return "", fmt.Errorf("sign error :%v", signResult.Result.Errors)
	}
	return signResult.Result.Hex, nil

}

func (zec *ZecServiceV2) CreateAddr(params *bo.CreateAddrParam, createPath, readPath string) (*vo.CreateAddrResult, error) {
	...
	for i := 0; i < params.Num; i++ {
		addrInfo, err := rylink.ZecRpcClient.GetNewAddress()
		if err != nil {
