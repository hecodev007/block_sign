...
//读取指定文件
func (s *LoadService) ReadFile(fileAPath, fileBPath string, addressIndex int) {
	if fileAPath == "" || fileBPath == "" {
		logrus.Error("ReadFile加载指定地址文件,empty path")
		return
	}
	logrus.Infof("ReadFile 加载指定地址文件，A:%s,B:%s", fileAPath, fileBPath)
	encryptWifMap := readUsbFileA(fileAPath, addressIndex)
	wifKeyListMap := readUsbFileB(fileBPath, addressIndex)
	for k, _ := range encryptWifMap {
...
//新文件地址在下标0 ,旺旺旧文件地址下标1
func readUsbFileA(usb_a string, addressIndex int) map[string]string {
	...
	for i := 0; i < len(encryptList); i++ {
		encryptWifMap[encryptList[i][addressIndex]] = encryptList[i][addressIndex+1]
	}
	return encryptWifMap
}

//新文件地址在下标0 ,旺旺旧文件地址下标1
func readUsbFileB(usb_b string, addressIndex int) map[string]string {
	...
	for i := 0; i < len(keyList); i++ {
		...
		wifKeyListMap[keyList[i][addressIndex]] = keyList[i][addressIndex+1]
	}
...
//读取所有
func (s *LoadService) ReadOleFolder(folderpath string) {
	// 读取csv目录
