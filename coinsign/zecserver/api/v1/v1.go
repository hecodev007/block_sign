...
import (
	...
	"github.com/group-coldwallet/zecserver/api"
	...
	"github.com/group-coldwallet/zecserver/service/zecservice"
...
type APIV1 struct {
	zecService zecservice.BasicService
}

//实例化指定版本的service
func NewZecAPI() api.API {
	if conf.GlobalConf.EnableRPC {
		return &APIV1{
			//zecService: zecservice.NewZecServiceV2(),
			zecService: zecservice.NewZecServiceV2(),
		}
	} else {
		return &APIV1{
			//zecService: zecservice.NewZecServiceV2(),
			zecService: zecservice.NewZecServiceV1(),
		}
	}
...
func (a *APIV1) CreateTpl(c *gin.Context) {
	var (
		err          error
		fromAmount   int64
		toAmount     int64
	...
	if txinput.ExpiryHeight < 923229 {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, "error ExpiryHeight:", nil)
		return
	}
	tpl.ExpiryHeight = txinput.ExpiryHeight
	//校验参数
	...
	if txinput.Fee < conf.GlobalConf.ZecCfg.MinFee {
		err = fmt.Errorf("Fee too low,min:%d ,now:%d", conf.GlobalConf.ZecCfg.MinFee, txinput.Fee)
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		return
	}

	if txinput.Fee > conf.GlobalConf.ZecCfg.MaxFee {
		err = fmt.Errorf("Fee too high,max:%d ,now:%d", conf.GlobalConf.ZecCfg.MaxFee, txinput.Fee)
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		return
	}
	...
	for i, v := range txinput.Txins {
		...
		//if v.ScriptPubKey == "" {
		//	err = fmt.Errorf("Txins index:%d ,address:%s,error,empty ScriptPubKey", i, v.Address)
		//	api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		//	return
		//}
		if v.Amount < 0 {
		...
		tpl.TxIns = append(tpl.TxIns, bo.ZecTxInTpl{
			FromAddr:         v.Address,
			FromTxid:         v.Txid,
			FromAmount:       v.Amount,
			FromIndex:        uint32(v.Vout),
			FromScriptPubKey: v.ScriptPubKey,
			FromRedeemScript: v.RedeemScript,
		})
	}

	//临时存储输出地址，rpc无法制造两个一样的输出
	//addrMap := make(map[string]string)
	for i, v := range txinput.Txouts {
		//if _, ok := addrMap[v.ToAddress]; ok {
		//	//存在
		//	api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, fmt.Errorf("duplicated address:%s,目前不支持两个相同的输出地址", v.ToAddress).Error(), nil)
		//	return
		//}
		...
		toAmount += v.ToAmount
		tpl.TxOuts = append(tpl.TxOuts, bo.ZecTxOutTpl{
			ToAddr:   v.ToAddress,
			ToAmount: v.ToAmount,
		})
		//addrMap[v.ToAddress] = v.ToAddress
	}
...
// @Success 200 {string} json "{"code":0,"message":"ok","data":{},"hash":"8978608dad8f150ea142e1c076f6564e"}"
// @Router /v1/sign [post]
func (a *APIV1) SignTx(c *gin.Context) {
	...
	hex, err := a.zecService.SignTx(tpl)
	if err != nil {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), vo.SignResult{
			MchInfo: tpl.MchInfo,
		})
...
func (a *APIV1) GetPrivkey(c *gin.Context) {
...
// @Success 200 {string} json "{"code":0,"message":"ok","data":{},"hash":"8978608dad8f150ea142e1c076f6564e"}"
// @Router /v1/push [post]
func (a *APIV1) SendTx(c *gin.Context) {
	...
	result, err := rylink.ZecRpcClient.PushTransaction(pushParam.Hex)
	...
	if result.Error != "" {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, fmt.Errorf("push error:%s", result.Error).Error(), nil)
		return
	}
	resultMap := make(map[string]interface{})
	resultMap["txid"] = result.Result
	api.HttpResponseByMsg(c, http.StatusOK, e.SUCCESS, "success", resultMap)
}

// @Summary 生成地址,冷系统有效，热系统无效
// @Produce  json
// @param body body string true "{'num':10,'orderId':'123456','mchId':'test','coinName':'zec'}"
// @Success 200 {string} json "{"code":0,"message":"ok","data":{},"hash":"8978608dad8f150ea142e1c076f6564e"}"
// @Router /v1/addr [post]
func (a *APIV1) CreateAddrs(c *gin.Context) {
	...
	filepath := conf.GlobalConf.ZecCfg.CreateAddrPath
	copyPath := conf.GlobalConf.ZecCfg.AddrPath
	resultVo, err := a.zecService.CreateAddr(params, filepath, copyPath)
	if err != nil {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, err.Error(), nil)
		return
	}
...
func (a *APIV1) CheckAddr(c *gin.Context) {
...
func (a *APIV1) ImportAddr(c *gin.Context) {
	if conf.GlobalConf.SystemModel != "cold" {
		api.HttpResponseByMsg(c, http.StatusOK, e.ERROR, "Model is not cold", nil)
		return
	}
