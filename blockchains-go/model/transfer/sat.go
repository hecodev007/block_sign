...
type SatOrderRequest struct {
	OrderRequestHead
	Amount       int64                  `json:"amount,omitempty"`
	Fee          int64                  `json:"fee,omitempty"`
	OrderAddress []*SatOrderAddrRequest `json:"order_address,omitempty"`
}

type SatOrderAddrRequest struct {
...
//====================Sat验证地址====================
type SatAddressResp struct {
	Code    int               `json:"code"`
	Data    *SatAddressResult `json:"data"`
	Message string            `json:"message"`
}
type SatAddressResult struct {
...
//Sat unspents切片排序
type SatUnspentDesc []SatUtxo

//实现排序三个接口
//为集合内元素的总数
func (s SatUnspentDesc) Len() int {
	return len(s)
}

//Swap 交换索引为 i 和 j 的元素
func (s SatUnspentDesc) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

//从大到小，最大金额排序
//如果index为i的元素大于index为j的元素，则返回true，否则返回false
func (s SatUnspentDesc) Less(i, j int) bool {
	//return s[i].Amount > s[j].Amount
	return s[i].AmountInt64.GreaterThan(s[j].AmountInt64)
}

//Sat unspents切片排序
type SatUnspentAsc []SatUtxo

//实现排序三个接口
//为集合内元素的总数
func (s SatUnspentAsc) Len() int {
	return len(s)
}

//Swap 交换索引为 i 和 j 的元素
func (s SatUnspentAsc) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

//从小到大，最小金额排序
//如果index为i的元素大于index为j的元素，则返回true，否则返回false
func (s SatUnspentAsc) Less(i, j int) bool {
	//return s[i].Amount < s[j].Amount
...
func DecodeSatGasResult(ds []byte) (*SatGasResult, error) {
	ri := &SatGasResult{}
...
func DecodeSatAddressResult(data []byte) *SatAddressResp {
	if len(data) != 0 {
		result := new(SatAddressResp)
...
type SatTxTpl struct {
	MchId    string        `json:"mchId,omitempty"`
	OrderId  string        `json:"orderId,omitempty"`
	CoinName string        `json:"coinName"`
	TxIns    []SatTxInTpl  `json:"txIns"`
	TxOuts   []SatTxOutTpl `json:"txOuts"`
}

//utxo模板
type SatTxInTpl struct {
