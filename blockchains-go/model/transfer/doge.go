...
type DogeOrderRequest struct {
	OrderRequestHead
	Amount       int64                   `json:"amount,omitempty"`
	Fee          int64                   `json:"fee,omitempty"`
	OrderAddress []*DogeOrderAddrRequest `json:"order_address,omitempty"`
}

type DogeOrderAddrRequest struct {
...
//====================Doge验证地址====================
type DogeAddressResp struct {
	Code    int                `json:"code"`
	Data    *DogeAddressResult `json:"data"`
	Message string             `json:"message"`
}
type DogeAddressResult struct {
...
//Doge unspents切片排序
type DogeUnspentDesc []DogeUtxo

//实现排序三个接口
//为集合内元素的总数
func (s DogeUnspentDesc) Len() int {
	return len(s)
}

//Swap 交换索引为 i 和 j 的元素
func (s DogeUnspentDesc) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

//从大到小，最大金额排序
//如果index为i的元素大于index为j的元素，则返回true，否则返回false
func (s DogeUnspentDesc) Less(i, j int) bool {
	return s[i].Amount > s[j].Amount
}

//Doge unspents切片排序
type DogeUnspentAsc []DogeUtxo

//实现排序三个接口
//为集合内元素的总数
func (s DogeUnspentAsc) Len() int {
	return len(s)
}

//Swap 交换索引为 i 和 j 的元素
func (s DogeUnspentAsc) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

//从小到大，最小金额排序
//如果index为i的元素大于index为j的元素，则返回true，否则返回false
func (s DogeUnspentAsc) Less(i, j int) bool {
	return s[i].Amount < s[j].Amount
}

//========================json解析====================

func DecodeDogeGasResult(ds []byte) (*DogeGasResult, error) {
	ri := &DogeGasResult{}
...
func DecodeDogeAddressResult(data []byte) *DogeAddressResp {
	if len(data) != 0 {
		result := new(DogeAddressResp)
