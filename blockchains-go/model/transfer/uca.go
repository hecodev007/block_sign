...
type UcaOrderRequest struct {
	OrderRequestHead
	Amount       int64                  `json:"amount,omitempty"`
	Fee          int64                  `json:"fee,omitempty"`
	OrderAddress []*UcaOrderAddrRequest `json:"order_address,omitempty"`
}

type UcaOrderAddrRequest struct {
...
//====================Uca验证地址====================
type UcaAddressResp struct {
	Code    int               `json:"code"`
	Data    *UcaAddressResult `json:"data"`
	Message string            `json:"message"`
}
type UcaAddressResult struct {
...
//Uca unspents切片排序
type UcaUnspentDesc []UcaUtxo

//实现排序三个接口
//为集合内元素的总数
func (s UcaUnspentDesc) Len() int {
	return len(s)
}

//Swap 交换索引为 i 和 j 的元素
func (s UcaUnspentDesc) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

//从大到小，最大金额排序
//如果index为i的元素大于index为j的元素，则返回true，否则返回false
func (s UcaUnspentDesc) Less(i, j int) bool {
	return s[i].Amount > s[j].Amount
}

//Uca unspents切片排序
type UcaUnspentAsc []UcaUtxo

//实现排序三个接口
//为集合内元素的总数
func (s UcaUnspentAsc) Len() int {
	return len(s)
}

//Swap 交换索引为 i 和 j 的元素
func (s UcaUnspentAsc) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

//从小到大，最小金额排序
//如果index为i的元素大于index为j的元素，则返回true，否则返回false
func (s UcaUnspentAsc) Less(i, j int) bool {
	return s[i].Amount < s[j].Amount
}

//========================json解析====================

func DecodeUcaGasResult(ds []byte) (*UcaGasResult, error) {
	ri := &UcaGasResult{}
...
func DecodeUcaAddressResult(data []byte) *UcaAddressResp {
	if len(data) != 0 {
		result := new(UcaAddressResp)
