...
type HecoDingService struct {
	*BaseDingService
}

func (bds *BaseDingService) NewHecoDingService() *HecoDingService {
	eds := new(HecoDingService)
	eds.BaseDingService = bds
	return eds
}

func (eds *HecoDingService) TransferFee(feeAddr, toAddr string, appId int64, feeApply *entity.FcTransfersApply, fee decimal.Decimal) error {
	//1. 构建交易数据
...
func (eds *HecoDingService) CollectToken(name, to string, mch *entity.FcMch, fromAddresses []string, tokenCoinSet *entity.FcCoinSet) error {
	var errMsg = "代币归集有错误："
	var success int
...
func (eds *HecoDingService) walletServerCreateHot(orderReq *transfer.HecoOrderRequest, coinName string) (string, error) {
...
func (eds *HecoDingService) FindCoinFee(mainName, address string, mch *entity.FcMch) (chainAmount string, err error) {
...
func (eds *HecoDingService) getChainBalance(coinName, address, contract string) (string, error) {
	...
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/heco/getBalance", cfg.Url), cfg.User, cfg.Password, req)
	if err != nil {
		return "", err
	}
