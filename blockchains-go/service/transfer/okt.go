...
type OktTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewOktTransferService() service.TransferService {
	return &OktTransferService{
		CoinName: "okt",
		Lock:     &sync.Mutex{},
	}
}

func (s *OktTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid okt address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *OktTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *OktTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *OktTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *OktTransferService) walletServerCreateHot(orderReq *transfer.OktOrderRequest) (string, error) {
...
func (s *OktTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.OktOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(OktDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *OktTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
