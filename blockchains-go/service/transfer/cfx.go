...
type CfxTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewCfxTransferService() service.TransferService {
	return &CfxTransferService{
		CoinName: "cfx",
		Lock:     &sync.Mutex{},
	}
}

func (s *CfxTransferService) VaildAddr(address string) error {
	//if !strings.HasPrefix(address, "0x1") {
	//	return fmt.Errorf("address is not start `0x1`")
	//}
	//if !common.IsHexAddress(address) {
	//	return errors.New("valid cfx address error")
	//}
	//return nil
...
func (s *CfxTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *CfxTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	return errors.New("do not support cold transfer")
}

//=================私有方法=================
//创建交易接口参数
func (s *CfxTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *CfxTransferService) walletServerCreateHot(orderReq *transfer.CfxOrderRequest) (string, error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	if !ok {
...
func (s *CfxTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.CfxOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	*/
	if ta.Eostoken != "" { //如果是代币转账
		...
		orderReq.Amount = toAmount.Shift(int32(CfxDecimal)).String()
	}
