...
type ZecTransferService struct {
	CoinName string
}

func NewZecTransferService() service.TransferService {
	return &ZecTransferService{
		CoinName: "zec",
	}
}

func (srv *ZecTransferService) VaildAddr(address string) error {
	url := conf.Cfg.CoinServers[srv.CoinName].Url + "/api/v1/zec/validateaddress?address=%s"
	...
	zecResp := transfer.DecodeZecAddressResult(data)
	if zecResp != nil && zecResp.Data != nil {
		if zecResp.Data.Isvalid {
			return nil
		}
	}
...
func (srv *ZecTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	//无需实现
	return "", errors.New("implement me")
}

func (srv *ZecTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//随机选择可用机器
...
//params worker 指定机器
func (srv *ZecTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.ZecOrderRequest, error) {
	var (
	...
	if len(changes) == 0 {
		return nil, fmt.Errorf("zec 商户=[%d],查询zec找零地址失败", ta.AppId)
	}
	//随机选择
	...
	if err != nil {
		log.Errorf("下单表订单id：%d,zec 交易手续费转换异常:%s", ta.Id, err.Error())
		return nil, err
	}
	//zec api需要金额整型
	...
	//todo Worker读取数据库
	orderReq := &transfer.ZecOrderRequest{
...
//创建交易接口参数
func (srv *ZecTransferService) walletServerCreate(orderReq *transfer.ZecOrderRequest) error {
...
//fee 手续费
func (srv *ZecTransferService) setUtxoData(appid int64, orderReq *transfer.ZecOrderRequest, toAddr, changeAddr string) error {
	var (
	...
	utxoTpl := make([]*transfer.ZecOrderAddrRequest, 0) //utxo模板
	...
	zecCfg = conf.Cfg.CoinServers[srv.CoinName]
	if zecCfg == nil {
	...
	var sortUtxo transfer.ZecUnspentDesc

	var sortUtxoTmp transfer.ZecUnspentAsc //临时使用，小金额排序
	...
	for _, v := range sortUtxo {
		...
		oar := &transfer.ZecOrderAddrRequest{
	...
	//组装to
	utxoTpl = append(utxoTpl, &transfer.ZecOrderAddrRequest{
...
//手续费计算
func (srv *ZecTransferService) getFee(inNum, outNum int) (int64, error) {
...
func (srv *ZecTransferService) findExpiryHeight() (int64, error) {
