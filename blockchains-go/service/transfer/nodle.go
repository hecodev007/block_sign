...
type NodleTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewNodleTransferService() service.TransferService {
	return &NodleTransferService{
		CoinName: "nodle",
		Lock:     &sync.Mutex{},
	}
}
...
var (
	NodlePrefix = byte(0x25)
)

func (s *NodleTransferService) VaildAddr(address string) error {
	if address == "" {
...
func (s *NodleTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *NodleTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *NodleTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *NodleTransferService) walletServerCreateHot(orderReq *transfer.NodleOrderRequest) (string, error) {
...
func (s *NodleTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.NodleOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	//
	fromAmount := toAmount.Add(decimal.NewFromFloat(0.0027))
	...
	if len(fromAddrs) == 0 {
		return nil, fmt.Errorf("outorderNo:%s 没有符合条件的出账地址，大于0.0027 \n amount: %v \n to: %s \n ", ta.OutOrderid, toAddrs[0].ToAmount, toAddr)
	}
