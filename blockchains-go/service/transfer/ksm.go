...
type KsmTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewKsmTransferService() service.TransferService {
	return &KsmTransferService{
		CoinName: "ksm",
		Lock:     &sync.Mutex{},
	}
}
...
var (
	KSMPrefix   = byte(0x02)
	ksmSSPrefix = []byte{0x53, 0x53, 0x35, 0x38, 0x50, 0x52, 0x45}
)

func (s *KsmTransferService) VaildAddr(address string) error {
	if address == "" {
	...
	var input []byte
	input = append(input, ksmSSPrefix...)
...
func (s *KsmTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *KsmTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *KsmTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *KsmTransferService) walletServerCreateHot(orderReq *transfer.KsmOrderRequest) (string, error) {
...
func (s *KsmTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.KsmOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
