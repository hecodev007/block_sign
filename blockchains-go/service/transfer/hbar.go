...
type HbarTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewHbarTransferService() service.TransferService {
	return &HbarTransferService{
		CoinName: "hbar",
		Lock:     &sync.Mutex{},
	}
}

func (s *HbarTransferService) VaildAddr(address string) error {
...
func (s *HbarTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *HbarTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *HbarTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *HbarTransferService) walletServerCreateHot(orderReq *transfer.HbarOrderRequest) (string, error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	if !ok {
...
func (s *HbarTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.HbarOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(HbarDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *HbarTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
