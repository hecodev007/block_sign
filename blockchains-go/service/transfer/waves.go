...
const WavesDecimal = 8

type WavesTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewWavesTransferService() service.TransferService {
	return &WavesTransferService{
		CoinName: "waves",
		Lock:     &sync.Mutex{},
	}
}

func (tt *WavesTransferService) VaildAddr(address string) error {
...
func (s *WavesTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.WavesOrderRequest
	...
	if orderReq.ContractAddress != "" {
		...
		if tokenCoinSet == nil {
			return "", errors.New("没找到代币" + ta.Eoskey)
		}
		orderReq.ContractAddress = tokenCoinSet.Token
...
func (s *WavesTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *WavesTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *WavesTransferService) walletServerCreateHot(orderReq *transfer.WavesOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeWavesTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *WavesTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.WavesOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(WavesDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *WavesTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
