...
type RonTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewRonTransferService() service.TransferService {
	return &RonTransferService{
		CoinName: "ron",
		Lock:     &sync.Mutex{},
	}
}

func (s *RonTransferService) VaildAddr(address string) error {
	toAddress, err := icap.ConvertICAPToAddressRon(address)
	if err != nil {
		return err
	}
	if !common.IsHexAddress(toAddress.String()) {
		return errors.New("valid ron address error")
	}
	return nil
}

func (s *RonTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.RonOrderRequest
...
func (s *RonTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *RonTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *RonTransferService) walletServerCreateHot(orderReq *transfer.RonOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeRonTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *RonTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.RonOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(RonDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *RonTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
