...
type DomTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewDomTransferService() service.TransferService {
	return &DomTransferService{
		CoinName: "dom",
		Lock:     &sync.Mutex{},
	}
}

func (s *DomTransferService) VaildAddr(address string) error {

	if !strings.HasPrefix(address, "1") && len(address) < 26 {
		return fmt.Errorf("dom error address:%s", address)
	}
	var addressChecksumLen = 4
	pubKeyHash := base58.Decode(address)
	if len(pubKeyHash) < 5 {
		return fmt.Errorf("dom pubKeyHash error address:%s", address)
	}
	actualChecksum := pubKeyHash[len(pubKeyHash)-addressChecksumLen:]
	version := pubKeyHash[0]
	pubKeyHash = pubKeyHash[1 : len(pubKeyHash)-addressChecksumLen]
	targetChecksum := s.checksum(append([]byte{version}, pubKeyHash...))
	if !(bytes.Compare(actualChecksum, targetChecksum) == 0) {
		return fmt.Errorf("dom  checksum error address:%s", address)
	}
	return nil
}

func (s *DomTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.DomOrderRequest
	...
	orderHot := &entity.FcOrderHot{
		...
		FromAddress:  orderReq.Data.From,
		ToAddress:    orderReq.Data.To,
...
func (s *DomTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
func (s *DomTransferService) walletServerCreateHot(orderReq *transfer.DomOrderRequest) (string, error) {
	...
	if thr.Code != 0 || thr.Data == "" {
		return "", fmt.Errorf("order表 请求下单接口返回值失败,服务器返回异常，outOrderId：%s，err:%s", orderReq.OrderID, string(data))
	}
	txid := fmt.Sprintf("%v", thr.Data)
	return txid, nil
}
func (s *DomTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.DomOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	orderReq := &transfer.DomOrderRequest{
		...
		OrderID:  ta.OrderId,
		CoinName: ta.CoinName,
		Data: transfer.DomTransferData{
			From:   fromAddr,
			To:     toAddr,
			Amount: toAmount.Shift(coinDecimal).IntPart(),
		},
	}
	return orderReq, nil
}

func (s *DomTransferService) checksum(payload []byte) []byte {
