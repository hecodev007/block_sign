...
type CdsTransferService struct {
	CoinName string
	Lock     *sync.Mutex
...
func NewCdsTransferService() service.TransferService {
	return &CdsTransferService{
		CoinName: "cds",
		Lock:     &sync.Mutex{},
		//nonceCtl: map[string]int{},
		lastFromAddress: "",
	}
}

func (s *CdsTransferService) VaildAddr(address string) error {

	//data, err := util.Get(fmt.Sprintf("%s/eth/validaddress?address=%s", conf.Cfg.Walletserver.Url, address))
	//if err != nil {
	//	return err
	//}
...
func (s *CdsTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *CdsTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *CdsTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *CdsTransferService) walletServerCreateHot(orderReq *transfer.CdsOrderRequest) (string, error) {
...
func (s *CdsTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.CdsOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	orderReq.Amount = toAmount.Shift(int32(CdsDecimal)).String()
	// orderReq.GasLimit = uint64(60000)		// 小币种限制死没多大关系
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *CdsTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
...
*/
func (s *CdsTransferService) getFromAddress(lastFromAddress string, addr []string) string {
