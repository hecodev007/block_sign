...
type RoseTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewRoseTransferService() service.TransferService {
	return &RoseTransferService{
		CoinName: "rose",
		Lock:     &sync.Mutex{},
	}
}

func (s *RoseTransferService) VaildAddr(address string) error {
	if !strings.HasPrefix(address, "oasis1q") {
		return fmt.Errorf("地址校验失败1: %s", address)
	}
	if len(address) != 46 {
		return fmt.Errorf("地址校验失败2: %s", address)
	}
	return nil
}

func (s *RoseTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.RoseOrderRequest
...
func (s *RoseTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *RoseTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *RoseTransferService) walletServerCreateHot(orderReq *transfer.RoseOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeRoseTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *RoseTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.RoseOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(RoseDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *RoseTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
