...
const HecoDecimal = 18

type HecoTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewHecoTransferService() service.TransferService {
	return &HecoTransferService{
		CoinName: "heco",
		Lock:     &sync.Mutex{},
	}
}

func (s *HecoTransferService) VaildAddr(address string) error {
...
func (s *HecoTransferService) has0xPrefix(str string) bool {
	return len(str) >= 2 && str[0] == '0' && (str[1] == 'x' || str[1] == 'X')
}

func (s *HecoTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *HecoTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
func (s *HecoTransferService) walletServerCreateHot(orderReq *transfer.HecoOrderRequest) (string, error) {
	...
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/heco/transfer", cfg.Url), cfg.User, cfg.Password, orderReq)
	if err != nil {
		return "", err
	}
...
//=================私有方法=================
//创建交易接口参数
func (s *HecoTransferService) walletServerCreate(orderReq *transfer.HecoOrderRequest) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "main net fee too high") {
			return errors.New("HECO主网手续费过高，请值班在留意主网手续费并重推")
		}
		return err
	}
	dd, _ := json.Marshal(orderReq)
...
//私有方法 构建heco订单
func (s *HecoTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.HecoOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if len(fromAddrs) == 0 {
		dao.ReportColdAddrBalanceNotEnough(int64(ta.AppId), ta.OutOrderid, "heco", coinType, searchAmount.String())
	...
	if ta.Eostoken != "" { //如果是代币转账
		...
		orderReq.Amount = toAmount.Shift(int32(HecoDecimal)).String()
	}
	return orderReq, nil
}

func (s *HecoTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.HecoOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
	...
	if len(fromAddrs) == 0 {
		dao.ReportColdAddrBalanceNotEnough(int64(ta.AppId), ta.OutOrderid, "heco", coinType, toAddrs[0].ToAmount)
		return nil, fmt.Errorf("outorderNo:%s 没有符合条件的出账地址\n amount: %v \n to: %s \n ", ta.OutOrderid, toAddrs[0].ToAmount, toAddr)
	}
