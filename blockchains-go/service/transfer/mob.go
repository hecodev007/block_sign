...
type MobTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewMobTransferService() service.TransferService {
	return &MobTransferService{
		CoinName: "mob",
		Lock:     &sync.Mutex{},
	}
}

func (s *MobTransferService) VaildAddr(address string) error {
	cfg, _ := conf.Cfg.HotServers[s.CoinName]
	params := make(map[string]interface{})
	params["address"] = address
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/%s/validAddress", cfg.Url, s.CoinName), cfg.User, cfg.Password, params)
	if err != nil {
		return fmt.Errorf("mob VaildAddr  error: %v", err)
	}
	err = transfer.DecodeValidAddressResp(data)
	if err != nil {
		return err
	}
	return nil
...
func (s *MobTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *MobTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *MobTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *MobTransferService) walletServerCreateHot(orderReq *transfer.MobOrderRequest) (string, error) {
...
func (s *MobTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.MobOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(MobDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *MobTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
...
func (s *MobTransferService) getChainBalance(mchName, coinName, address string) (string, error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	params := make(map[string]interface{})
	params["coin_name"] = coinName
	params["mch_name"] = mchName
	params["address"] = address
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/%s/balance", cfg.Url, s.CoinName), cfg.User, cfg.Password, params)
	if err != nil {
