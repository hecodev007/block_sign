...
type IostTransferService struct {
	CoinName string
}

func NewIostTransferService() service.TransferService {
	return &IostTransferService{
		CoinName: "iost",
	}
}

func (s *IostTransferService) VaildAddr(address string) error {
...
func (s *IostTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.IostOrderRequest
...
func (s *IostTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	return errors.New("do not support cold transfer")
}

func (s *IostTransferService) walletServerCreateHot(orderReq *transfer.IostOrderRequest) (string, error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	if !ok {
...
func (s *IostTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinSet *entity.FcCoinSet) (*transfer.IostOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	chainBalance, err := s.getChainBalance(coinType, ta.Eostoken, fromAddr)
	if err != nil {
	...
	json.Unmarshal([]byte(coldAddrs[0].Json), &jsonkey)
	orderReq.FromAddress = jsonkey["key"]
	...
	if orderReq.FromAddress == "" {
		return nil, errors.New("出账账户:" + orderReq.FromAddress + ",没有配置公钥")
	}
	return orderReq, nil
}

func (s *IostTransferService) getChainBalance(coinName, contractAddress, address string) (string, error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	if !ok {
		return "", fmt.Errorf("don't find %s config", s.CoinName)
	}
	if contractAddress == "" {
		contractAddress = "iost"
	}
	params := make(map[string]interface{})
