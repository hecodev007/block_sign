...
type AurTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewAurTransferService() service.TransferService {
	return &AurTransferService{
		CoinName: "aur",
		Lock:     &sync.Mutex{},
	}
}

func (s *AurTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid aur address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *AurTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *AurTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *AurTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *AurTransferService) walletServerCreateHot(orderReq *transfer.AurOrderRequest) (string, error) {
...
func (s *AurTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.AurOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(AurDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *AurTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
