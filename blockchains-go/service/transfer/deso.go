...
type DesoTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewDesoTransferService() service.TransferService {
	return &DesoTransferService{
		CoinName: "deso",
		Lock:     &sync.Mutex{},
	}
}

func (s *DesoTransferService) VaildAddr(address string) error {
	...
	if err != nil {
		return fmt.Errorf("deso VaildAddr  error: %v", err)
	}
...
func (s *DesoTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.DesoOrderRequest
...
func (s *DesoTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *DesoTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *DesoTransferService) walletServerCreateHot(orderReq *transfer.DesoOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeDesoTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *DesoTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.DesoOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(DesoDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *DesoTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
