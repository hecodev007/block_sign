...
type KarTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewKarTransferService() service.TransferService {
	return &KarTransferService{
		CoinName: "kar",
		Lock:     &sync.Mutex{},
	}
}
...
var (
	KarPrefix   = byte(0x08)
	KarSSPrefix = []byte{0x53, 0x53, 0x35, 0x38, 0x50, 0x52, 0x45}
)

func (s *KarTransferService) VaildAddr(address string) error {
	if address == "" {
	...
	var input []byte
	input = append(input, KarSSPrefix...)
...
func (s *KarTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *KarTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *KarTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *KarTransferService) walletServerCreateHot(orderReq *transfer.KarOrderRequest) (string, error) {
...
func (s *KarTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.KarOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
