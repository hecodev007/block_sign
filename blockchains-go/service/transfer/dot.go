...
type DotTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewDotTransferService() service.TransferService {
	return &DotTransferService{
		CoinName: "dot",
		Lock:     &sync.Mutex{},
	}
}
...
func (s *DotTransferService) VaildAddr(address string) error {
	if address == "" {
...
func (s *DotTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *DotTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *DotTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *DotTransferService) walletServerCreateHot(orderReq *transfer.DotOrderRequest) (string, error) {
...
func (s *DotTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.DotOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
