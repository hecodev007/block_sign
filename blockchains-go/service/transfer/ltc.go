...
type LtcTransferService struct {
	CoinName string
}

func NewLtcTransferService() service.TransferService {
	return &LtcTransferService{
		CoinName: "ltc",
	}
}

func (srv *LtcTransferService) VaildAddr(address string) error {
	url := conf.Cfg.CoinServers[srv.CoinName].Url + "/api/v1/ltc/validateaddress?address=%s"
	...
	ltcResp := transfer.DecodeLtcAddressResult(data)
	if ltcResp != nil && ltcResp.Data != nil {
		if ltcResp.Data.Isvalid {
			return nil
		}
	}
...
func (srv *LtcTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	//无需实现
	return "", errors.New("implement me")
}

func (srv *LtcTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//随机选择可用机器
...
//params worker 指定机器
func (srv *LtcTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.LtcOrderRequest, error) {
	var (
	...
	log.Infof("订单=%s getEstimateTpl", ta.OutOrderid)
	...
	if len(changes) == 0 {
		return nil, fmt.Errorf("ltc 商户=[%d],查询ltc找零地址失败", ta.AppId)
	}
	//随机选择
	randIndex := util.RandInt64(0, int64(len(changes)))
	changeAddress = changes[randIndex]
	log.Infof("订单=%s FcGenerateAddressListFindChangeAddr", ta.OutOrderid)
	...
	log.Infof("订单=%s FcTransfersApplyCoinAddressFindAddrInfo", ta.OutOrderid)
	...
	if err != nil {
		log.Errorf("下单表订单id：%d,ltc 交易手续费转换异常:%s", ta.Id, err.Error())
		return nil, err
	}
	//ltc api需要金额整型
	...
	//todo Worker读取数据库
	orderReq := &transfer.LtcOrderRequest{
...
//创建交易接口参数
func (srv *LtcTransferService) walletServerCreate(orderReq *transfer.LtcOrderRequest) error {
	log.Infof("订单=%s walletServerCreate", orderReq.OuterOrderNo)
	...
	log.Infof("订单=%s walletServerCreate 完成", orderReq.OuterOrderNo)
...
//fee 手续费
func (srv *LtcTransferService) setUtxoData(appid int64, orderReq *transfer.LtcOrderRequest, toAddr, changeAddr string) error {
	var (
		...
		coinSet      *entity.FcCoinSet //db币种配置
		unspents     *transfer.LtcUnspents
		err          error
	)
	log.Infof("订单=%s setUtxoData", orderReq.OuterOrderNo)
	...
	utxoTpl := make([]*transfer.LtcOrderAddrRequest, 0) //utxo模板
	...
	log.Infof("订单=%s FcMchAmountGetInfo", orderReq.OuterOrderNo)
	...
	ltcCfg = conf.Cfg.CoinServers[srv.CoinName]
	if ltcCfg == nil {
	...
	log.Infof("订单=%s FcAddressAmountFindTransfer", orderReq.OuterOrderNo)
	...
	log.Infof("订单=%s ltc/unspents", orderReq.OuterOrderNo)
	...
	log.Infof("订单=%s getFee", orderReq.OuterOrderNo)

	feeAmountTmp = decimal.New(feeTmp, 0)

	//排序unspent，先进行降序，找出大额的数值
	var sortUtxo transfer.LtcUnspentDesc

	var sortUtxoTmp transfer.LtcUnspentAsc //临时使用，小金额排序
	...
	log.Infof("订单=%s sortUtxoTmp", orderReq.OuterOrderNo)
	...
	for _, v := range sortUtxo {
		...
		oar := &transfer.LtcOrderAddrRequest{
	...
	log.Infof("订单=%s for sortUtxo", orderReq.OuterOrderNo)
	...
	//组装to
	utxoTpl = append(utxoTpl, &transfer.LtcOrderAddrRequest{
	...
	orderReq.Fee = fee
	log.Infof("订单=%s setUtxoData 完成", orderReq.OuterOrderNo)
	return nil

}

//手续费计算
func (srv *LtcTransferService) getFee(inNum, outNum int) (int64, error) {
