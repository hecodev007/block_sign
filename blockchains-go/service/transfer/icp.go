...
type IcpTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewIcpTransferService() service.TransferService {
	return &IcpTransferService{
		CoinName: "icp",
		Lock:     &sync.Mutex{},
	}
}

func (s *IcpTransferService) VaildAddr(address string) error {
	if len(address) != 64 {
		return errors.New("非法地址")
	}
	return nil
}

func (s *IcpTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *IcpTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
func (s *IcpTransferService) walletServerCreateHot(orderReq *transfer.IcpOrderRequest) (string, error) {
...
func (s *IcpTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.IcpOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(IcpDecimal)).IntPart()
	}
	return orderReq, nil
}
