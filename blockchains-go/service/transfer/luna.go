...
type LunaTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewLunaTransferService() service.TransferService {
	return &LunaTransferService{CoinName: "luna",
		Lock: &sync.Mutex{}}
}

func (s *LunaTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *LunaTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	return fmt.Errorf("don't implement for luna cold ")
}
func (s *LunaTransferService) VaildAddr(address string) error {
...
//创建交易接口参数
func (s *LunaTransferService) walletServerCreate(orderReq *transfer.LunaOrderRequest) (txid string, err error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	if !ok {
		return "", fmt.Errorf("don't find %s config", s.CoinName)
	}
...
//======================私有方法==================
//私有方法 构建luna订单
func (s *LunaTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.LunaOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if coinSet == nil {
		return nil, fmt.Errorf("缺少luna币种信息")
	}
	...
	//填充参数
	orderReq := &transfer.LunaOrderRequest{}
	...
	orderReq.Data = transfer.LunaPaymentRequest{
		FromAddress: fromAddr,
		ToAddress:   toAddr,
		Amount:      toAmount.Shift(LunaDecimal).IntPart(), //转换成
		Memo:        ta.Memo,
		Token:       ta.Eostoken,
	}
	return orderReq, nil
