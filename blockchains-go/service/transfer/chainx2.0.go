...
type ChainXTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewChainXTransferService() service.TransferService {
	return &ChainXTransferService{
		CoinName: "pcx",
		Lock:     &sync.Mutex{},
	}
}

func (s *ChainXTransferService) VaildAddr(address string) error {
	return ss58.VerityAddress(address, ss58.ChainXPrefix)
}

func (s *ChainXTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *ChainXTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *ChainXTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *ChainXTransferService) walletServerCreateHot(orderReq *transfer.ChainXOrderRequest) (string, error) {
...
func (s *ChainXTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.ChainXOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	//
	fromAmount := toAmount.Add(decimal.NewFromFloat(0.004))
