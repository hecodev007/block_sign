...
type LatTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewLatTransferService() service.TransferService {
	return &LatTransferService{
		CoinName: "lat",
		Lock:     &sync.Mutex{},
	}
}

func (s *LatTransferService) VaildAddr(address string) error {
	if len(address) != 42 || !strings.HasPrefix(address, "lat1") {
		return errors.New("valid lat address error")
	}
	return nil
}

func (s *LatTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.LatOrderRequest
...
func (s *LatTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *LatTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *LatTransferService) walletServerCreateHot(orderReq *transfer.LatOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeLatTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *LatTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.LatOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(LatDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *LatTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
