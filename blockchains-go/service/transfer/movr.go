...
const MovrDecimal = 18

type MovrTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewMovrTransferService() service.TransferService {
	return &MovrTransferService{
		CoinName: "movr",
		Lock:     &sync.Mutex{},
	}
}

func (s *MovrTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid movr address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *MovrTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.MovrOrderRequest
...
func (s *MovrTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *MovrTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *MovrTransferService) walletServerCreateHot(orderReq *transfer.MovrOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeMovrTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *MovrTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.MovrOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(MovrDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *MovrTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
