...
type FisTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewFisTransferService() service.TransferService {
	return &FisTransferService{
		CoinName: "fis",
		Lock:     &sync.Mutex{},
	}
}
...
func (s *FisTransferService) VaildAddr(address string) error {
	return ss58.VerityAddress(address, ss58.StafiPrefix)
}

func (s *FisTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.FisOrderRequest
...
func (s *FisTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *FisTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *FisTransferService) walletServerCreateHot(orderReq *transfer.FisOrderRequest) (string, error) {
...
func (s *FisTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.FisOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
