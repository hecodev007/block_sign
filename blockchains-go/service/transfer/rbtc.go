...
const RbtcDecimal = 18

type RbtcTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewRbtcTransferService() service.TransferService {
	return &RbtcTransferService{
		CoinName: "rbtc",
		Lock:     &sync.Mutex{},
	}
}

func (s *RbtcTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid rbtc address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *RbtcTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *RbtcTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *RbtcTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *RbtcTransferService) walletServerCreateHot(orderReq *transfer.RbtcOrderRequest) (string, error) {
...
func (s *RbtcTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.RbtcOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(RbtcDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *RbtcTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
