...
import (
	...
	"fmt"
	_ "fmt"
	"github.com/group-coldwallet/blockchains-go/pkg/util/icap"
	"math/rand"
	"strings"
	"sync"
	"time"
...
type IotexTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewIotexTransferService() service.TransferService {
	return &IotexTransferService{
		CoinName: "iotx",
		Lock:     &sync.Mutex{},
	}
}
func checkIotxAddress(address string) bool {
	if strings.ToLower(address[:2]) == "io" {
		return true
	}
	return false
}

func tryBench32ToAddress(address string) (addr string, err error) {
	if !checkIotxAddress(address) {
		return address, nil
	}
	toAddress, err := icap.Bech32ToAddressHrp(address, Bech32AddressHRP)
	if err == nil {
...
func (s *IotexTransferService) VaildAddr(address string) error {
	addr, err := tryBench32ToAddress(address)
	if err != nil {
		return err
	}
	if !common.IsHexAddress(addr) {
		return errors.New("valid iotx address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *IotexTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.IotexOrderRequest
...
func (s *IotexTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *IotexTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *IotexTransferService) walletServerCreateHot(orderReq *transfer.IotexOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeIotexTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *IotexTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.IotexOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	orderReq.FromAddress, _ = tryBench32ToAddress(fromAddr)
	orderReq.ToAddress, _ = tryBench32ToAddress(toAddr)
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(IotexDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *IotexTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
