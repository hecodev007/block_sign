...
type ZenTransferService struct {
	CoinName  string
	minAmount int64
}

func NewZenTransferService() service.TransferService {
	return &ZenTransferService{
		CoinName:  "zen",
		minAmount: 546,
	}
}

func (srv *ZenTransferService) VaildAddr(address string) error {
	url := conf.Cfg.CoinServers[srv.CoinName].Url + "/api/v1/zen/validateaddress?address=%s"
	...
	ZenResp := transfer.DecodeZenAddressResult(data)
	if ZenResp != nil && ZenResp.Data != nil {
		if ZenResp.Data.Isvalid {
			return nil
		}
	}
...
func (srv *ZenTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
...
func (srv *ZenTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//无需实现
	return errors.New("implement me")
}

//params worker 指定机器
func (srv *ZenTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.ZenTxTpl, error) {
	var (
	...
	if len(changes) == 0 {
		return nil, fmt.Errorf("Zen 商户=[%d],查询Zen找零地址失败", ta.AppId)
	}
	//随机选择
	...
	if err != nil {
		log.Errorf("下单表订单id：%d,Zen 交易手续费转换异常:%s", ta.Id, err.Error())
		return nil, err
	}
	//Zen api需要金额整型
	...
	//todo Worker读取数据库
	orderReq := &transfer.ZenOrderRequest{
...
//创建交易接口参数
func (srv *ZenTransferService) walletServerCreate(orderReq *transfer.ZenTxTpl) (string, error) {

	log.Infof("zen 发送url：%s", conf.Cfg.HotServers[srv.CoinName].Url+"/v1/zen/transfer")
	log.Infof("zen 发送结构：%+v", orderReq)
	data, err := util.PostJsonByAuth(conf.Cfg.HotServers[srv.CoinName].Url+"/v1/zen/transfer", conf.Cfg.HotServers[srv.CoinName].User, conf.Cfg.HotServers[srv.CoinName].Password, orderReq)
	if err != nil {
		return "", err
	}
	log.Infof("zen 发送返回：%s", string(data))
...
//fee 手续费
func (srv *ZenTransferService) setUtxoData(appid int64, orderReq *transfer.ZenOrderRequest, toAddr, changeAddr string) (*transfer.ZenTxTpl, error) {
	var (
		...
		ZenCfg          *conf.CoinServers //币种数据服务
		coinSet         *entity.FcCoinSet //db币种配置
		//unspents     *transfer.BtcUnspents
		err      error
		txInTpl  = make([]transfer.ZenTxInTpl, 0)
		txOutTpl = make([]transfer.ZenTxOutTpl, 0)
		tpl      *transfer.ZenTxTpl
		unspents *transfer.BtcUnspents
	)
	...
	ZenCfg = conf.Cfg.CoinServers[srv.CoinName]
	if ZenCfg == nil {
		return nil, errors.New("配置文件缺少Zen coinservers设置")
	}
	...
	//查询utxo数量
	byteData, err := util.PostJson(ZenCfg.Url+"/api/v1/zen/unspents", addrs)
	if err != nil {
	...
	if unspents.Code != 0 {
		return nil, fmt.Errorf("获取utxo异常，err:%s", unspents.Message)
	}
	if len(unspents.Data) == 0 {
		return nil, errors.New("zen empty unspents")
	}
	...
	if fee != 0 {
		if fee < 50000 || fee > 10000000 {
			//使用指定手续费
			return nil, errors.New("指定的手续费不在合理范围值[[0.00005-0.1]")
		}
		feeTmp = fee
	} else {
		//先提前预估手续费
	...
	//组装to
	txOutTpl = append(txOutTpl, transfer.ZenTxOutTpl{
		ToAddr:   toAddr,
...
//手续费计算
func (srv *ZenTransferService) getFee(inNum, outNum int) (int64, error) {
