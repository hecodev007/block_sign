...
const BriseDecimal = 18

type BriseTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewBriseTransferService() service.TransferService {
	return &BriseTransferService{
		CoinName: "brise-brise",
		Lock:     &sync.Mutex{},
	}
}

func (s *BriseTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid brise-brise address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *BriseTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *BriseTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *BriseTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *BriseTransferService) walletServerCreateHot(orderReq *transfer.BriseOrderRequest) (string, error) {
...
func (s *BriseTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.BriseOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(BriseDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *BriseTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
