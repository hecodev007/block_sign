...
type MwTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewMwTransferService() service.TransferService {
	return &MwTransferService{
		CoinName: "mw",
		Lock:     &sync.Mutex{},
	}
}

func (s *MwTransferService) VaildAddr(address string) error {
	if !strings.HasPrefix(address, "CDW-") {
		return fmt.Errorf("address do not have prefix `CDW-`: %s", address)
	}
	if len(address) != 24 {
		return fmt.Errorf("address length is not equal 24: len=%d", len(address))
	}
	for i, a := range address {
		if i == 3 || i == 8 || i == 13 || i == 18 {
			if string(a) != "-" {
				return fmt.Errorf("address '-' is not error: %s", address)
			}
			continue
		}
		if a > 57 || a < 50 { //2-9
			if a < 65 || a > 90 { //A-Z
				return fmt.Errorf("address is contain unsupport char=%s", string(a))
			} else {
				if a == 73 || a == 79 { //去除O I
					return fmt.Errorf("address is contain unsupport char=%s", string(a))
				}
...
func (s *MwTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.MwOrderRequest
...
func (s *MwTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *MwTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *MwTransferService) walletServerCreateHot(orderReq *transfer.MwOrderRequest) (string, error) {
...
func (s *MwTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.MwOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
