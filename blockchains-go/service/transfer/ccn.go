...
type CcnTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewCcnTransferService() service.TransferService {
	return &CcnTransferService{
		CoinName: "ccn",
		Lock:     &sync.Mutex{},
	}
}

func (s *CcnTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid ccn address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *CcnTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *CcnTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *CcnTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *CcnTransferService) walletServerCreateHot(orderReq *transfer.CcnOrderRequest) (string, error) {
...
func (s *CcnTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.CcnOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(CcnDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *CcnTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
