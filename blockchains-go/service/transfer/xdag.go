...
type XdagTransferService struct {
	CoinName string
}

func NewXdagTransferService() service.TransferService {
	return &XdagTransferService{
		CoinName: "xdag",
	}
}

func (s *XdagTransferService) VaildAddr(address string) error {
...
func (s *XdagTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.XdagOrderRequest
...
func (s *XdagTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	return errors.New("do not support cold transfer")
}

func (s *XdagTransferService) walletServerCreateHot(orderReq *transfer.XdagOrderRequest) (string, error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	if !ok {
...
func (s *XdagTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinSet *entity.FcCoinSet) (*transfer.XdagOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
...
func (s *XdagTransferService) getChainBalance(coinName, address string) (string, error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	if !ok {
		return "", fmt.Errorf("don't find %s config", s.CoinName)
	}
