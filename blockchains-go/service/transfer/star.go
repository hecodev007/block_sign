...
type StarTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewStarTransferService() service.TransferService {
	return &StarTransferService{
		CoinName: "star",
		Lock:     &sync.Mutex{},
	}
}

func (s *StarTransferService) VaildAddr(address string) error {
	if len(address) == 0 {
...
func (s *StarTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.StarOrderRequest
...
func (s *StarTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *StarTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *StarTransferService) walletServerCreateHot(orderReq *transfer.StarOrderRequest) (string, error) {
...
func (s *StarTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.StarOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
