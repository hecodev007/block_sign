...
type EtcTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewEtcTransferService() service.TransferService {
	return &EtcTransferService{
		CoinName: "etc",
		Lock:     &sync.Mutex{},
	}
}

func (s *EtcTransferService) VaildAddr(address string) error {
...
func (s *EtcTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	panic("implement me")
}

func (s *EtcTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *EtcTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
//私有方法 构建eth订单
func (s *EtcTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
