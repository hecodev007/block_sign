...
type UcaTransferService struct {
	CoinName string
}

func NewUcaTransferService() service.TransferService {
	return &UcaTransferService{
		CoinName: "uca",
	}
}

func (srv *UcaTransferService) VaildAddr(address string) error {
	url := conf.Cfg.CoinServers[srv.CoinName].Url + "/api/v1/uca/validateaddress?address=%s"
	...
	UcaResp := transfer.DecodeUcaAddressResult(data)
	if UcaResp != nil && UcaResp.Data != nil {
		if UcaResp.Data.Isvalid {
			return nil
		}
	}
...
func (srv *UcaTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
...
func (srv *UcaTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//无需实现
	return errors.New("implement me")
}

//params worker 指定机器
func (srv *UcaTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.UcaTxTpl, error) {
	var (
	...
	if len(changes) == 0 {
		return nil, fmt.Errorf("Uca 商户=[%d],查询Uca找零地址失败", ta.AppId)
	}
	//随机选择
	...
	if err != nil {
		log.Errorf("下单表订单id：%d,Uca 交易手续费转换异常:%s", ta.Id, err.Error())
		return nil, err
	}
	//Uca api需要金额整型
	...
	//todo Worker读取数据库
	orderReq := &transfer.UcaOrderRequest{
...
//创建交易接口参数
func (srv *UcaTransferService) walletServerCreate(orderReq *transfer.UcaTxTpl) (string, error) {

	log.Infof("uca 发送url：%s", conf.Cfg.HotServers[srv.CoinName].Url+"/v1/transfer")
	log.Infof("uca 发送结构：%+v", orderReq)
	...
	log.Infof("uca 发送返回：%s", string(data))
...
//fee 手续费
func (srv *UcaTransferService) setUtxoData(appid int64, orderReq *transfer.UcaOrderRequest, toAddr, changeAddr string) (*transfer.UcaTxTpl, error) {
	var (
		...
		UcaCfg       *conf.CoinServers //币种数据服务
		coinSet      *entity.FcCoinSet //db币种配置
		//unspents     *transfer.UcaUnspents
		err      error
		txInTpl  = make([]transfer.UcaTxInTpl, 0)
		txOutTpl = make([]transfer.UcaTxOutTpl, 0)
		tpl      *transfer.UcaTxTpl
	)
	...
	UcaCfg = conf.Cfg.CoinServers[srv.CoinName]
	if UcaCfg == nil {
		return nil, errors.New("配置文件缺少Uca coinservers设置")
	}
	...
	//由于uca币种无法使用unspent接口，只能从数据库维护
	tpush, err := dao.FcTransPushFindVaildUtxo(int(appid), srv.CoinName)
	if err != nil {
		return nil, fmt.Errorf("find uca utxo error:%s", err.Error())
	}
	if len(tpush) == 0 {
		return nil, errors.New("uca:No utxo available ")
	}

	////出账的话优先使用前面50个大金额地址
	//addrInfos, err := dao.FcAddressAmountFindTransferToUca(appid, 50, "desc")
	//if err != nil {
	//	return err
	//}
	//if len(addrInfos) == 0 {
	...
	//}
	////查询utxo数量
	//byteData, err := util.PostJson(UcaCfg.Url+"/api/v1/uca/unspents", addrs)
	//if err != nil {
	//	return fmt.Errorf("获取utxo异常，err:%s", err.Error())
	//}
	//unspents = new(transfer.UcaUnspents)
	...
	//	return errors.New("Uca empty unspents")
	//}
	//
	...
	if fee != 0 {
		if fee < 1000 || fee > 10000000 {
			//使用指定手续费
			return nil, errors.New("指定的手续费不在合理范围值[[0.00000546-0.1]")
		}
		feeTmp = fee
	} else {
		//先提前预估手续费
	...
	var sortUtxoTmp entity.DBUnspentAsc //临时使用，小金额排序
	...
	for _, uv := range sortUtxoTmp {
		...
		rediskeyName := fmt.Sprintf("%s_%s_%d", rediskey.UCA_UTXO_LOCK, uv.TransactionId, uv.TrxN)
		if has, _ := redisHelper.Exists(rediskeyName); has {
			//已经占用utxo，跳过
			continue
		}
		amFloat, _ := decimal.NewFromString(uv.Amount)
		if amFloat.Shift(8).GreaterThanOrEqual(toAmount.Add(feeAmountTmp)) {
	...
	for _, v := range sortUtxo {
		rediskeyName := fmt.Sprintf("%s_%s_%d", rediskey.UCA_UTXO_LOCK, v.TransactionId, v.TrxN)
		if has, _ := redisHelper.Exists(rediskeyName); has {
			//已经占用utxo，跳过
			continue
		}
		...
		oar := transfer.UcaTxInTpl{
			FromAddr:   v.Address,
			FromTxid:   v.TransactionId,
			FromIndex:  uint32(v.TrxN),
			FromAmount: famount.Shift(8).IntPart(),
		}
		fromAmount = fromAmount.Add(famount.Shift(8))
	...
	//组装to
	txOutTpl = append(txOutTpl, transfer.UcaTxOutTpl{
		ToAddr:   toAddr,
...
//手续费计算
func (srv *UcaTransferService) getFee(inNum, outNum int) (int64, error) {
