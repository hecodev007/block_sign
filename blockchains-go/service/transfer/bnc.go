...
type BncTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewBncTransferService() service.TransferService {
	return &BncTransferService{
		CoinName: "bnc",
		Lock:     &sync.Mutex{},
	}
}
...
var (
	BncPrefix   = byte(0x06)
	bncSSPrefix = []byte{0x53, 0x53, 0x35, 0x38, 0x50, 0x52, 0x45}
)

func (s *BncTransferService) VaildAddr(address string) error {
	if address == "" {
	...
	var input []byte
	input = append(input, bncSSPrefix...)
...
func (s *BncTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.BncOrderRequest
...
func (s *BncTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *BncTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *BncTransferService) walletServerCreateHot(orderReq *transfer.BncOrderRequest) (string, error) {
...
func (s *BncTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.BncOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
