...
const UencDecimal = 6
const UencFromAddr = "uenc_from_addr"

type UencTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewUencTransferService() service.TransferService {
	return &UencTransferService{
		CoinName: "uenc",
		Lock:     &sync.Mutex{},
	}
}

func (s *UencTransferService) VaildAddr(address string) error {
	if !strings.HasPrefix(address, "1") {
		return errors.New("VaildAddr Error")
	}
	if len(address) < 26 || len(address) > 34 {
		return errors.New("VaildAddr Error")
	}
	return nil
}

func (s *UencTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *UencTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *UencTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *UencTransferService) walletServerCreateHot(orderReq *transfer.UencOrderRequest) (string, error) {
...
func (s *UencTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.UencOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
...
RETRY:
	addrLen, err := redis.Client.ListLen(UencFromAddr)
	if addrLen == 0 || err != nil {
		...
		for _, addr := range fromAddrs {
			redis.Client.ListRPush(UencFromAddr, addr)
		}
	}
	popAddr, err := redis.Client.ListPop(UencFromAddr)
	if err != nil {
		return nil, fmt.Errorf("err:%s", err.Error())
	}
	fromAddr = string(popAddr)
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(UencDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *UencTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
