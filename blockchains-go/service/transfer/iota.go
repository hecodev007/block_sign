...
type IotaTransferService struct {
	CoinName string
}

func NewIotaTransferService() service.TransferService {
	return &IotaTransferService{
		CoinName: "iota",
	}
}
...
func (s *IotaTransferService) VaildAddr(address string) error {
	if address == "" {
		return errors.New("address is null")
	}
	prefix, _, err := iotago.ParseBech32(address)
	if iotago.PrefixMainnet != prefix {
		return errors.New("invalid address")
	}

	return err
}
func (srv *IotaTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
...
func (srv *IotaTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//无需实现
	return errors.New("implement me")
}

//params worker 指定机器
func (srv *IotaTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.EacTxTpl, error) {
	var (
	...
	toAddr = toAddrs[0].Address
	toAddrAmount, _ := decimal.NewFromString(toAddrs[0].ToAmount)
	if toAddrAmount.LessThan(decimal.NewFromFloat(0.000001)) {
		return nil, fmt.Errorf("内部订单ID：%d，外部订单号：%s,接收地址金额异常,最小金额0.00000001", ta.Id, ta.OutOrderid)
	}
	fmt.Println("iotablock:", toAddr, toAddrAmount)
	//Eac api需要金额整型
	toAmount = toAddrAmount.Shift(6).IntPart()
	...
	//Eac api需要金额整型
	fee = feeDecimal.Shift(6).IntPart()
...
//创建交易接口参数
func (srv *IotaTransferService) walletServerCreate(orderReq *transfer.EacTxTpl) (string, error) {
...
//fee 手续费
func (srv *IotaTransferService) setUtxoData(appid int64, orderReq *transfer.EacOrderRequest, toAddr, changeAddr string) (*transfer.EacTxTpl, error) {
	var (
		...
		IotaCfg         *conf.CoinServers //币种数据服务
		coinSet         *entity.FcCoinSet //db币种配置
	...
	if orderReq.Amount < 1000000 {
		return nil, fmt.Errorf("发送金额不能小于1,目前金额：%s", decimal.New(orderReq.Amount, -6).String())
	}
	fee = orderReq.Fee

	fmt.Println("orderReq.Amount:", orderReq.Amount)
	...
	if mchAmount.Shift(6).LessThanOrEqual(toAmountInt64) {
		return nil, fmt.Errorf("商户:%d,币种：%s，余额不足,存量（包含发送中冻结）：%s，需要发送：%s,手续费未计算",
			appid,
			srv.CoinName,
			mchAmount.String(),
			toAmountInt64.Shift(-6).String(),
		)
	}
	IotaCfg = conf.Cfg.CoinServers[srv.CoinName]
	if IotaCfg == nil {
	...
	//查询utxo数量
	log.Infof("发送地址：%s", IotaCfg.Url+"/v1/iota/unspents")
	byteData, err := util.PostJson(IotaCfg.Url+"/v1/iota/unspents", addrs)
	if err != nil {
	...
	for _, uv := range sortUtxoTmp {
		...
		if amInt64.GreaterThanOrEqual(toAmountInt64.Add(decimal.New(1, 6))) {
			log.Infof("订单：%s，查询到最符合出账utxo金额：%s,address:%s,刷新utxo列表", orderReq.OuterOrderNo, amInt64.Shift(-6).String(), uv.Address)
	...
	for _, v := range sortUtxo {
		...
		if fromAmountInt64.GreaterThanOrEqual(toAmountInt64.Add(decimal.New(1, 6))) {
			//满足出账
			break
		}
	...
	if fromAmountInt64.LessThan(toAmountInt64) {
		return nil, fmt.Errorf("使用的utxo数量金额不足出账金额，请等待归集或者入账，商户余额(包含冻结)：%s，限量utxo使用金额：%s,出账金额：%s，预估手续费：%s",
			mchAmount.String(),
			fromAmountInt64.Shift(-6).String(),
			toAmountInt64.Shift(-6).String(),
			feeAmountTmp.Shift(-6).String(),
		)
	}
	...
	if changeAmount.LessThan(decimal.Zero) {
		return nil, fmt.Errorf("找零金额异常使用金额：%s,出账金额：%s，手续费：%s，找零：%s",
			fromAmountInt64.Shift(-6).String(),
			toAmountInt64.Shift(-6).String(),
			feeAmount.Shift(-6).String(),
			changeAmount.Shift(-6).String(),
		)
	}
...
//手续费计算
func (srv *IotaTransferService) getFee(inNum, outNum int) (int64, error) {
