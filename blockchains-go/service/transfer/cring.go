...
type CringTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewCringTransferService() service.TransferService {
	return &CringTransferService{
		CoinName: "crab",
		Lock:     &sync.Mutex{},
	}
}
...
var (
	CringPrefix   = byte(0x2a)
	cringSSPrefix = []byte{0x53, 0x53, 0x35, 0x38, 0x50, 0x52, 0x45}
)

func (s *CringTransferService) VaildAddr(address string) error {
	if address == "" {
	...
	if data[0] != CringPrefix {
		return fmt.Errorf("invalid version,version=%v", data[0])
	}

	var input []byte
	input = append(input, cringSSPrefix...)
...
func (s *CringTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *CringTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *CringTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *CringTransferService) walletServerCreateHot(orderReq *transfer.CringOrderRequest) (string, error) {
...
func (s *CringTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.CringOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
