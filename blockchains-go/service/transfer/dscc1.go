...
const Dscc1Decimal = 9

type Dscc1TransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewDscc1TransferService() service.TransferService {
	return &Dscc1TransferService{
		CoinName: "dscc1",
		Lock:     &sync.Mutex{},
	}
}

func (s *Dscc1TransferService) VaildAddr(address string) error {

	//if !common.IsHexAddress(address) {
	//	return errors.New("valid dscc1 address error")
	//}
	if len(address) != 66 {
		return errors.New("valid Dscc1 address error")
	}
...
func (s *Dscc1TransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *Dscc1TransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *Dscc1TransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *Dscc1TransferService) walletServerCreateHot(orderReq *transfer.Dscc1OrderRequest) (string, error) {
...
func (s *Dscc1TransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.Dscc1OrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(Dscc1Decimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *Dscc1TransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
