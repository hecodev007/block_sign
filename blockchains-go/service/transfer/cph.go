...
type CphTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewCphTransferService() service.TransferService {
	return &CphTransferService{
		CoinName: "cph-cph",
		Lock:     &sync.Mutex{},
	}
}

func (s *CphTransferService) VaildAddr(address string) error {
	if !strings.HasPrefix(strings.ToLower(address), "cph") {
		return fmt.Errorf("%s not had prefix `cph`", address)
	}
	if len(address) != 43 {
		return fmt.Errorf("%s 地址长度应为43", address)
	}
	return nil
}

func (s *CphTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.CphOrderRequest
...
func (s *CphTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *CphTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *CphTransferService) walletServerCreateHot(orderReq *transfer.CphOrderRequest) (string, error) {
...
func (s *CphTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.CphOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
