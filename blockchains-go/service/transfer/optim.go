...
const OptimDecimal = 18

type OptimTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewOptimTransferService() service.TransferService {
	return &OptimTransferService{
		CoinName: "optim",
		Lock:     &sync.Mutex{},
	}
}

func (s *OptimTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid optim address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *OptimTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *OptimTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *OptimTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *OptimTransferService) walletServerCreateHot(orderReq *transfer.OptimOrderRequest) (string, error) {
...
func (s *OptimTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.OptimOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(OptimDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *OptimTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
