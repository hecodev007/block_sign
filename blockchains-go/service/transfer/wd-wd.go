...
type Wd_wdTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewWd_wdTransferService() service.TransferService {
	return &Wd_wdTransferService{
		CoinName: "wd",
		Lock:     &sync.Mutex{},
	}
}

func (s *Wd_wdTransferService) VaildAddr(address string) error {
	if len(address) == 0 {
...
func (s *Wd_wdTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.Wd_wdOrderRequest
...
func (s *Wd_wdTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *Wd_wdTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *Wd_wdTransferService) walletServerCreateHot(orderReq *transfer.Wd_wdOrderRequest) (string, error) {
...
func (s *Wd_wdTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.Wd_wdOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
