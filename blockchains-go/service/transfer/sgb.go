...
type SgbTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewSgbTransferService() service.TransferService {
	return &SgbTransferService{
		CoinName: "sgb-sgb",
		Lock:     &sync.Mutex{},
	}
}
...
var (
	SgbPrefix   = byte(0x1b)
	SgbSSPrefix = []byte{0x53, 0x53, 0x35, 0x38, 0x50, 0x52, 0x45}
)

func (s *SgbTransferService) VaildAddr(address string) error {
	if address == "" {
	...
	var input []byte
	input = append(input, SgbSSPrefix...)
...
func (s *SgbTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *SgbTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *SgbTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *SgbTransferService) walletServerCreateHot(orderReq *transfer.SgbOrderRequest) (string, error) {
...
func (s *SgbTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.SgbOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	//
	fromAmount := toAmount.Add(decimal.NewFromFloat(0))
