...
type NearTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewNearTransferService() service.TransferService {
	return &NearTransferService{
		CoinName: "near",
		Lock:     &sync.Mutex{},
	}
}

func (s *NearTransferService) VaildAddr(address string) error {
	if len(address) != 64 {
		return fmt.Errorf("地址不正确，addr=%s", address)
	}
...
func (s *NearTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.NearOrderRequest
...
func (s *NearTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *NearTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *NearTransferService) walletServerCreateHot(orderReq *transfer.NearOrderRequest) (string, error) {
...
func (s *NearTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.NearOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	//
	//加上手续费
	am := toAmount.Add(decimal.NewFromFloat(0.02))

	fromAddrs, err := entity.FcAddressAmount{}.FindAddress(builder.Expr("coin_type = ? and amount > ? and forzen_amount = 0", coinType, am.String()).
