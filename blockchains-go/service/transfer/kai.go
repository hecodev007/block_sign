...
type KaiTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewKaiTransferService() service.TransferService {
	return &KaiTransferService{
		CoinName: "kai",
		Lock:     &sync.Mutex{},
	}
}

func (s *KaiTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid kai address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *KaiTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *KaiTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *KaiTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *KaiTransferService) walletServerCreateHot(orderReq *transfer.KaiOrderRequest) (string, error) {
...
func (s *KaiTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.KaiOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(KaiDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *KaiTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
