...
type PcxTransferService struct {
	CoinName string
}

var PcxPrefix = byte(0x2c)

func NewPcxTransferService() service.TransferService {
	return &PcxTransferService{
		CoinName: "pcx",
	}
}

func (s *PcxTransferService) VaildAddr(address string) error {
	if address == "" {
...
func (s *PcxTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.PcxOrderRequest
	...
	var fee int64
	txid, fee, err = s.walletServerCreateHot(orderReq)
	if err != nil {
...
func (s *PcxTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	return errors.New("do not support cold transfer")
}

//=================私有方法=================
//创建交易接口参数
func (s *PcxTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *PcxTransferService) walletServerCreateHot(orderReq *transfer.PcxOrderRequest) (string, int64, error) {
	cfg, ok := conf.Cfg.HotServers[s.CoinName]
	if !ok {
		return "", 0, fmt.Errorf("don't find %s config", s.CoinName)
	}
	...
	if err1 != nil {
		return "", 0, fmt.Errorf("order表 请求下单接口失败，outOrderId：%s,err: %v", orderReq.OuterOrderNo, err1)
	}
	if thr.Code != 0 || thr.Data == nil {
		return "", 0, fmt.Errorf("order表 请求下单接口返回值失败,服务器返回异常，outOrderId：%s，err:%s", orderReq.OuterOrderNo, string(data))
	}
	result := thr.Data.(map[string]interface{})
	txid := result["txid"].(string)
	fee := int64(result["txFee"].(float64))
	return txid, fee, nil
}
func (s *PcxTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.PcxOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
