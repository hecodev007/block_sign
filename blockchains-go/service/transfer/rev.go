...
type RevTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewRevTransferService() service.TransferService {
	return &RevTransferService{
		CoinName: "rev",
		Lock:     &sync.Mutex{},
	}
}

func (s *RevTransferService) VaildAddr(address string) error {
	//if !common.IsHexAddress(address) {
	//	return errors.New("valid rev address error")
	//}
	if address[:4] != "1111" {
		return errors.New("error address code1")
	}
	if len(base58.Decode(address)) != 40 {
		return errors.New("error address code2")
	}
	return nil
}

func (s *RevTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.RevOrderRequest
...
func (s *RevTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *RevTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *RevTransferService) walletServerCreateHot(orderReq *transfer.RevOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeRevTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *RevTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.RevOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(RevDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *RevTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
