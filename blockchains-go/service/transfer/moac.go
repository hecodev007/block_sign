...
const MoacDecimal = 18

type MoacTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewMoacTransferService() service.TransferService {
	return &MoacTransferService{
		CoinName: "moac",
		Lock:     &sync.Mutex{},
	}
}

func (s *MoacTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid moac address error")
	}
	return nil
}

func (s *MoacTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *MoacTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
// =================私有方法=================
// //创建交易接口参数
// func (s *MoacTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *MoacTransferService) walletServerCreateHot(orderReq *transfer.MoacOrderRequest) (string, error) {
...
func (s *MoacTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.MoacOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Value = toAmount.Shift(int32(coin.Decimal)).String()
	} else {
		orderReq.Value = toAmount.Shift(int32(MoacDecimal)).String()
	}
	return orderReq, nil
}

// 私有方法 构建eth订单
func (s *MoacTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
	...
	if ta.Eostoken != "" { // 如果是代币转账
		...
		orderReq.Amount = toAmount.Shift(int32(MoacDecimal)).String()
	}
	return orderReq, nil
}
