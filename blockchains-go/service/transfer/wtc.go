...
type WtcTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewWtcTransferService() service.TransferService {
	return &WtcTransferService{
		CoinName: "wtc",
		Lock:     &sync.Mutex{},
	}
}

func (s *WtcTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid wtc address error")
	}
	return nil
}

func (s *WtcTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *WtcTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
// =================私有方法=================
// //创建交易接口参数
// func (s *WtcTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *WtcTransferService) walletServerCreateHot(orderReq *transfer.WtcOrderRequest) (string, error) {
...
func (s *WtcTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.WtcOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Value = toAmount.Shift(int32(WtcDecimal)).String()
	}
	return orderReq, nil
}

// 私有方法 构建eth订单
func (s *WtcTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
	...
	if ta.Eostoken != "" { // 如果是代币转账
		...
		orderReq.Amount = toAmount.Shift(int32(WtcDecimal)).String()
	}
	return orderReq, nil
}
