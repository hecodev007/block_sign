...
const redisKeyPrefix = "cspr_tx_toaddr_"

type CsprTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewCsprTransferService() service.TransferService {
	return &CsprTransferService{
		CoinName: "cspr",
		Lock:     &sync.Mutex{},
	}
}

func (s *CsprTransferService) VaildAddr(address string) error {
	if strings.HasPrefix(address, "01") {
		if len(address) != 66 {
			return errors.New("01 prefix address invalid len")
		}
	} else if strings.HasPrefix(address, "02") {
		if len(address) != 68 {
			return errors.New("02 prefix invalid address len")
		}
	}
	//todo 验证服务器，验证雨
	return nil
...
func (s *CsprTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.CsprOrderRequest
...
func (s *CsprTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *CsprTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *CsprTransferService) walletServerCreateHot(orderReq *transfer.CsprOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeCsprTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
	...
	if redis.Client != nil {
		key := redisKeyPrefix + strings.ToLower(txId)
		redis.Client.SetUseRawKey(key, orderReq.ToAddress, time.Hour*24*14)
		log.Infof("CSPR交易key=%s，将to地址 %s 设置到缓存完成", key, orderReq.ToAddress)
	} else {
		log.Info("没有启用redis集群")
	}

	return txId, nil
}
func (s *CsprTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.CsprOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(CsprDecimal)).IntPart()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *CsprTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
	...
	if ta.Eostoken != "" { //如果是代币转账
		...
		orderReq.Amount = toAmount.Shift(int32(CsprDecimal)).String()
	}
	return orderReq, nil
}
