...
type DashTransferService struct {
	CoinName string
}

func NewDashTransferService() service.TransferService {
	return &DashTransferService{
		CoinName: "dash",
	}
}

var NetParams = new(chaincfg.Params)
...
func (srv *DashTransferService) VaildAddr(address string) error {
	if strings.HasPrefix(address, "X") || strings.HasPrefix(address, "7") {
...
func (srv *DashTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
...
func (srv *DashTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//无需实现
	return errors.New("implement me")
}

//params worker 指定机器
func (srv *DashTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.DashTxTpl, error) {
	var (
	...
	if len(changes) == 0 {
		return nil, fmt.Errorf("Dash 商户=[%d],查询Dash找零地址失败", ta.AppId)
	}
	//随机选择
	...
	if err != nil {
		log.Errorf("下单表订单id：%d,Dash 交易手续费转换异常:%s", ta.Id, err.Error())
		return nil, err
	}
	//Dash api需要金额整型
	...
	//todo Worker读取数据库
	orderReq := &transfer.DashOrderRequest{
...
//创建交易接口参数
func (srv *DashTransferService) walletServerCreate(orderReq *transfer.DashTxTpl) (string, error) {
...
//fee 手续费
func (srv *DashTransferService) setUtxoData(appid int64, orderReq *transfer.DashOrderRequest, toAddr, changeAddr string) (*transfer.DashTxTpl, error) {
	var (
		...
		coinSet         *entity.FcCoinSet //db币种配置
		//unspents     *transfer.DashUnspents
		err      error
		txInTpl  = make([]transfer.DashTxInTpl, 0)
		txOutTpl = make([]transfer.DashTxOutTpl, 0)
		tpl      *transfer.DashTxTpl
		unspents *transfer.DashUnspents
	)
	...
	DashCfg = conf.Cfg.CoinServers[srv.CoinName]
	if DashCfg == nil {
		return nil, errors.New("配置文件缺少Dash coinservers设置")
	}
	...
	if len(unspents.Data) == 0 {
		return nil, errors.New("dash empty unspents")
	}
	...
	if fee != 0 {
		if fee < 1 || fee > 100000 {
			//使用指定手续费
			return nil, errors.New("指定的手续费不在合理范围值[[0.00000001-0.001]")
		}
		feeTmp = fee
	} else {
		//先提前预估手续费
	...
	var sortUtxo transfer.DashUnspentDesc
	var sortUtxoTmp transfer.DashUnspentAsc //临时使用，小金额排序
	...
	for _, uv := range sortUtxoTmp {
		...
		amInt64 := uv.AmountFloat.Shift(8)
	...
	for _, v := range sortUtxo {
		...
		famountInt64 := v.AmountFloat.Shift(8)
		oar := transfer.DashTxInTpl{
			FromAddr:   v.Address,
			FromTxid:   v.Txid,
	...
	//组装to
	txOutTpl = append(txOutTpl, transfer.DashTxOutTpl{
		ToAddr:   toAddr,
...
//手续费计算
func (srv *DashTransferService) getFee(inNum, outNum int) (int64, error) {
