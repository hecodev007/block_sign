...
type DhxTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewDhxTransferService() service.TransferService {
	return &DhxTransferService{
		CoinName: "dhx",
		Lock:     &sync.Mutex{},
	}
}
...
var (
	DhxPrefix   = byte(0x21)
	DHXSSPrefix = []byte{0x53, 0x53, 0x35, 0x38, 0x50, 0x52, 0x45}
)

func (s *DhxTransferService) VaildAddr(address string) error {
	if address == "" {
	...
	var input []byte
	input = append(input, DHXSSPrefix...)
...
func (s *DhxTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *DhxTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *DhxTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *DhxTransferService) walletServerCreateHot(orderReq *transfer.DhxOrderRequest) (string, error) {
...
func (s *DhxTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.DhxOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	fromAddrs, err := entity.FcAddressAmount{}.FindAddress(builder.Expr("coin_type = ? and amount > ? ", coinType, fromAmount.String()).
