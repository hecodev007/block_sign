...
const MaticDecimal = 18

type MaticTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewMaticTransferService() service.TransferService {
	return &MaticTransferService{
		CoinName: "matic-matic",
		Lock:     &sync.Mutex{},
	}
}

func (s *MaticTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid matic address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *MaticTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *MaticTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *MaticTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *MaticTransferService) walletServerCreateHot(orderReq *transfer.MaticOrderRequest) (string, error) {
...
func (s *MaticTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.MaticOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(MaticDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *MaticTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
