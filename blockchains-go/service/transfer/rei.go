...
type ReiTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewReiTransferService() service.TransferService {
	return &ReiTransferService{
		CoinName: "rei",
		Lock:     &sync.Mutex{},
	}
}

func (s *ReiTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid rei address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *ReiTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *ReiTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *ReiTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *ReiTransferService) walletServerCreateHot(orderReq *transfer.ReiOrderRequest) (string, error) {
...
func (s *ReiTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.ReiOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(ReiDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *ReiTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
