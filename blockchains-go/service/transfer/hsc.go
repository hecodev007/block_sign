...
type HscTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewHscTransferService() service.TransferService {
	return &HscTransferService{
		CoinName: "hsc",
		Lock:     &sync.Mutex{},
	}
}

func (s *HscTransferService) VaildAddr(address string) error {
	if !s.has0xPrefix(address) {
		return errors.New("address must start with 0x")
	}
	if !common.IsHexAddress(address) {
		return errors.New("valid hsc address error")
	}
	//todo 验证服务器，验证雨
	return nil
...
func (s *HscTransferService) has0xPrefix(str string) bool {
	return len(str) >= 2 && str[0] == '0' && (str[1] == 'x' || str[1] == 'X')
}

func (s *HscTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.HscOrderRequest
...
func (s *HscTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *HscTransferService) walletServerCreate(orderReq *transfer.HscOrderRequest) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "main net fee too high") {
			return errors.New("HSC主网手续费过高，请值班在留意主网手续费并重推")
		}
		return err
	}
	...
	result := transfer.DecodeWalletServerRespOrder(data)
	if result == nil {
		return fmt.Errorf("order表 请求下单接口失败，outOrdrId：%s", orderReq.OuterOrderNo)
	}
	if result.Code != 0 || result.Data == nil {
		log.Error(result)
...
func (s *HscTransferService) walletServerCreateHot(orderReq *transfer.HscOrderRequest) (string, error) {
...
//私有方法 构建hsc订单
func (s *HscTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.HscOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
	)
	log.Infof("hsc platform_id", ta.AppId)
	log.Infof("hsc coin_name", ta.CoinName)
	// 查找from地址和金额
	...
	log.Infof("hsc cold addr", coldAddrs)
	log.Infof("hsc ta.Id", ta.Id)
	//查询出账地址和金额
	...
	if len(fromAddrs) == 0 {
		dao.ReportColdAddrBalanceNotEnough(int64(ta.AppId), ta.OutOrderid, "hsc", coinType, searchAmount.String())
	...
	if ta.Eostoken != "" { //如果是代币转账
		...
		orderReq.Amount = toAmount.Shift(int32(HscDecimal)).String()
	}
	return orderReq, nil
}

func (s *HscTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.HscOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
	...
	if len(fromAddrs) == 0 {
		dao.ReportColdAddrBalanceNotEnough(int64(ta.AppId), ta.OutOrderid, "hsc", coinType, toAddrs[0].ToAmount)
		return nil, fmt.Errorf("outorderNo:%s 没有符合条件的出账地址\n amount: %v \n to: %s \n ", ta.OutOrderid, toAddrs[0].ToAmount, toAddr)
	}
