...
type OneTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewOneTransferService() service.TransferService {
	return &OneTransferService{
		CoinName: "one",
		Lock:     &sync.Mutex{},
	}
}

func (s *OneTransferService) VaildAddr(address string) error {
	if strings.HasPrefix(address, "0x") {
		if !common.IsHexAddress(address) {
			return errors.New("valid one address error")
		} else {
			return nil
		}
	} else if strings.HasPrefix(address, "one") {
		toAddress, err := icap.Bech32ToAddress(address)
		if err != nil {
			return err
		}
		if !common.IsHexAddress(toAddress.String()) {
			return errors.New("valid one address error")
		} else {
			return nil
		}
	} else {
		return errors.New("不支持该地址: " + address)
	}
}

func (s *OneTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.OneOrderRequest
...
func (s *OneTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *OneTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *OneTransferService) walletServerCreateHot(orderReq *transfer.OneOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeOneTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *OneTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.OneOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(OneDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *OneTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
