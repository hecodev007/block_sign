...
const N3neoDecimal = 0

type N3neoTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewN3neoTransferService() service.TransferService {
	return &N3neoTransferService{
		CoinName: "neo",
		Lock:     &sync.Mutex{},
	}
}

func (s *N3neoTransferService) VaildAddr(address string) error {
...
func verifyAddress(to_ string) bool {
	to := strings.Replace(to_, " ", "", -1)
	if strings.HasPrefix(to, "N") && len(to) == 34 && len(to_) == 34 {
		return true
	}
	return false
}

func (s *N3neoTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.N3neoOrderRequest
	...
	if orderReq.ContractAddress != "" {
		orderReq.Token = "gas"
		orderReq.Contract = orderReq.ContractAddress
	}
...
func (s *N3neoTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *N3neoTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *N3neoTransferService) walletServerCreateHot(orderReq *transfer.N3neoOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeN3neoTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *N3neoTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.N3neoOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(N3neoDecimal)).IntPart()
	}
	return orderReq, nil
}
