...
type SatcoinTransferService struct {
	CoinName string
}

func NewSatcoinTransferService() service.TransferService {
	return &SatcoinTransferService{
		CoinName: "satcoin",
	}
}

var SatcoinNetParams = new(chaincfg.Params)

func init() {
	SatcoinNetParams.PubKeyHashAddrID = 0x3f
	SatcoinNetParams.ScriptHashAddrID = 0x41
	SatcoinNetParams.PrivateKeyID = 0x1e
	SatcoinNetParams.HDPrivateKeyID = [4]byte{0x04, 0x88, 0xad, 0xe4}
	SatcoinNetParams.HDPublicKeyID = [4]byte{0x04, 0x88, 0xb2, 0x1e}

}

func (srv *SatcoinTransferService) VaildAddr(address string) error {
	if strings.HasPrefix(address, "sat1") && len(address) == 43 {
		return nil
	}

	if strings.HasPrefix(address, "S") || strings.HasPrefix(address, "T") {
		_, err := btcutil.DecodeAddress(address, SatcoinNetParams)
		if err != nil {
			return err
		}
		return nil
...
func (srv *SatcoinTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
...
func (srv *SatcoinTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//无需实现
	return errors.New("implement me")
}

//params worker 指定机器
func (srv *SatcoinTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.SatTxTpl, error) {
	var (
	...
	if len(changes) == 0 {
		return nil, fmt.Errorf("Sat 商户=[%d],查询Sat找零地址失败", ta.AppId)
	}
	//随机选择
	...
	if err != nil {
		log.Errorf("下单表订单id：%d,Sat 交易手续费转换异常:%s", ta.Id, err.Error())
		return nil, err
	}
	//Sat api需要金额整型
	...
	//todo Worker读取数据库
	orderReq := &transfer.SatOrderRequest{
...
//创建交易接口参数
func (srv *SatcoinTransferService) walletServerCreate(orderReq *transfer.SatTxTpl) (string, error) {

	log.Infof("sat 发送url：%s", conf.Cfg.HotServers[srv.CoinName].Url+"/v1/"+strings.ToLower(srv.CoinName)+"/transfer")
	log.Infof("sat 发送结构：%+v", orderReq)
	...
	log.Infof("sat 发送返回：%s", string(data))
...
//fee 手续费
func (srv *SatcoinTransferService) setUtxoData(appid int64, orderReq *transfer.SatOrderRequest, toAddr, changeAddr string) (*transfer.SatTxTpl, error) {
	var (
		...
		SatCfg          *conf.CoinServers //币种数据服务
		coinSet         *entity.FcCoinSet //db币种配置
		//unspents     *transfer.SatUnspents
		err      error
		txInTpl  = make([]transfer.SatTxInTpl, 0)
		txOutTpl = make([]transfer.SatTxOutTpl, 0)
		tpl      *transfer.SatTxTpl
		unspents *transfer.SatUnspents
	)
	...
	SatCfg = conf.Cfg.CoinServers[srv.CoinName]
	if SatCfg == nil {
		return nil, errors.New("配置文件缺少Sat coinservers设置")
	}
	...
	//查询utxo数量
	log.Infof("发送地址：%s", SatCfg.Url+"/api/v1/satcoin/unspents")
	byteData, err := util.PostJson(SatCfg.Url+"/api/v1/satcoin/unspents", addrs)
	if err != nil {
	...
	if len(unspents.Data) == 0 {
		return nil, errors.New("sat empty unspents")
	}
	...
	var sortUtxo transfer.SatUnspentDesc
	var sortUtxoTmp transfer.SatUnspentAsc //临时使用，小金额排序
	...
	//组装to
	txOutTpl = append(txOutTpl, transfer.SatTxOutTpl{
		ToAddr:   toAddr,
...
//手续费计算
func (srv *SatcoinTransferService) getFee(inNum, outNum int) (int64, error) {
