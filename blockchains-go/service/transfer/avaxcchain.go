...
const AvaxcchainDecimal = 18

type AvaxcchainTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewAvaxcchainTransferService() service.TransferService {
	return &AvaxcchainTransferService{
		CoinName: "avaxcchain",
		Lock:     &sync.Mutex{},
	}
}

func (s *AvaxcchainTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid avaxcchain address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *AvaxcchainTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *AvaxcchainTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *AvaxcchainTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *AvaxcchainTransferService) walletServerCreateHot(orderReq *transfer.AvaxcchainOrderRequest) (string, error) {
...
func (s *AvaxcchainTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.AvaxcchainOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(AvaxcchainDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *AvaxcchainTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
