...
type DsccTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewDsccTransferService() service.TransferService {
	return &DsccTransferService{
		CoinName: "dscc",
		Lock:     &sync.Mutex{},
	}
}

func (s *DsccTransferService) VaildAddr(address string) error {

	//if !common.IsHexAddress(address) {
	//	return errors.New("valid dscc address error")
	//}
	if len(address) != 66 {
		return errors.New("valid Dscc address error")
	}
...
func (s *DsccTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *DsccTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *DsccTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *DsccTransferService) walletServerCreateHot(orderReq *transfer.DsccOrderRequest) (string, error) {
...
func (s *DsccTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.DsccOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(DsccDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *DsccTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
