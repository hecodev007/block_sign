...
type FtmTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewFtmTransferService() service.TransferService {
	return &FtmTransferService{
		CoinName: "ftm",
		Lock:     &sync.Mutex{},
	}
}

func (s *FtmTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid ftm address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *FtmTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *FtmTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *FtmTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *FtmTransferService) walletServerCreateHot(orderReq *transfer.FtmOrderRequest) (string, error) {
...
func (s *FtmTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.FtmOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(FtmDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *FtmTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
