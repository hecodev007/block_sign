...
type OriTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewOriTransferService() service.TransferService {
	return &OriTransferService{
		CoinName: "ori",
		Lock:     &sync.Mutex{},
	}
}
...
func (s *OriTransferService) VaildAddr(address string) error {
	return ss58.VerityAddress(address, ss58.SubstratePrefix)
}

func (s *OriTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.OriOrderRequest
...
func (s *OriTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *OriTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	return nil
}

func (s *OriTransferService) walletServerCreateHot(orderReq *transfer.OriOrderRequest) (string, error) {
...
func (s *OriTransferService) buildOrderHot(ta *entity.FcTransfersApply, coinDecimal int32) (*transfer.OriOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
