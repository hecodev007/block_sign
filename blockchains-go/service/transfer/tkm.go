...
type TkmTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewTkmTransferService() service.TransferService {
	return &TkmTransferService{
		CoinName: "tkm",
		Lock:     &sync.Mutex{},
	}
}

func (s *TkmTransferService) VaildAddr(address string) error {
	toAddress, err := icap.ConvertICAPToAddress(address)
	if err != nil {
		return err
	}
	if !common.IsHexAddress(toAddress.String()) {
		return errors.New("valid tkm address error")
	}
	return nil
}

func (s *TkmTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		coinSet    *entity.FcCoinSet
		orderReq   *transfer.TkmOrderRequest
...
func (s *TkmTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *TkmTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *TkmTransferService) walletServerCreateHot(orderReq *transfer.TkmOrderRequest) (string, error) {
	...
	log.Infof("%s 交易发送内容 :%s", s.CoinName, string(dd))
	log.Infof("%s 交易返回内容 :%s", s.CoinName, string(data))
	result := transfer.DecodeTkmTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
...
func (s *TkmTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.TkmOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(TkmDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *TkmTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
