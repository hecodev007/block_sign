...
const GlmrDecimal = 18

type GlmrTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewGlmrTransferService() service.TransferService {
	return &GlmrTransferService{
		CoinName: "glmr",
		Lock:     &sync.Mutex{},
	}
}

func (s *GlmrTransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid glmr address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *GlmrTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *GlmrTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *GlmrTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *GlmrTransferService) walletServerCreateHot(orderReq *transfer.GlmrOrderRequest) (string, error) {
...
func (s *GlmrTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.GlmrOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(GlmrDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *GlmrTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
