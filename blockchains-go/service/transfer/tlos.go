...
type TlosTransferService struct {
	CoinName string
	//Lock     *sync.Mutex
}

func NewTlosTransferService() service.TransferService {
	return &TlosTransferService{
		CoinName: "tlos",
		//Lock:     &sync.Mutex{},
	}
}
func (tt *TlosTransferService) VaildAddr(address string) error {
	return nil
}

func (tt *TlosTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
		orderReq   *transfer.TlosOrderRequest
	...
	orderHot := &entity.FcOrderHot{
		...
		Amount:       orderReq.Data.Amount, //转换整型
		Quantity:     orderReq.Data.Quantity,
...
func (tt *TlosTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	return nil
}
func (tt *TlosTransferService) buildOrder(ta *entity.FcTransfersApply, isCold bool) (*transfer.TlosOrderRequest, error) {
	var (
		fromAddr string
		pubKey   string
		toAddr   string
	...
	//填充参数
	orderReq := new(transfer.TlosOrderRequest)
	...
	reqTlosData := new(transfer.TlosOrderData)
	reqTlosData.FromAddress = fromAddr
	reqTlosData.ToAddress = toAddr
	reqTlosData.Memo = ta.Memo
	reqTlosData.SignPubKey = pubKey
	reqTlosData.Amount = toAmount.IntPart()
	if ta.Eostoken != "" {
		if coin.Token != ta.Eostoken {
			return nil, fmt.Errorf("合约地址不匹配 %s <> %s", coin.Token, ta.Eostoken)
		}
		reqTlosData.Token = ta.Eostoken
	} else {
		reqTlosData.Token = "eosio.token"
	}
	qcoin := "TLOS"
	reqTlosData.Quantity = fmt.Sprintf("%."+strconv.Itoa(coin.Decimal)+"f"+" %s", toAmount2, qcoin)
	if isCold {
		//todo 切换冷钱包的时候做处理
		reqTlosData.BlockId = ""
	}

	orderReq.Data = reqTlosData
	return orderReq, nil
}

func (tt *TlosTransferService) walletServiceTransfer(orderReq *transfer.TlosOrderRequest) (string, error) {
