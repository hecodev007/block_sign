...
type WelTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewWelTransferService() service.TransferService {
	return &WelTransferService{
		CoinName: "welups",
		Lock:     &sync.Mutex{},
	}
}

func (s *WelTransferService) VaildAddr(address string) error {
	_, err := util.DecodeCheck(address)
	if err != nil {
		log.Infof("vaildAddr %s error : ", address)
		return errors.New("error address")
	}
	return nil
}

func (s *WelTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *WelTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *WelTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *WelTransferService) walletServerCreateHot(orderReq *transfer.WelOrderRequest) (string, error) {
...
func (s *WelTransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.WelOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(WelDecimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *WelTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
