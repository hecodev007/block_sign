...
type BiwTransferService struct {
	CoinName string
}

func NewBiwTransferService() service.TransferService {
	return &BiwTransferService{
		CoinName: "biw",
	}
}

var BiwNetParams = new(chaincfg.Params)

func init() {
	BiwNetParams.PubKeyHashAddrID = 0x00
	BiwNetParams.ScriptHashAddrID = 0x05
	BiwNetParams.PrivateKeyID = 0x80
	BiwNetParams.HDPrivateKeyID = [4]byte{0x04, 0x88, 0xad, 0xe4}
	BiwNetParams.HDPublicKeyID = [4]byte{0x04, 0x88, 0xb2, 0x1e}

}

func (srv *BiwTransferService) VaildAddr(address string) error {
	if strings.HasPrefix(address, "1") || strings.HasPrefix(address, "3") {
		_, err := btcutil.DecodeAddress(address, BiwNetParams)
		if err != nil {
			return err
		}
		return nil
...
func (srv *BiwTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
...
func (srv *BiwTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//无需实现
	return errors.New("implement me")
}

//params worker 指定机器
func (srv *BiwTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.BiwTxTpl, error) {
	var (
	...
	if len(changes) == 0 {
		return nil, fmt.Errorf("Biw 商户=[%d],查询Biw找零地址失败", ta.AppId)
	}
	//随机选择
	...
	if err != nil {
		log.Errorf("下单表订单id：%d,Biw 交易手续费转换异常:%s", ta.Id, err.Error())
		return nil, err
	}
	//Biw api需要金额整型
	...
	//todo Worker读取数据库
	orderReq := &transfer.BiwOrderRequest{
...
//创建交易接口参数
func (srv *BiwTransferService) walletServerCreate(orderReq *transfer.BiwTxTpl) (string, error) {

	log.Infof("biw 发送url：%s", conf.Cfg.HotServers[srv.CoinName].Url+"/v1/"+strings.ToLower(srv.CoinName)+"/transfer")
	log.Infof("biw 发送结构：%+v", orderReq)
	...
	log.Infof("biw 发送返回：%s", string(data))
...
//fee 手续费
func (srv *BiwTransferService) setUtxoData(appid int64, orderReq *transfer.BiwOrderRequest, toAddr, changeAddr string) (*transfer.BiwTxTpl, error) {
	var (
		...
		BiwCfg          *conf.CoinServers //币种数据服务
		coinSet         *entity.FcCoinSet //db币种配置
		//unspents     *transfer.BiwUnspents
		err      error
		txInTpl  = make([]transfer.BiwTxInTpl, 0)
		txOutTpl = make([]transfer.BiwTxOutTpl, 0)
		tpl      *transfer.BiwTxTpl
		unspents *transfer.BiwUnspents
	)
	...
	BiwCfg = conf.Cfg.CoinServers[srv.CoinName]
	if BiwCfg == nil {
		return nil, errors.New("配置文件缺少Biw coinservers设置")
	}
	...
	//查询utxo数量
	byteData, err := util.PostJson(BiwCfg.Url+"/api/v1/biw/unspents", addrs)
	if err != nil {
		return nil, fmt.Errorf("获取utxo异常，err:%s", err.Error())
	}
	unspents = new(transfer.BiwUnspents)
	...
	if len(unspents.Data) == 0 {
		return nil, errors.New("biw empty unspents")
	}
	...
	var sortUtxo transfer.BiwUnspentDesc
	var sortUtxoTmp transfer.BiwUnspentAsc //临时使用，小金额排序
	...
	//组装to
	txOutTpl = append(txOutTpl, transfer.BiwTxOutTpl{
		ToAddr:   toAddr,
...
//手续费计算
func (srv *BiwTransferService) getFee(inNum, outNum int) (int64, error) {
