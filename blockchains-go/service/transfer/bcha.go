...
type BchaTransferService struct {
	CoinName  string
	minAmount int64
}

func NewBchaTransferService() service.TransferService {
	return &BchaTransferService{
		CoinName:  "bcha",
		minAmount: 546,
	}
}

func (srv *BchaTransferService) VaildAddr(address string) error {
	url := conf.Cfg.CoinServers[srv.CoinName].Url + "/api/v1/bcha/validateaddress?address=%s"
	...
	BchaResp := transfer.DecodeBchaAddressResult(data)
	if BchaResp != nil && BchaResp.Data != nil {
		if BchaResp.Data.Isvalid {
			return nil
		}
	}
...
func (srv *BchaTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
...
func (srv *BchaTransferService) TransferCold(ta *entity.FcTransfersApply) error {
	//无需实现
	return errors.New("implement me")
}

//params worker 指定机器
func (srv *BchaTransferService) getEstimateTpl(ta *entity.FcTransfersApply, worker string) (*transfer.BchaTxTpl, error) {
	var (
	...
	if len(changes) == 0 {
		return nil, fmt.Errorf("Bcha 商户=[%d],查询Bcha找零地址失败", ta.AppId)
	}
	//随机选择
	...
	if err != nil {
		log.Errorf("下单表订单id：%d,Bcha 交易手续费转换异常:%s", ta.Id, err.Error())
		return nil, err
	}
	//Bcha api需要金额整型
	...
	//todo Worker读取数据库
	orderReq := &transfer.BchaOrderRequest{
...
//创建交易接口参数
func (srv *BchaTransferService) walletServerCreate(orderReq *transfer.BchaTxTpl) (string, error) {

	log.Infof("bcha 发送url：%s", conf.Cfg.HotServers[srv.CoinName].Url+"/v1/bcha/transfer")
	log.Infof("bcha 发送结构：%+v", orderReq)
	data, err := util.PostJsonByAuth(conf.Cfg.HotServers[srv.CoinName].Url+"/v1/bcha/transfer", conf.Cfg.HotServers[srv.CoinName].User, conf.Cfg.HotServers[srv.CoinName].Password, orderReq)
	if err != nil {
		return "", err
	}
...
//fee 手续费
func (srv *BchaTransferService) setUtxoData(appid int64, orderReq *transfer.BchaOrderRequest, toAddr, changeAddr string) (*transfer.BchaTxTpl, error) {
	var (
		...
		BchaCfg         *conf.CoinServers //币种数据服务
		coinSet         *entity.FcCoinSet //db币种配置
		//unspents     *transfer.BtcUnspents
		err      error
		txInTpl  = make([]transfer.BchaTxInTpl, 0)
		txOutTpl = make([]transfer.BchaTxOutTpl, 0)
		tpl      *transfer.BchaTxTpl
		unspents *transfer.BtcUnspents
	)
	...
	BchaCfg = conf.Cfg.CoinServers[srv.CoinName]
	if BchaCfg == nil {
		return nil, errors.New("配置文件缺少Bcha coinservers设置")
	}
	...
	//查询utxo数量
	byteData, err := util.PostJson(BchaCfg.Url+"/api/v1/bcha/unspents", addrs)
	if err != nil {
	...
	if len(unspents.Data) == 0 {
		return nil, errors.New("bcha empty unspents")
	}
	...
	//组装to
	txOutTpl = append(txOutTpl, transfer.BchaTxOutTpl{
		ToAddr:   toAddr,
...
//手续费计算
func (srv *BchaTransferService) getFee(inNum, outNum int) (int64, error) {
