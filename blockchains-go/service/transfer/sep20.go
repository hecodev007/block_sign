...
const Sep20Decimal = 18

type Sep20TransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewSep20TransferService() service.TransferService {
	return &Sep20TransferService{
		CoinName: "sep20",
		Lock:     &sync.Mutex{},
	}
}

func (s *Sep20TransferService) VaildAddr(address string) error {
	if !common.IsHexAddress(address) {
		return errors.New("valid sep20 address error")
	}
	//todo 验证服务器，验证雨
	return nil
}

func (s *Sep20TransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	var (
		mch        *entity.FcMch
...
func (s *Sep20TransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
////创建交易接口参数
//func (s *Sep20TransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
...
func (s *Sep20TransferService) walletServerCreateHot(orderReq *transfer.Sep20OrderRequest) (string, error) {
...
func (s *Sep20TransferService) buildOrderHot(ta *entity.FcTransfersApply) (*transfer.Sep20OrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if ta.Eostoken != "" {
		...
		orderReq.Amount = toAmount.Shift(int32(Sep20Decimal)).String()
	}
	return orderReq, nil
}

//私有方法 构建eth订单
func (s *Sep20TransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
		toAmount decimal.Decimal
