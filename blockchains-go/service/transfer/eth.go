...
type EthTransferService struct {
	CoinName string
	Lock     *sync.Mutex
}

func NewEthTransferService() service.TransferService {
...
func (s *EthTransferService) VaildAddr(address string) error {
...
func (s *EthTransferService) TransferHot(ta *entity.FcTransfersApply) (txid string, err error) {
	panic("implement me")
}

func (s *EthTransferService) TransferCold(ta *entity.FcTransfersApply) error {
...
//=================私有方法=================
//创建交易接口参数
func (s *EthTransferService) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "main net fee too high") {
			return errors.New("ETH主网手续费过高，请值班在留意主网手续费并重推")
		}
		return err
	}
...
//私有方法 构建eth订单
func (s *EthTransferService) buildOrder(ta *entity.FcTransfersApply) (*transfer.EthOrderRequest, error) {
	var (
		fromAddr string
		toAddr   string
	...
	if len(fromAddrs) == 0 {
		dao.ReportColdAddrBalanceNotEnough(int64(ta.AppId), ta.OutOrderid, "eth", coinType, searchAmount.String())
	...
	if ta.Eostoken != "" { //如果是代币转账
		...
		orderReq.Amount = toAmount.Shift(int32(EthDecimal)).String()
	}
	return orderReq, nil
}
