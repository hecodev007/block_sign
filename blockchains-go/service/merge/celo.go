...
var (
	celoName = "celo"
	celoFee  = decimal.NewFromFloat(0.000375)
)

type CeloMergeService struct {
	CoinName string
}

func NewCeloMergeService() service.MergeService {
	return &CeloMergeService{CoinName: "celo"}
}

func (b *CeloMergeService) MergeCoin(params merge.MergeParams) (string, error) {
	...
	var (
		...
		CeloEof           decimal.Decimal
		CeloMergeMin      decimal.Decimal
		CeloMergeTokenMin decimal.Decimal
	)

	CeloEof = conf.Cfg.Merge[strings.ToLower(params.Coin)].BalanceThreshold
	if CeloEof.LessThanOrEqual(celoFee) {
		CeloEof = celoFee.Mul(decimal.New(2, 0))
	}
	CeloMergeMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeThreshold
	if CeloMergeMin.LessThanOrEqual(celoFee) {
		CeloMergeMin = celoFee
	}
	CeloMergeTokenMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeTokenThreshold
	if CeloMergeTokenMin.LessThanOrEqual(decimal.Zero) {
		CeloMergeTokenMin = decimal.New(1, -8)
	}
	...
	for _, v := range params.Froms {
		...
		if params.Token == "" {
			...
			for _, am := range addressAmounts {
				if strings.ToLower(am.CoinType) == celoName {
					amDecimal, _ := decimal.NewFromString(am.Amount)
					mergeDecimal := amDecimal.Sub(CeloEof.Add(celoFee))
					if mergeDecimal.LessThan(CeloMergeMin) {

						msg = msg + "," + fmt.Sprintf("地址[%s],扣除手续费[%s], 保留余额[%s],归集金额[%s]，小于阈值[%s],抛弃归集",
							v, celoFee.String(), CeloEof.String(), mergeDecimal.String(), CeloMergeMin)
			...
			for _, am := range addressAmounts {
				amDecimal, _ := decimal.NewFromString(am.Amount)
				if strings.ToLower(am.CoinType) == celoName {
					if amDecimal.GreaterThanOrEqual(celoFee) {
						isHasCelo = true
					}
				}
				...
				if strings.ToLower(am.CoinType) == coinName {
					if amDecimal.GreaterThan(CeloMergeTokenMin) {
						isHasToken = true
						amount = amDecimal
					}
				}
...
//返回订单ID
func (b *CeloMergeService) createOrder(appid int, coin, token, from, to string, amountFloat decimal.Decimal) (*transfer.CeloOrderRequest, error) {
	...
	orderReq := &transfer.CeloOrderRequest{
		...
		FromAddress: from,
		ToAddress:   to,
		Amount:      amountFloat.Shift(18).String(),
		IsCollect:   1,
	}
...
//创建交易接口参数
func (b *CeloMergeService) walletServerCreateCold(orderReq *transfer.CeloOrderRequest) error {
	data, err := util.PostJsonByAuth(conf.Cfg.HotServers[orderReq.CoinName].Url+"/v1/celo/transfer", conf.Cfg.HotServers[orderReq.CoinName].User, conf.Cfg.HotServers[orderReq.CoinName].Password, orderReq)
	if err != nil {
		return err
	}
