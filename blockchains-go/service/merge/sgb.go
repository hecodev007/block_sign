...
var (
	sgbName    = "sgb-sgb"
	sgbDecimal = int32(10)
	sgbMinFee  = decimal.NewFromFloat(0.1)
)

type SgbMergeService struct {
	CoinName string
}

func NewSgbMergeService() service.MergeService {
	return &SgbMergeService{CoinName: "sgb-sgb"}
}

func (b *SgbMergeService) MergeCoin(params merge.MergeParams) (string, error) {
	...
	var (
		...
		redisKeyName     string
		coinName         string
		orderReq         *transfer.SgbOrderRequest
		sgbEof           decimal.Decimal //合并金额的时候保留阈值金额在冷地址
		sgbMergeMin      decimal.Decimal //主链币归集起始金额
		sgbMergeTokenMin decimal.Decimal //代币归集起始金额
	)
	//保留的金额 最低为1.1
	sgbEof = conf.Cfg.Merge[strings.ToLower(params.Coin)].BalanceThreshold
	if sgbEof.IsZero() {
		//sgbEof = decimal.NewFromFloat(1.1)
	}

	sgbMergeMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeThreshold
	if sgbMergeMin.IsZero() {
		sgbMergeMin = decimal.NewFromFloat(1)
	}

	sgbMergeTokenMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeTokenThreshold
	if sgbMergeTokenMin.IsZero() {
		sgbMergeTokenMin = decimal.NewFromFloat(1)
	}
	...
	for _, v := range params.Froms {
		...
		if params.Token == "" {
			...
			for _, am := range addressAmounts {
				if strings.ToLower(am.CoinType) == sgbName {
					amDecimal, _ := decimal.NewFromString(am.Amount)
					mergeDecimal := amDecimal.Sub(sgbEof)
					log.Infof("amDecimal:%s", amDecimal.String())
					log.Infof("sgbEof:%s", sgbEof.String())
					log.Infof("mergeDecimal:%s", mergeDecimal.String())
					if mergeDecimal.LessThan(sgbMergeMin) {
						msg = msg + "," + fmt.Sprintf("地址[%s], 保留余额[%s],归集金额[%s]，小于阈值[%s],抛弃归集",
							v, sgbEof.String(), mergeDecimal.String(), sgbMergeMin)
			...
			for _, am := range addressAmounts {
				amDecimal, _ := decimal.NewFromString(am.Amount)
				if strings.ToLower(am.CoinType) == sgbName {
					if amDecimal.GreaterThanOrEqual(sgbMinFee) {
						isHasSgb = true
					}
				}
				...
				if strings.ToLower(am.CoinType) == coinName {
					if amDecimal.GreaterThan(sgbMergeTokenMin) {
						isHasToken = true
						amount = amDecimal
					}
				}
...
//返回订单ID
func (b *SgbMergeService) createOrder(appid int, coin, token, from, to string, amountFloat decimal.Decimal) (*transfer.SgbOrderRequest, error) {
	...
	orderReq := &transfer.SgbOrderRequest{
		...
		FromAddress: from,
		ToAddress:   to,
		Amount:      amountFloat.Shift(sgbDecimal).String(),
	}
...
//创建交易接口参数
func (b *SgbMergeService) walletServerCreateHot(orderReq *transfer.SgbOrderRequest) (string, error) {
