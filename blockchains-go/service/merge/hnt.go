...
var (
	hntName    = "hnt"
	hntDecimal = int32(8)
	hntMinFee  = decimal.NewFromFloat(0.0)
)

type HntMergeService struct {
	CoinName string
}

func NewHntMergeService() service.MergeService {
	return &HntMergeService{CoinName: "hnt"}
}

func (b *HntMergeService) MergeCoin(phntams merge.MergeParams) (string, error) {
	if _, ok := conf.Cfg.Merge[strings.ToLower(phntams.Coin)]; !ok {
		return "", fmt.Errorf("配置文件缺少币种[%s]合并配置", phntams.Coin)
	}

	log.Infof("合并配置：%+v", conf.Cfg.Merge[strings.ToLower(phntams.Coin)])

	if phntams.AppId == 0 || strings.ToLower(phntams.Coin) != b.CoinName || len(phntams.Froms) == 0 || phntams.To == "" {
		return "", errors.New("error phntams")
	}
	...
	var (
		...
		hntEof           decimal.Decimal //合并金额的时候保留阈值金额在冷地址
		hntMergeMin      decimal.Decimal //主链币归集起始金额
		hntMergeTokenMin decimal.Decimal //代币归集起始金额
	)

	hntEof = conf.Cfg.Merge[strings.ToLower(phntams.Coin)].BalanceThreshold
	if hntEof.IsZero() {
		hntEof = decimal.NewFromFloat(0.5)
	}

	hntMergeMin = conf.Cfg.Merge[strings.ToLower(phntams.Coin)].MergeThreshold
	if hntMergeMin.IsZero() {
		hntMergeMin = decimal.NewFromFloat(1)
	}

	hntMergeTokenMin = conf.Cfg.Merge[strings.ToLower(phntams.Coin)].MergeTokenThreshold
	if hntMergeTokenMin.IsZero() {
		hntMergeTokenMin = decimal.NewFromFloat(1)
	}
	coinName = phntams.Coin
	if phntams.Token != "" {
		coinName = phntams.Token
	}
	...
	//批量找出冷地址
	addresses, err = dao.FcGenerateAddressListFindAddressesData(address.AddressTypeCold.ToInt(), int(address.AddressStatusAlloc), phntams.AppId, phntams.Coin)
	if err != nil {
		return "", err
	}

	//不允许自己归集自己
	for _, v := range phntams.Froms {
		if v == phntams.To {
	...
	for _, v := range addresses {
		if v.Address == phntams.To {
			isColdAddrTo = true
			break
		}
	}
	if !isColdAddrTo {
		return "", fmt.Errorf("接收地址：%s,非商户冷地址", phntams.To)
	}

	//验证from地址是否是冷地址,用时添加进入map临时备用
	for _, from := range phntams.Froms {
	...
	for _, v := range phntams.Froms {
		...
		if has, _ := redisHelper.Exists(redisKeyName); has {
			...
			//过期时间
			redisHelper.Set(redisKeyName, phntams)
		...
		//查找这个地址的所有币种金额
		addressAmounts, err := dao.FcAddressAmountFindAddress(address.AddressTypeCold.ToInt(), phntams.AppId, v)
		if err != nil {
		...
		if phntams.Token == "" {
			...
			for _, am := range addressAmounts {
				if strings.ToLower(am.CoinType) == hntName {
					amDecimal, _ := decimal.NewFromString(am.Amount)
					mergeDecimal := amDecimal.Sub(hntEof)
					log.Infof("amDecimal:%s", amDecimal.String())
					log.Infof("hntEof:%s", hntEof.String())
					log.Infof("mergeDecimal:%s", mergeDecimal.String())
					if mergeDecimal.LessThan(hntMergeMin) {
						msg = msg + "," + fmt.Sprintf("地址[%s], 保留余额[%s],归集金额[%s]，小于阈值[%s],抛弃归集",
							v, hntEof.String(), mergeDecimal.String(), hntMergeMin)
			...
			for _, am := range addressAmounts {
				amDecimal, _ := decimal.NewFromString(am.Amount)
				if strings.ToLower(am.CoinType) == hntName {
					if amDecimal.GreaterThanOrEqual(hntMinFee) {
						isHasHnt = true
					}
				}
				...
				if strings.ToLower(am.CoinType) == coinName {
					if amDecimal.GreaterThan(hntMergeTokenMin) {
						isHasToken = true
						amount = amDecimal
					}
				}
		...
		//构建交易
		orderReq, err = b.createOrder(phntams.AppId, phntams.Coin, phntams.Token, v, phntams.To, amount)
		//然后直接发送给底层交易
		if err != nil {
...
//返回订单ID
func (b *HntMergeService) createOrder(appid int, coin, token, from, to string, amountFloat decimal.Decimal) (*transfer.HntOrderRequest, error) {
	...
	orderReq := &transfer.HntOrderRequest{
		...
		FromAddress: from,
		ToAddress:   to,
		Amount:      amountFloat.Shift(hntDecimal).String(),
	}
...
//创建交易接口参数
func (b *HntMergeService) walletServerCreateHot(orderReq *transfer.HntOrderRequest) (string, error) {
