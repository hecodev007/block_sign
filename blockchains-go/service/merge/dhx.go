...
var (
	dhxName    = "dhx"
	dhxDecimal = int32(18)
	dhxMinFee  = decimal.NewFromFloat(0.1)
)

type DhxMergeService struct {
	CoinName string
}

func NewDhxMergeService() service.MergeService {
	return &DhxMergeService{CoinName: "dhx"}
}

func (b *DhxMergeService) MergeCoin(params merge.MergeParams) (string, error) {
	...
	var (
		...
		redisKeyName     string
		coinName         string
		orderReq         *transfer.DhxOrderRequest
		dhxEof           decimal.Decimal //合并金额的时候保留阈值金额在冷地址
		dhxMergeMin      decimal.Decimal //主链币归集起始金额
		dhxMergeTokenMin decimal.Decimal //代币归集起始金额
	)
	//保留的金额 最低为1.1
	dhxEof = conf.Cfg.Merge[strings.ToLower(params.Coin)].BalanceThreshold
	if dhxEof.IsZero() {
		dhxEof = decimal.NewFromFloat(1.1)
	}

	dhxMergeMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeThreshold
	if dhxMergeMin.IsZero() {
		dhxMergeMin = decimal.NewFromFloat(1)
	}

	dhxMergeTokenMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeTokenThreshold
	if dhxMergeTokenMin.IsZero() {
		dhxMergeTokenMin = decimal.NewFromFloat(1)
	}
	...
	for _, v := range params.Froms {
		...
		if params.Token == "" {
			...
			for _, am := range addressAmounts {
				if strings.ToLower(am.CoinType) == dhxName {
					amDecimal, _ := decimal.NewFromString(am.Amount)
					mergeDecimal := amDecimal.Sub(dhxEof)
					log.Infof("amDecimal:%s", amDecimal.String())
					log.Infof("dhxEof:%s", dhxEof.String())
					log.Infof("mergeDecimal:%s", mergeDecimal.String())
					if mergeDecimal.LessThan(dhxMergeMin) {
						msg = msg + "," + fmt.Sprintf("地址[%s], 保留余额[%s],归集金额[%s]，小于阈值[%s],抛弃归集",
							v, dhxEof.String(), mergeDecimal.String(), dhxMergeMin)
			...
			for _, am := range addressAmounts {
				amDecimal, _ := decimal.NewFromString(am.Amount)
				if strings.ToLower(am.CoinType) == dhxName {
					if amDecimal.GreaterThanOrEqual(dhxMinFee) {
						isHasDhx = true
					}
				}
				...
				if strings.ToLower(am.CoinType) == coinName {
					if amDecimal.GreaterThan(dhxMergeTokenMin) {
						isHasToken = true
						amount = amDecimal
					}
				}
...
//返回订单ID
func (b *DhxMergeService) createOrder(appid int, coin, token, from, to string, amountFloat decimal.Decimal) (*transfer.DhxOrderRequest, error) {
	...
	orderReq := &transfer.DhxOrderRequest{
		...
		FromAddress: from,
		ToAddress:   to,
		Amount:      amountFloat.Shift(dhxDecimal).String(),
	}
...
//创建交易接口参数
func (b *DhxMergeService) walletServerCreateHot(orderReq *transfer.DhxOrderRequest) (string, error) {
