...
var (
	cdsName    = "cds"
	cdsDecimal = int32(18)
	cdsMinFee  = decimal.NewFromFloat(0.0008)
)

type CdsMergeService struct {
	CoinName string
}

func NewCdsMergeService() service.MergeService {
	return &CdsMergeService{CoinName: "cds"}
}

func (b *CdsMergeService) MergeCoin(params merge.MergeParams) (string, error) {
	...
	var (
		...
		redisKeyName     string
		coinName         string
		orderReq         *transfer.CdsOrderRequest
		cdsEof           decimal.Decimal //合并金额的时候保留阈值金额在冷地址
		cdsMergeMin      decimal.Decimal //主链币归集起始金额
		cdsMergeTokenMin decimal.Decimal //代币归集起始金额
	)

	cdsEof = conf.Cfg.Merge[strings.ToLower(params.Coin)].BalanceThreshold
	if cdsEof.IsZero() {
		cdsEof = decimal.NewFromFloat(0.5)
	}

	cdsMergeMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeThreshold
	if cdsMergeMin.IsZero() {
		cdsMergeMin = decimal.NewFromFloat(1)
	}

	cdsMergeTokenMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeTokenThreshold
	if cdsMergeTokenMin.IsZero() {
		cdsMergeTokenMin = decimal.NewFromFloat(1)
	}
	...
	for _, v := range params.Froms {
		...
		if params.Token == "" {
			...
			for _, am := range addressAmounts {
				if strings.ToLower(am.CoinType) == cdsName {
					amDecimal, _ := decimal.NewFromString(am.Amount)
					mergeDecimal := amDecimal.Sub(cdsEof)
					if mergeDecimal.LessThan(cdsMergeMin) {

						msg = msg + "," + fmt.Sprintf("地址[%s], 保留余额[%s],归集金额[%s]，小于阈值[%s],抛弃归集",
							v, cdsEof.String(), mergeDecimal.String(), cdsMergeMin)
			...
			for _, am := range addressAmounts {
				amDecimal, _ := decimal.NewFromString(am.Amount)
				if strings.ToLower(am.CoinType) == cdsName {
					if amDecimal.GreaterThanOrEqual(cdsMinFee) {
						isHasCds = true
					}
				}
				...
				if strings.ToLower(am.CoinType) == coinName {
					if amDecimal.GreaterThan(cdsMergeTokenMin) {
						isHasToken = true
						amount = amDecimal
					}
				}
...
//返回订单ID
func (b *CdsMergeService) createOrder(appid int, coin, token, from, to string, amountFloat decimal.Decimal) (*transfer.CdsOrderRequest, error) {
	...
	orderReq := &transfer.CdsOrderRequest{
		...
		FromAddress: from,
		ToAddress:   to,
		Amount:      amountFloat.Shift(cdsDecimal).String(),
		IsCollect:   0,
	}
...
//创建交易接口参数
func (b *CdsMergeService) walletServerCreateHot(orderReq *transfer.CdsOrderRequest) (string, error) {
	...
	log.Infof("cds返回来内容：%s", string(data))
	result := transfer.DecodeCdsTransferResp(data)
	if result == nil {
