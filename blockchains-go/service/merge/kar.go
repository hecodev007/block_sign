...
var (
	karName    = "kar"
	karDecimal = int32(12)
	karMinFee  = decimal.NewFromFloat(0.1)
)

type KarMergeService struct {
	CoinName string
}

func NewKarMergeService() service.MergeService {
	return &KarMergeService{CoinName: "kar"}
}

func (b *KarMergeService) MergeCoin(params merge.MergeParams) (string, error) {
	...
	var (
		...
		redisKeyName     string
		coinName         string
		orderReq         *transfer.KarOrderRequest
		karEof           decimal.Decimal //合并金额的时候保留阈值金额在冷地址
		karMergeMin      decimal.Decimal //主链币归集起始金额
		karMergeTokenMin decimal.Decimal //代币归集起始金额
	)
	//保留的金额 最低为1.1
	karEof = conf.Cfg.Merge[strings.ToLower(params.Coin)].BalanceThreshold
	if karEof.IsZero() {
		//karEof = decimal.NewFromFloat(1.1)
	}

	karMergeMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeThreshold
	if karMergeMin.IsZero() {
		karMergeMin = decimal.NewFromFloat(1)
	}

	karMergeTokenMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeTokenThreshold
	if karMergeTokenMin.IsZero() {
		karMergeTokenMin = decimal.NewFromFloat(1)
	}
	...
	for _, v := range params.Froms {
		...
		if params.Token == "" {
			...
			for _, am := range addressAmounts {
				if strings.ToLower(am.CoinType) == karName {
					amDecimal, _ := decimal.NewFromString(am.Amount)
					mergeDecimal := amDecimal.Sub(karEof)
					log.Infof("amDecimal:%s", amDecimal.String())
					log.Infof("karEof:%s", karEof.String())
					log.Infof("mergeDecimal:%s", mergeDecimal.String())
					if mergeDecimal.LessThan(karMergeMin) {
						msg = msg + "," + fmt.Sprintf("地址[%s], 保留余额[%s],归集金额[%s]，小于阈值[%s],抛弃归集",
							v, karEof.String(), mergeDecimal.String(), karMergeMin)
			...
			for _, am := range addressAmounts {
				amDecimal, _ := decimal.NewFromString(am.Amount)
				if strings.ToLower(am.CoinType) == karName {
					if amDecimal.GreaterThanOrEqual(karMinFee) {
						isHasKar = true
					}
				}
				...
				if strings.ToLower(am.CoinType) == coinName {
					if amDecimal.GreaterThan(karMergeTokenMin) {
						isHasToken = true
						amount = amDecimal
					}
				}
...
//返回订单ID
func (b *KarMergeService) createOrder(appid int, coin, token, from, to string, amountFloat decimal.Decimal) (*transfer.KarOrderRequest, error) {
	...
	orderReq := &transfer.KarOrderRequest{
		...
		FromAddress: from,
		ToAddress:   to,
		Amount:      amountFloat.Shift(karDecimal).String(),
	}
...
//创建交易接口参数
func (b *KarMergeService) walletServerCreateHot(orderReq *transfer.KarOrderRequest) (string, error) {
