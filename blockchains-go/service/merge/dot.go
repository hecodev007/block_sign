...
var (
	dotName    = "dot"
	dotDecimal = int32(10)
	dotMinFee  = decimal.NewFromFloat(0.1)
)

type DotMergeService struct {
	CoinName string
}

func NewDotMergeService() service.MergeService {
	return &DotMergeService{CoinName: "dot"}
}

func (b *DotMergeService) MergeCoin(params merge.MergeParams) (string, error) {
	...
	var (
		...
		dotEof           decimal.Decimal //合并金额的时候保留阈值金额在冷地址
		dotMergeMin      decimal.Decimal //主链币归集起始金额
		dotMergeTokenMin decimal.Decimal //代币归集起始金额
	)
	//保留的金额 最低为1.1
	dotEof = conf.Cfg.Merge[strings.ToLower(params.Coin)].BalanceThreshold
	if dotEof.IsZero() {
		dotEof = decimal.NewFromFloat(1.1)
	}

	dotMergeMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeThreshold
	if dotMergeMin.IsZero() {
		dotMergeMin = decimal.NewFromFloat(1)
	}

	dotMergeTokenMin = conf.Cfg.Merge[strings.ToLower(params.Coin)].MergeTokenThreshold
	if dotMergeTokenMin.IsZero() {
		dotMergeTokenMin = decimal.NewFromFloat(1)
	}
	...
	for _, v := range params.Froms {
		...
		if params.Token == "" {
			...
			for _, am := range addressAmounts {
				if strings.ToLower(am.CoinType) == dotName {
					amDecimal, _ := decimal.NewFromString(am.Amount)
					mergeDecimal := amDecimal.Sub(dotEof)
					log.Infof("amDecimal:%s", amDecimal.String())
					log.Infof("dotEof:%s", dotEof.String())
					log.Infof("mergeDecimal:%s", mergeDecimal.String())
					if mergeDecimal.LessThan(dotMergeMin) {
						msg = msg + "," + fmt.Sprintf("地址[%s], 保留余额[%s],归集金额[%s]，小于阈值[%s],抛弃归集",
							v, dotEof.String(), mergeDecimal.String(), dotMergeMin)
			...
			for _, am := range addressAmounts {
				amDecimal, _ := decimal.NewFromString(am.Amount)
				if strings.ToLower(am.CoinType) == dotName {
					if amDecimal.GreaterThanOrEqual(dotMinFee) {
						isHasDot = true
					}
				}
				...
				if strings.ToLower(am.CoinType) == coinName {
					if amDecimal.GreaterThan(dotMergeTokenMin) {
						isHasToken = true
						amount = amDecimal
					}
				}
...
//返回订单ID
func (b *DotMergeService) createOrder(appid int, coin, token, from, to string, amountFloat decimal.Decimal) (*transfer.DotOrderRequest, error) {
	...
	orderReq := &transfer.DotOrderRequest{
		...
		FromAddress: from,
		ToAddress:   to,
		Amount:      amountFloat.Shift(dotDecimal).String(),
	}
...
//创建交易接口参数
func (b *DotMergeService) walletServerCreateHot(orderReq *transfer.DotOrderRequest) (string, error) {
