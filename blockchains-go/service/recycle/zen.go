...
type ZenRecycleService struct {
	CoinName    string
	CoinDecimal int32
}

func NewZenRecycleService() service.RecycleService {
	return &ZenRecycleService{CoinName: "zen", CoinDecimal: 8}
}

//params model : 0小额合并 1大额合并
func (b *ZenRecycleService) RecycleCoin(reqHead *transfer.OrderRequestHead, toAddr string, feeFloat decimal.Decimal, model int) (msg string, err error) {
	var (
		addrAms         []*entity.FcAddressAmount
		scanNum         int
		zenCfg          *conf.CoinServers                 //币种数据服务
		addrs           = make([]string, 0)               //utxo地址
		unspents        *transfer.ZenUnspents             //utxo
		feeTmp          int64                             //临时估算手续费
		fromAmountInt64 decimal.Decimal                   //from金额
		toAmountInt64   decimal.Decimal                   //to金额
		sortUtxoDesc    transfer.ZenUnspentDesc           //大额
		sortUtxoAsc     transfer.ZenUnspentAsc            //小额
		txins           = make([]transfer.ZenTxInTpl, 0)  //utxo模板
		txouts          = make([]transfer.ZenTxOutTpl, 0) //utxo模板
		tpl             *transfer.ZenTxTpl                //模板
	)
	...
	//手续计算
	feeTmp, err = b.getZenFee(len(txins), 1)
	//step4：组装交易发送给冷签名端
	toAmountInt64 = fromAmountInt64.Sub(decimal.New(feeTmp, 0))
	txouts = append(txouts, transfer.ZenTxOutTpl{
		ToAddr:   toAddr,
...
//创建交易接口参数
func (srv *ZenRecycleService) walletServerCreate(orderReq *transfer.ZenTxTpl) (string, error) {
...
//手续费计算
func (srv *ZenRecycleService) getZenFee(inNum, outNum int) (int64, error) {
