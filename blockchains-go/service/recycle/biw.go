...
type BiwRecycleService struct {
	CoinName    string
	CoinDecimal int32
}

func NewBiwRecycleService() service.RecycleService {
	return &BiwRecycleService{CoinName: "biw", CoinDecimal: 8}
}

//params model : 0小额合并 1大额合并
func (b *BiwRecycleService) RecycleCoin(reqHead *transfer.OrderRequestHead, toAddr string, feeFloat decimal.Decimal, model int) (msg string, err error) {
	var (
		addrAms         []*entity.FcAddressAmount
		scanNum         int
		biwCfg          *conf.CoinServers                 //币种数据服务
		addrs           = make([]string, 0)               //utxo地址
		unspents        *transfer.BiwUnspents             //utxo
		feeTmp          int64                             //临时估算手续费
		fromAmountInt64 decimal.Decimal                   //from金额
		toAmountInt64   decimal.Decimal                   //to金额
		sortUtxoDesc    transfer.BiwUnspentDesc           //大额
		sortUtxoAsc     transfer.BiwUnspentAsc            //小额
		txins           = make([]transfer.BiwTxInTpl, 0)  //utxo模板
		txouts          = make([]transfer.BiwTxOutTpl, 0) //utxo模板
		tpl             *transfer.BiwTxTpl                //模板
	)
	...
	biwCfg = conf.Cfg.CoinServers[reqHead.CoinName]
	if biwCfg == nil {
		return "", errors.New("配置文件缺少biw coinservers设置")
	}
	...
	if unspents.Code != 0 || len(unspents.Data) == 0 {
		return "", errors.New("biw empty unspents")
	}
	...
	//手续计算
	feeTmp, err = b.getBiwFee(len(txins), 1)
	//step4：组装交易发送给冷签名端
	toAmountInt64 = fromAmountInt64.Sub(decimal.New(feeTmp, 0))
	txouts = append(txouts, transfer.BiwTxOutTpl{
		ToAddr:   toAddr,
...
//创建交易接口参数
func (srv *BiwRecycleService) walletServerCreate(orderReq *transfer.BiwTxTpl) (string, error) {
...
//手续费计算
func (srv *BiwRecycleService) getBiwFee(inNum, outNum int) (int64, error) {
