...
type BchaRecycleService struct {
	CoinName    string
	CoinDecimal int32
}

func NewBchaRecycleService() service.RecycleService {
	return &BchaRecycleService{CoinName: "bcha", CoinDecimal: 8}
}

//params model : 0小额合并 1大额合并
func (b *BchaRecycleService) RecycleCoin(reqHead *transfer.OrderRequestHead, toAddr string, feeFloat decimal.Decimal, model int) (msg string, err error) {
	var (
		addrAms         []*entity.FcAddressAmount
		scanNum         int
		bchaCfg         *conf.CoinServers                  //币种数据服务
		addrs           = make([]string, 0)                //utxo地址
		unspents        *transfer.BchaUnspents             //utxo
		feeTmp          int64                              //临时估算手续费
		fromAmountInt64 decimal.Decimal                    //from金额
		toAmountInt64   decimal.Decimal                    //to金额
		sortUtxoDesc    transfer.BchaUnspentDesc           //大额
		sortUtxoAsc     transfer.BchaUnspentAsc            //小额
	...
	bchaCfg = conf.Cfg.CoinServers[reqHead.CoinName]
	if bchaCfg == nil {
		return "", errors.New("配置文件缺少bcha coinservers设置")
	}
	...
	//手续计算
	feeTmp, err = b.getBchaFee(len(txins), 1)
	//step4：组装交易发送给签名端
...
//创建交易接口参数
func (srv *BchaRecycleService) walletServerCreate(orderReq *transfer.BchaTxTpl) (string, error) {
...
//手续费计算
func (srv *BchaRecycleService) getBchaFee(inNum, outNum int) (int64, error) {
