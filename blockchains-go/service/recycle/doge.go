...
type DogeRecycleService struct {
	CoinName    string
	CoinDecimal int32
}

func NewDogeRecycleService() service.RecycleService {
	return &DogeRecycleService{CoinName: "doge", CoinDecimal: 8}
}

//params model : 0小额合并 1大额合并
func (b *DogeRecycleService) RecycleCoin(reqHead *transfer.OrderRequestHead, toAddr string, feeFloat decimal.Decimal, model int) (msg string, err error) {
	var (
		addrAms         []*entity.FcAddressAmount
		scanNum         int
		dogeCfg         *conf.CoinServers                  //币种数据服务
		addrs           = make([]string, 0)                //utxo地址
		unspents        *transfer.DogeUnspents             //utxo
		feeTmp          int64                              //临时估算手续费
		fromAmountInt64 decimal.Decimal                    //from金额
		toAmountInt64   decimal.Decimal                    //to金额
		sortUtxoDesc    transfer.DogeUnspentDesc           //大额
		sortUtxoAsc     transfer.DogeUnspentAsc            //小额
		txins           = make([]transfer.DogeTxInTpl, 0)  //utxo模板
		txouts          = make([]transfer.DogeTxOutTpl, 0) //utxo模板
		tpl             *transfer.DogeTxTpl                //模板
	)
	...
	//手续计算
	feeTmp, err = b.getDogeFee(len(txins), 1)
	//step4：组装交易发送给冷签名端
	toAmountInt64 = fromAmountInt64.Sub(decimal.New(feeTmp, 0))
	txouts = append(txouts, transfer.DogeTxOutTpl{
		ToAddr:   toAddr,
...
//创建交易接口参数
func (srv *DogeRecycleService) walletServerCreate(orderReq *transfer.DogeTxTpl) (string, error) {
...
//手续费计算
func (srv *DogeRecycleService) getDogeFee(inNum, outNum int) (int64, error) {
