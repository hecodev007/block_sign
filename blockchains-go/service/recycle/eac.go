...
type EacRecycleService struct {
	CoinName    string
	CoinDecimal int32
}

func NewEacRecycleService() service.RecycleService {
	return &EacRecycleService{CoinName: "eaccoin", CoinDecimal: 8}
}

//params model : 0小额合并 1大额合并
func (b *EacRecycleService) RecycleCoin(reqHead *transfer.OrderRequestHead, toAddr string, feeFloat decimal.Decimal, model int) (msg string, err error) {
	var (
		addrAms         []*entity.FcAddressAmount
		scanNum         int
		eaccoinCfg      *conf.CoinServers                 //币种数据服务
		addrs           = make([]string, 0)               //utxo地址
		unspents        *transfer.EacUnspents             //utxo
		feeTmp          int64                             //临时估算手续费
		fromAmountInt64 decimal.Decimal                   //from金额
		toAmountInt64   decimal.Decimal                   //to金额
		sortUtxoDesc    transfer.EacUnspentDesc           //大额
		sortUtxoAsc     transfer.EacUnspentAsc            //小额
		txins           = make([]transfer.EacTxInTpl, 0)  //utxo模板
		txouts          = make([]transfer.EacTxOutTpl, 0) //utxo模板
		tpl             *transfer.EacTxTpl                //模板
	)
	...
	eaccoinCfg = conf.Cfg.CoinServers[reqHead.CoinName]
	if eaccoinCfg == nil {
		return "", errors.New("配置文件缺少eaccoin coinservers设置")
	}
	log.Infof("发送地址：%s", eaccoinCfg.Url+"/v1/eac/unspents")
	byteData, err := util.PostJson(eaccoinCfg.Url+"/v1/eac/unspents", addrs)
	if err != nil {
	...
	if unspents.Code != 0 || len(unspents.Data) == 0 {
		return "", errors.New("eaccoin empty unspents")
	}
	...
	//手续计算
	feeTmp, err = b.getEacFee(len(txins), 1)
	//step4：组装交易发送给冷签名端
	toAmountInt64 = fromAmountInt64.Sub(decimal.New(feeTmp, 0))
	txouts = append(txouts, transfer.EacTxOutTpl{
		ToAddr:   toAddr,
...
//创建交易接口参数
func (srv *EacRecycleService) walletServerCreate(orderReq *transfer.EacTxTpl) (string, error) {
	log.Infof("eaccoin 发送url：%s", conf.Cfg.HotServers[srv.CoinName].Url+"/v1/"+strings.ToLower(srv.CoinName)+"/transfer")
	log.Infof("eaccoin 发送结构：%+v", orderReq)
	...
	log.Infof("eaccoin 发送返回：%s", string(data))
...
//手续费计算
func (srv *EacRecycleService) getEacFee(inNum, outNum int) (int64, error) {
	var (
		rate int64 = 1000
	)
