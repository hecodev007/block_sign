...
func Transfer(ctx *gin.Context) {
	clientId := ctx.PostForm("clientId")
	...
	if err != nil {
		...
		if applyId == httpresp.MchAmountNotEnough {
			httpresp.HttpRespCodeError(ctx, httpresp.MchAmountNotEnough, httpresp.GetMsg(httpresp.MchAmountNotEnough), nil)
		} else {
			httpresp.HttpRespCodeError(ctx, httpresp.FAIL, httpresp.GetMsg(httpresp.FAIL), nil)
		}
	...
	if err != nil {
		...
		if dtaErr := dao.DeleteTransferApplyById(applyId); dtaErr != nil {
			log.Errorf("执行删除DeleteTransferApplyById 失败:%v", dtaErr)
		}
		if dtfacErr := dao.DeleteTransfersApplyCoinAddressByApplyId(applyId); dtfacErr != nil {
			log.Errorf("执行删除DeleteTransfersApplyCoinAddressByApplyId 失败:%v", dtfacErr)
		}
		httpresp.HttpRespCodeError(ctx, httpresp.FAIL, httpresp.GetMsg(httpresp.FAIL), nil)
		return
	}
