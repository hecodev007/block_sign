...
func Transfer(ctx *gin.Context) {
	...
	walletId := ctx.PostForm("walletId") // 交易所钱包ID，每个用户对应一个，用于KYT
	coinName = strings.ToLower(coinName)
	amount, _ := decimal.NewFromString(ctx.PostForm("amount"))
	toAddress := ctx.PostForm("toAddress")
	tokenName := strings.ToLower(ctx.PostForm("tokenName"))
	contractAddress := strings.ToLower(ctx.PostForm("contractAddress"))
	banFromAddress := strings.ToLower(ctx.PostForm("banFromAddress"))
	memo := ctx.PostForm("memo")
	fee, _ := decimal.NewFromString(ctx.PostForm("fee"))
	isCustodyStr := ctx.PostForm("isCustody")
	isCustody := 0 // 值整数类型 0 非商户 1商户
	if isCustodyStr != "" {
		isCustody, _ = strconv.Atoi(isCustodyStr)
	}
	...
	if err != nil {
		...
		if orderId == httpresp.MchAmountNotEnough {
			httpresp.HttpRespCodeError(ctx, httpresp.MchAmountNotEnough, httpresp.GetMsg(httpresp.MchAmountNotEnough), nil)
		} else {
			httpresp.HttpRespCodeError(ctx, httpresp.FAIL, httpresp.GetMsg(httpresp.FAIL), nil)
		}
	...
	log.Infof("订单 %s 接收完成", outOrderId)

	// KYT风险检测
	go func() {
		rating, kytMsg, err := checkRiskFromKYT(outOrderId, walletId, params.CoinName, params.ToAddress)
		log.Errorf("checkRiskFromKYT风险等级结果: %s %s %v", rating, kytMsg, err)
	}()
