...
//存入appid值 币种名
const interceptKeySpec2 = "transfer_%d_%s"

//轮询数据库数据 进行交易
// Job Specific Functions
type TransferApplyHSCJob struct {
}
...
func (e TransferApplyHSCJob) Run() {
	var (
	...
	results, err = dao.FcTransfersApplyGroupValidOrderHSC(60)
	if err != nil {
		log.Errorf("HSC查询订单数据异常:%s", err.Error())
		return
	}
	if len(results) > 0 {
		log.Infof("=======%sHSC处理出账订单=======", nowTime)
		log.Infof("=======HSC订单数量:%d=======", len(results))
	}
	//数据交易
	for _, v := range results {
		...
		err = tmpMethod(v, redisHelper)
		if err != nil {
			log.Errorf("%s TransferApplyHSCJob error,[%s]", v.CoinName, v.OutOrderid)
		}
	}
}

func tmpMethod(v *entity.FcTransfersApply, redisHelper *util.RedisClient) error {
	...
	//
	//if has, _ := redisHelper.Exists(interceptKey); has {
	//	redisOutorderNo, _ := redisHelper.Get(interceptKey)
	//	log.Infof("存在正在交易中的：%s,订单号：%s，暂时跳过此订单：%s,等待下一次执行", v.CoinName, redisOutorderNo, v.OutOrderid)
	//	continue
	//} else {
	//	redisHelper.SetNx(interceptKey, v.OutOrderid)
...
func transferApplyHSC(v *entity.FcTransfersApply, wg *sync.WaitGroup) {
	//defer wg.Done()

	log.Infof("%s transferApplyHSC-001，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
	var (
		coinName string //币种名
		has      bool   //是否存在
		err      error
	...
	//查询是否存在相关交易的币种
	log.Infof("%s transferApplyHSC-002，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
	...
	log.Infof("%s transferApplyHSC-003，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
	...
	//判断提取数据的状态
	log.Infof("%s transferApplyHSC-004，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
	...
	log.Infof("%s transferApplyHSC-005，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
	...
	// 验证订单
	log.Infof("%s transferApplyHSC-006，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
	err = validTransferApplyHSC(v.Id)
	if err != nil {
	...
	log.Infof("%s transferApplyHSC-007，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
	if status.WalletType_Cold == walletType {
		...
		log.Infof("%s transferApplyHSC-008，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
		...
		if txid != "" {
			...
			log.Infof("%s transferApplyHSC-009，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
			log.Info("交易成功，回调给商户")
			...
			log.Infof("%s transferApplyHSC-010，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
			...
			//  完成订单后立马删除验证订单
			log.Infof("%s transferApplyHSC-011，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
			...
			log.Infof("%s transferApplyHSC-012，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
			errstr := ""
			if err != nil {
			...
			//txid为空，但是err没有异常
			log.Infof("%s transferApplyHSC-013，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
			err2 := orderService.SendApplyFail(v.Id)
			if err2 != nil {
		...
		log.Infof("%s transferApplyHSC-014，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
	...
	log.Infof("%s transferApplyHSC-015，订单：%s,时间：%s", v.CoinName, v.OutOrderid, util.GetChinaTimeNowFormat())
}

/*
验证apply表的数据
*/
func validTransferApplyHSC(applyId int) error {
