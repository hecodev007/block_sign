...
type CollectAtpJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectAtpJob(cfg conf.Collect2) cron.Job {
	return CollectAtpJob{
		coinName: "atp",
		cfg:      cfg,
	}
}

func (c CollectAtpJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectAtpJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	AtpCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(AtpCoins) == 0 {
		return errors.New("do not find atp coin")
	}
	//只有一个coin
	coin := AtpCoins[0]
	//获取有余额的地址
	...
	for _, from := range fromAddrs {
		...
		if err == nil {
			orderReq := &transfer.AtpOrderRequest{}
...
func (c *CollectAtpJob) walletServerCreateHot(orderReq *transfer.AtpOrderRequest) (string, error) {
