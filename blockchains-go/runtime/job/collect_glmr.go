...
type CollectGlmrJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectGlmrJob(cfg conf.Collect2) cron.Job {
	return CollectGlmrJob{
		coinName: "glmr",
		cfg:      cfg,
	}
}

func (c CollectGlmrJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectGlmrJob) collect(mchId int, mchName string) error {
	// //获取归集的目标冷地址
	...
	// }
	// 获取币种的配置
	glmrCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(glmrCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					// 填充参数
					orderReq := &transfer.GlmrOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(glmrCoins[0].Decimal)).String()

					// 发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  glmrCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(glmrCoins[0].Decimal)).IntPart(), // 转换整型
						Quantity:     feeAmount.Shift(int32(glmrCoins[0].Decimal)).String(),
						Decimal:      int64(glmrCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectGlmrJob) walletServerCreateHot(orderReq *transfer.GlmrOrderRequest) (string, error) {
	...
	result := transfer.DecodeGlmrTransferResp(data)
	if result == nil {
...
func (c *CollectGlmrJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectGlmrJob) inArray(coin string) bool {
...
func (c *CollectGlmrJob) getChainBalance(address, contract string) (string, error) {
