...
type CollectMaticJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectMaticJob(cfg conf.Collect2) cron.Job {
	return CollectMaticJob{
		coinName: "matic-matic",
		cfg:      cfg,
	}
}

func (c CollectMaticJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectMaticJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	maticCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(maticCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					//填充参数
					orderReq := &transfer.MaticOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(maticCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  maticCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(maticCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(maticCoins[0].Decimal)).String(),
						Decimal:      int64(maticCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectMaticJob) walletServerCreateHot(orderReq *transfer.MaticOrderRequest) (string, error) {
	...
	result := transfer.DecodeMaticTransferResp(data)
	if result == nil {
	...
	// TODO 使用公共节点，不能太频繁访问（签名-》广播）
	// 等到切换回我们自己的节点后可以不需要sleep
	time.Sleep(3 * time.Second)

	return result["data"].(string), nil
}

func (c *CollectMaticJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectMaticJob) inArray(coin string) bool {
...
func (c *CollectMaticJob) getChainBalance(address, contract string) (string, error) {

	var req transfer.ReqGetBalanceParams
	req.Address = address
	req.CoinName = "matic-matic"
