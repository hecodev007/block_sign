...
type CollectFilJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectFilJob(cfg conf.Collect2) cron.Job {
	return CollectFilJob{
		coinName: "fil",
		cfg:      cfg,
	}
}

func (c CollectFilJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stFilt).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectFilJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	FilCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(FilCoins) == 0 {
		return errors.New("do not find fil coin")
	}
	//只有一个coin
	coin := FilCoins[0]
	//获取有余额的地址
...
func (c *CollectFilJob) walletServerCreateHot(orderReq *transfer.FilOrderRequest) (string, error) {
