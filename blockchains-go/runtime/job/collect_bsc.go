...
func (c CollectBscJob) isHighGas(address string) bool {
	for _, addr := range c.cfg.HighGasAddress {
		if strings.ToLower(addr) == strings.ToLower(address) {
			return true
		}
	}
	return false
...
func (c CollectBscJob) Run() {
	// todo 定时任务bug：逻辑没处理完成，如果时间到下一轮周期会重复执行
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectBscJob) collect(mchId int, mchName string) error {
	...
	// }
	// 获取币种的配置
	bscCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(bscCoins) == 0 {
			...
			for _, fromAddr := range fromAddrs {
				...
				if isNeed {
					if f, ok := pendingFeeTx[fromAddr.Address]; !ok {
						if c.isHighGas(coin.Token) {
							highFee := fee.Mul(decimal.NewFromInt(2))
							pendingFeeTx[fromAddr.Address] = highFee
							log.Infof("高手续费地址 %s 使用双倍手续费 %s", coin.Token, highFee.String())
						} else {
	...
	if len(pendingFeeTx) > 0 {
		...
		log.Infof("BSC-BNB需要打手续费的笔数为：%d", totalNum)
		// 查找手续费地址
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(bscCoins[0].Decimal)).String()

					// 发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  bscCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(bscCoins[0].Decimal)).IntPart(), // 转换整型
						Quantity:     feeAmount.Shift(int32(bscCoins[0].Decimal)).String(),
						Decimal:      int64(bscCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
			...
			return fmt.Errorf("BSC-BNB 打手续费失败，查询fee address error,Err=%v", err)
		}
		log.Infof("BSC-BNB打手续费成功，可以打手续费笔数：%d，总共需要打手续费笔数：%d，锁定笔数：%d，成功笔数：%d，失败笔数：%d",
			useNum, totalNum, lockNum, successNum, failNum)
	}
	return nil
}

func (c *CollectBscJob) walletServerCreateHot(orderReq *transfer.BscOrderRequest) (string, error) {
...
func (c *CollectBscJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {

	chainAmountStr, err := c.getChainBalance(address, "")
	if err != nil {
		log.Errorf("get fee address balance error,err=%v", err)
		return false, decimal.NewFromFloat(0)
	}

	chainAm, err := decimal.NewFromString(chainAmountStr)
	if err != nil {
		log.Errorf("change fee address balance error,err=%v", err)
		return false, decimal.NewFromFloat(0)
	}
	chainAmFloat := chainAm.Shift(-18)
	minAm := decimal.NewFromFloat(minAmount)
	log.Infof("address[%s],chain fee[%s],min fee[%s]", address, chainAmFloat.String(), minAm.String())
	if chainAmFloat.LessThan(minAm) {
		return true, chainAmFloat.Sub(minAm)
	}

	// // 获取 余额是否足够一笔归集
	// fromAddr, err := entity.FcAddressAmount{}.FindAddressAndAmount(
	//	builder.Expr("type =? and address=? and coin_type=?", 2, address, c.coinName), 1)
	// if err != nil {
	//	log.Errorf("get fee address balance error,err=%v", err)
	// }
	// if len(fromAddr) > 0 {
	//	fromAm, err := decimal.NewFromString(fromAddr[0].Amount)
	//	if err != nil {
	//		log.Errorf("change from  address fee balance error,err=%v", err)
	//	} else {
	//		if !chainAmFloat.Equals(fromAm) {
	//			//不等于手续费，修正他
	//			err = dao.FcAddressAmountUpdateAmount(c.coinName, address, chainAmFloat)
	//			if err != nil {
	//				log.Info()
	//			}
	//		}
...
func (c *CollectBscJob) inArray(coin string) bool {
...
func (c *CollectBscJob) getChainBalance(address, contract string) (string, error) {
