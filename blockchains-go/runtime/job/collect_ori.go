...
type CollectOriJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectOriJob(cfg conf.Collect2) cron.Job {
	return CollectOriJob{
		coinName: "ori",
		cfg:      cfg,
	}
}

func (c CollectOriJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stOrit).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectOriJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	OriCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(OriCoins) == 0 {
		return errors.New("do not find ori coin")
	}
	//只有一个coin
	coin := OriCoins[0]
	//获取有余额的地址
	...
	if mchId == 1 {
		if toAddrs[0] != "5FA7zqEdBqYLcfiw2xMezoMbmBRXYbMHRp8jrjK3xjXZ5pyU" {
...
func (c *CollectOriJob) walletServerCreateHot(orderReq *transfer.OriOrderRequest) (string, error) {
