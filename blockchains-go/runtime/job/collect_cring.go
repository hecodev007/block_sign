...
type CollectCringJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCringJob(cfg conf.Collect2) cron.Job {
	return CollectCringJob{
		coinName: "crab",
		cfg:      cfg,
	}
}

func (c CollectCringJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	log.Infof("*** %s collect task stCringt***", c.coinName)
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stCringt).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectCringJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	CringCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(CringCoins) == 0 {
		return errors.New("do not find crab coin")
	}
	//只有一个coin
	coin := CringCoins[0]
	//获取有余额的地址
...
func (c *CollectCringJob) walletServerCreateHot(orderReq *transfer.CringOrderRequest) (string, error) {
