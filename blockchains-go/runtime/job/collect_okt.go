...
type CollectOktJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectOktJob(cfg conf.Collect2) cron.Job {
	return CollectOktJob{
		coinName: "okt",
		cfg:      cfg,
	}
}

func (c CollectOktJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectOktJob) collect(mchId int, mchName string) error {
	// //获取归集的目标冷地址
	...
	// }
	// 获取币种的配置
	oktCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(oktCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					// 填充参数
					orderReq := &transfer.OktOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(oktCoins[0].Decimal)).String()

					// 发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  oktCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(oktCoins[0].Decimal)).IntPart(), // 转换整型
						Quantity:     feeAmount.Shift(int32(oktCoins[0].Decimal)).String(),
						Decimal:      int64(oktCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectOktJob) walletServerCreateHot(orderReq *transfer.OktOrderRequest) (string, error) {
	...
	result := transfer.DecodeOktTransferResp(data)
	if result == nil {
...
func (c *CollectOktJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectOktJob) inArray(coin string) bool {
...
func (c *CollectOktJob) getChainBalance(address, contract string) (string, error) {
