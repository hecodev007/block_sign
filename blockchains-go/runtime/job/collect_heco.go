...
func (c *CollectHecoJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	hecoCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(hecoCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if c.inArray(coin.Name) {
			...
			to = toAddrs[0]
			if mchId == 1 && toAddrs[0] != "0x987a926d80feaa37d2d97f1e315589e4b91df139" {
				log.Infof("hoo 归集地址异常,归集冷地址为=【%s】冷地址不等于：0x987a926d80feaa37d2d97f1e315589e4b91df139", to)
				continue
			}
		}
		// 3. 设置最小的归集金额
		...
		if strings.ToLower(coin.Name) != c.coinName {
			for _, fromAddr := range fromAddrs {
				isNeed, err := c.needTransferFee(fromAddr.Address, c.cfg.NeedFee)
				if err != nil {
					log.Errorf("查找是否需要手续费失败： %v", err)
					continue
				}
				if isNeed {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if chainAmount.LessThan(dbAmount) {
					log.Errorf("addrs[%s],数据库金额[%s]额大于链上金额[%s]，不进行归集", from.Address, dbAmount, chainAmount)
					failNum++
					continue
				}
				//生产归集订单
				...
				if err == nil {
					//填充参数
					orderReq := &transfer.HecoOrderRequest{}
					...
					completeAddress[orderReq.FromAddress] = true
					orderHot.TxId = txid
	...
	if len(pendingFeeTx) > 0 {
		...
		feeCompleteAddress := make(map[string]bool)
		totalNum = len(pendingFeeTx)
		log.Infof("%s需要打手续费的笔数为：%d", strings.ToUpper(c.coinName), totalNum)
		//查找手续费地址
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(hecoCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  hecoCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(hecoCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(hecoCoins[0].Decimal)).String(),
						Decimal:      int64(hecoCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
			...
			return fmt.Errorf("%s 打手续费失败，查询fee address error,Err=%v", strings.ToUpper(c.coinName), err)
		}
		log.Infof("%s打手续费成功，可以打手续费笔数：%d，总共需要打手续费笔数：%d，锁定笔数：%d，成功笔数：%d，失败笔数：%d",
			strings.ToUpper(c.coinName),
			useNum, totalNum, lockNum, successNum, failNum)
	}
	return nil
}

func (c *CollectHecoJob) walletServerCreateHot(orderReq *transfer.HecoOrderRequest) (string, error) {
	//添加参数验证
	...
	if err != nil {
		return "", fmt.Errorf("创建transfer参数签名错误： %v", err)
	}
	orderReq.CurrentTime = timeNow
	orderReq.Sign = sig
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/heco/transfer", c.cfg.Url), c.cfg.User, c.cfg.Password, orderReq)
	if err != nil {
	...
	result := transfer.DecodeHecoTransferResp(data)
	if result == nil {
...
func (c *CollectHecoJob) needTransferFee(address string, minAmount float64) (bool, error) {
	// 从链上获取
	reqBalance := make(map[string]interface{})
	reqBalance["address"] = address
	reqBalance["coin_name"] = "heco"
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/heco/getBalance", c.cfg.Url), c.cfg.User, c.cfg.Password, reqBalance)
	if err != nil {
		return false, fmt.Errorf("rpc get balance error: %v", err)
	}
	dgbr, err := transfer.DecodeGetBalanceResp(data)
	if err != nil {
		return false, err
	}
	amountStr, ok := dgbr.Data.(string)
	if !ok {
		return false, fmt.Errorf("amount is not string: %v", dgbr.Data)
	}
	amount, _ := decimal.NewFromString(amountStr)
	amount = amount.Shift(-18)
	if amount.LessThan(decimal.NewFromFloat(minAmount)) {
		return true, nil
	}
	return false, nil
...
func (c *CollectHecoJob) inArray(coin string) bool {
...
func (c *CollectHecoJob) getChainBalance(address, contract string) (string, error) {

	var req transfer.ReqGetBalanceParams
	req.Address = address
	req.CoinName = "heco"
	req.ContractAddress = contract
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/heco/getBalance", c.cfg.Url), c.cfg.User, c.cfg.Password, req)
	if err != nil {
		return "", err
	}
