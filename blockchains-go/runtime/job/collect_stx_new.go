...
type CollectStxNewJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectStxNewJob(cfg conf.Collect2) cron.Job {
	return CollectStxNewJob{
		coinName: "stx",
		cfg:      cfg,
	}
}

func (c CollectStxNewJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectStxNewJob) inArray(coin string) bool {
...
func (c *CollectStxNewJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	stxCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(stxCoins) == 0 {
	...
	coins = append(coins, stxCoins...)
	completeAddress := make(map[string]bool)
	sendFeeSuccess := make(map[string]bool)
	...
	for _, coin := range coins {
		...
		if strings.ToLower(coin.Name) != c.coinName {
			for _, fromAddr := range fromAddrs {
				...
				_, sendOk := sendFeeSuccess[fromAddr.Address]
				if isNeed && !sendOk {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				dbAmount := amount
				chainAmountStr, err := c.getChainBalance(from.Address, coin.Name)
				if err != nil {
				...
				if err == nil {
					...
					successNum++
					delete(sendFeeSuccess, orderReq.FromAddress)
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Value = feeAmount.String()
					orderReq.Fee = "0"
					...
					orderHot := &entity.FcOrderHot{
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.IntPart(), //转换整型
						Quantity:     feeAmount.String(),
						Decimal:      int64(stxCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
					...
					orderHot.Status = int(status.BroadcastStatus)
					successNum++
					sendFeeSuccess[orderReq.ToAddress] = true // 手续费发送成功记录
...
func (c *CollectStxNewJob) walletServerCreateHot(orderReq *transfer.StxNewOrderRequest) (string, error) {
...
func (c *CollectStxNewJob) getChainBalance(address, tokenName string) (string, error) {
...
func (c *CollectStxNewJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
