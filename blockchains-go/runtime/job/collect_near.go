...
type CollectNearJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectNearJob(cfg conf.Collect2) cron.Job {
	return CollectNearJob{
		coinName: "near",
		cfg:      cfg,
	}
}

func (c CollectNearJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	log.Infof("*** %s collect task stNeart***", c.coinName)
...
func (c *CollectNearJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	NearCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(NearCoins) == 0 {
...
func (c *CollectNearJob) walletServerCreateHot(orderReq *transfer.NearOrderRequest) (string, error) {
