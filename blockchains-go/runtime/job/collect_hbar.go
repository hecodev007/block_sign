...
type CollectHbarJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectHbarJob(cfg conf.Collect2) cron.Job {
	return CollectHbarJob{
		coinName: "hbar",
		cfg:      cfg,
	}
}

func (c CollectHbarJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectHbarJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	hbarCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(hbarCoins) == 0 {
	...
	if err != nil {
		log.Infof("fc_coin_set获取币种配置失败，status：1, pid:%v, err:%v\n", pid, err)
		return err
	}
	coins = append(coins, hbarCoins...)
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					//填充参数
					orderReq := &transfer.HbarOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(hbarCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  hbarCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(hbarCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(hbarCoins[0].Decimal)).String(),
						Decimal:      int64(hbarCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectHbarJob) walletServerCreateHot(orderReq *transfer.HbarOrderRequest) (string, error) {
	...
	result := transfer.DecodeHbarTransferResp(data)
	if result == nil {
...
func (c *CollectHbarJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectHbarJob) inArray(coin string) bool {
...
func (c *CollectHbarJob) getChainBalance(address, contract string) (string, error) {
