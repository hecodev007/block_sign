...
type CollectWd_wdJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectWd_wdJob(cfg conf.Collect2) cron.Job {
	return CollectWd_wdJob{
		coinName: "wd",
		cfg:      cfg,
	}
}

func (c CollectWd_wdJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stWd_wdt).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectWd_wdJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	Wd_wdCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(Wd_wdCoins) == 0 {
		return errors.New("do not find wd-wd coin")
	}
	//只有一个coin
	coin := Wd_wdCoins[0]
	//获取有余额的地址
...
func (c *CollectWd_wdJob) walletServerCreateHot(orderReq *transfer.Wd_wdOrderRequest) (string, error) {
