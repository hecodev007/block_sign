...
type CollectWtcJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectWtcJob(cfg conf.Collect2) cron.Job {
	return CollectWtcJob{
		coinName: "wtc",
		cfg:      cfg,
	}
}

func (c CollectWtcJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectWtcJob) collect(mchId int, mchName string) error {
	// //获取归集的目标冷地址
	...
	// }
	// 获取币种的配置
	wtcCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(wtcCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					// 填充参数
					orderReq := &transfer.WtcOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Value = feeAmount.Shift(int32(wtcCoins[0].Decimal)).String()

					// 发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  wtcCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(wtcCoins[0].Decimal)).IntPart(), // 转换整型
						Quantity:     feeAmount.Shift(int32(wtcCoins[0].Decimal)).String(),
						Decimal:      int64(wtcCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectWtcJob) walletServerCreateHot(orderReq *transfer.WtcOrderRequest) (string, error) {
	...
	result := transfer.DecodeWtcTransferResp(data)
	if result == nil {
...
func (c *CollectWtcJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectWtcJob) inArray(coin string) bool {
...
func (c *CollectWtcJob) getChainBalance(address, contract string) (string, error) {
