...
type CollectHntJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectHntJob(cfg conf.Collect2) cron.Job {
	return CollectHntJob{
		coinName: "hnt",
		cfg:      cfg,
	}
}

func (c CollectHntJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectHntJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	HntCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(HntCoins) == 0 {
		return errors.New("do not find Hnt coin")
	}
	//只有一个coin
	coin := HntCoins[0]
	//获取有余额的地址
	...
	for _, from := range fromAddrs {
		...
		if err == nil {
			...
			txid, txFeeFloat, err := c.walletServerCreateHot(orderReq)
			if err != nil {
			...
			feeFlota, _ := decimal.NewFromString(txFeeFloat)
			orderHot.TxId = txid
			orderHot.Fee = feeFlota.Shift(8).IntPart()
			orderHot.Status = int(status.BroadcastStatus)
			log.Infof("归集成功，txid=%s，fee=%s", txid, feeFlota.String())
			//保存热表
...
func (c *CollectHntJob) walletServerCreateHot(orderReq *transfer.HntOrderRequest) (txid, feeFloat string, err error) {
	...
	txid = thr.Data.Txid
	feeFloat = thr.Data.FeeFloat
	return txid, feeFloat, nil
}
