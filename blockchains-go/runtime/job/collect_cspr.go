...
type CollectCsprJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCsprJob(cfg conf.Collect2) cron.Job {
	return CollectCsprJob{
		coinName: "cspr",
		cfg:      cfg,
	}
}

func (c CollectCsprJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectCsprJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	csprCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(csprCoins) == 0 {
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(csprCoins[0].Decimal)).IntPart()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  csprCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(csprCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(csprCoins[0].Decimal)).String(),
						Decimal:      int64(csprCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectCsprJob) walletServerCreateHot(orderReq *transfer.CsprOrderRequest) (string, error) {
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/%s/transfer", c.cfg.Url, c.coinName), c.cfg.User, c.cfg.Password, orderReq)
	if err != nil {
		return "", fmt.Errorf("%v collect fail,,from=[%v],to=[%v],amount=[%v],err=%v", orderReq.CoinName, orderReq.FromAddress,
...
func (c *CollectCsprJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectCsprJob) inArray(coin string) bool {
...
func (c *CollectCsprJob) getChainBalance(address, contract string) (string, error) {
