...
type CollectDomJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectDomJob(cfg conf.Collect2) cron.Job {
	return CollectDomJob{
		coinName: "dom",
		cfg:      cfg,
	}
}

func (c CollectDomJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectDomJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	DomCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(DomCoins) == 0 {
		return errors.New("do not find dom coin")
	}
	//只有一个coin
	coin := DomCoins[0]
	//获取有余额的地址
	...
	for _, from := range fromAddrs {
		...
		if err == nil {
			...
			orderReq.OrderID = cltApply.OutOrderid
			orderReq.CoinName = c.coinName

			orderReq.Data.From = from.Address
			orderReq.Data.To = to
			orderReq.Data.Amount = amount.Shift(int32(coin.Decimal)).IntPart()
			//发送交易
...
func (c *CollectDomJob) walletServerCreateHot(orderReq *transfer.DomOrderRequest) (string, error) {
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/%s/transfer", c.cfg.Url, c.coinName), c.cfg.User, c.cfg.Password, orderReq)
	if err != nil {
		return "", fmt.Errorf("%s collect fail,,from=[%s],to=[%s],amount=[%d],err=%v", orderReq.CoinName, orderReq.Data.From,
			orderReq.Data.To, orderReq.Data.Amount, err)
	}
