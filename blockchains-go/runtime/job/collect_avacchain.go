...
type CollectAvaxcchainJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectAvaxcchainJob(cfg conf.Collect2) cron.Job {
	return CollectAvaxcchainJob{
		coinName: "avaxcchain",
		cfg:      cfg,
	}
}

func (c CollectAvaxcchainJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectAvaxcchainJob) collect(mchId int, mchName string) error {
	// //获取归集的目标冷地址
	...
	// }
	// 获取币种的配置
	avaxcchainCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(avaxcchainCoins) == 0 {
		return fmt.Errorf("do not find %s coin", c.coinName)
	}

	pid := avaxcchainCoins[0].Id
	coins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"pid": pid}).And(builder.NotIn("name", c.cfg.IgnoreCoins)))
	if err != nil {
		return err
	}
	coins = append(coins, avaxcchainCoins...)
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					// 填充参数
					orderReq := &transfer.AvaxcchainOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(avaxcchainCoins[0].Decimal)).String()

					// 发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  avaxcchainCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(avaxcchainCoins[0].Decimal)).IntPart(), // 转换整型
						Quantity:     feeAmount.Shift(int32(avaxcchainCoins[0].Decimal)).String(),
						Decimal:      int64(avaxcchainCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectAvaxcchainJob) walletServerCreateHot(orderReq *transfer.AvaxcchainOrderRequest) (string, error) {
	...
	result := transfer.DecodeAvaxcchainTransferResp(data)
	if result == nil {
...
func (c *CollectAvaxcchainJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectAvaxcchainJob) inArray(coin string) bool {
...
func (c *CollectAvaxcchainJob) getChainBalance(address, contract string) (string, error) {

	var req transfer.ReqGetBalanceParams
	req.Address = address
	req.CoinName = "avaxcchain"
