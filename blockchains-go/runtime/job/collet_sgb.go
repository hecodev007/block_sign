...
type CollectSgbJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectSgbJob(cfg conf.Collect2) cron.Job {
	return CollectSgbJob{
		coinName: "sgb-sgb",
		cfg:      cfg,
	}
}

func (c CollectSgbJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stSgbt).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectSgbJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	SgbCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(SgbCoins) == 0 {
		return errors.New("do not find sgb coin")
	}
	//只有一个coin
	coin := SgbCoins[0]
	//获取有余额的地址
	...
	if mchId == 1 {
		for _, av := range toAddrs {
			if av != "3jwGMeSdfdyM9kyxBaXNuF4Rb9cx3Xf7ESQKsFbRJCADEdnk" {
...
func (c *CollectSgbJob) walletServerCreateHot(orderReq *transfer.SgbOrderRequest) (string, error) {
