...
type CollectNodleJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectNodleJob(cfg conf.Collect2) cron.Job {
	return CollectNodleJob{
		coinName: "nodle",
		cfg:      cfg,
	}
}

func (c CollectNodleJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stNodlet).Seconds())
	log.Info("开始归集")
	if len(c.cfg.Mchs) != 0 {
		mchs, err = entity.FcMch{}.Find(builder.In("platform", c.cfg.Mchs).And(builder.Eq{"status": 2}))
	} else {
		log.Infof("开始归集查询:%s", c.coinName)
...
func (c *CollectNodleJob) collect(mchId int, mchName string) error {
	log.Info("获取币种的配置，status：1")
	//获取币种的配置
	NodleCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(NodleCoins) == 0 {
		return errors.New("do not find nodle coin")
	}
	//只有一个coin
	coin := NodleCoins[0]
	//获取有余额的地址
	...
	if len(toAddrs) == 0 {
		log.Info("获取归集的目标冷地址，toAddrs==0")
		return fmt.Errorf("%s don't have cold address", mchName)
	}
	log.Infof("获取归集的目标冷地址，fee：%s", c.cfg.NeedFee)
	...
	for _, from := range fromAddrs {
		...
		log.Infof("生产归集订单，fee：%v", cltApply)
		...
		if err == nil {
			orderReq := &transfer.NodleOrderRequest{}
			...
			log.Infof("发送交易，fee：%v", createData)
...
func (c *CollectNodleJob) walletServerCreateHot(orderReq *transfer.NodleOrderRequest) (string, error) {
