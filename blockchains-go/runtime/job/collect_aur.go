...
type CollectAurJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectAurJob(cfg conf.Collect2) cron.Job {
	return CollectAurJob{
		coinName: "aur",
		cfg:      cfg,
	}
}

func (c CollectAurJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectAurJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	AurCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(AurCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					//填充参数
					orderReq := &transfer.AurOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(AurCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  AurCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(AurCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(AurCoins[0].Decimal)).String(),
						Decimal:      int64(AurCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectAurJob) walletServerCreateHot(orderReq *transfer.AurOrderRequest) (string, error) {
	...
	result := transfer.DecodeAurTransferResp(data)
	if result == nil {
...
func (c *CollectAurJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectAurJob) inArray(coin string) bool {
...
func (c *CollectAurJob) getChainBalance(address, contract string) (string, error) {
