...
type CollectStarJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectStarJob(cfg conf.Collect2) cron.Job {
	return CollectStarJob{
		coinName: "star",
		cfg:      cfg,
	}
}

func (c CollectStarJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stStart).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectStarJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	StarCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(StarCoins) == 0 {
		return errors.New("do not find star coin")
	}
	//只有一个coin
	coin := StarCoins[0]
	//获取有余额的地址
...
func (c *CollectStarJob) walletServerCreateHot(orderReq *transfer.StarOrderRequest) (string, error) {
