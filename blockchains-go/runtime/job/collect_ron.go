...
type CollectRonJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectRonJob(cfg conf.Collect2) cron.Job {
	return CollectRonJob{
		coinName: "ron",
		cfg:      cfg,
	}
}

func (c CollectRonJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectRonJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	ronCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(ronCoins) == 0 {
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(ronCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  ronCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(ronCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(ronCoins[0].Decimal)).String(),
						Decimal:      int64(ronCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectRonJob) walletServerCreateHot(orderReq *transfer.RonOrderRequest) (string, error) {
...
func (c *CollectRonJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectRonJob) inArray(coin string) bool {
...
func (c *CollectRonJob) getChainBalance(address, contract string) (string, error) {
