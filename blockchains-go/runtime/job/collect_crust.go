...
type CollectCrustJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCrustJob(cfg conf.Collect2) cron.Job {
	return CollectCrustJob{
		coinName: "crust",
		cfg:      cfg,
	}
}

func (c CollectCrustJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	log.Infof("*** %s collect task CRust***", c.coinName)
...
func (c *CollectCrustJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	CrustCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(CrustCoins) == 0 {
		return errors.New("do not find crust coin")
	}
	//只有一个coin
	coin := CrustCoins[0]
	//获取有余额的地址
...
func (c *CollectCrustJob) walletServerCreateHot(orderReq *transfer.CRustOrderRequest) (string, error) {
