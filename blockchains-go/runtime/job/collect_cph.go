...
type CollectCphJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCphJob(cfg conf.Collect2) cron.Job {
	return CollectCphJob{
		coinName: "cph-cph",
		cfg:      cfg,
	}
}

func (c CollectCphJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectCphJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	CphCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(CphCoins) == 0 {
		return errors.New("do not find cph-cph coin")
	}
	//只有一个coin
	coin := CphCoins[0]
	//获取有余额的地址
	...
	//log.Info(String(fromAddrs))
	total = len(fromAddrs)
	for _, from := range fromAddrs {
		//log.Info(String(from))
...
func (c *CollectCphJob) walletServerCreateHot(orderReq *transfer.CphOrderRequest) (string, error) {
