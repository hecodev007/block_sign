...
type CollectDotJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectDotJob(cfg conf.Collect2) cron.Job {
	return CollectDotJob{
		coinName: "dot",
		cfg:      cfg,
	}
}

func (c CollectDotJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stDott).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectDotJob) collect(mchId int, mchName string) error {
	//获取币种的配置
...
func (c *CollectDotJob) walletServerCreateHot(orderReq *transfer.DotOrderRequest) (string, error) {
