...
type CollectMwJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectMwJob(cfg conf.Collect2) cron.Job {
	return CollectMwJob{
		coinName: "mw",
		cfg:      cfg,
	}
...
func (c *CollectMwJob) collect(mchId int, mchName string) error {
	stMwt := time.Now()
	log.Infof("*** %s collect task start***", c.coinName)
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stMwt).Seconds())
	//获取币种的配置
	MwCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(MwCoins) == 0 {
	...
	if len(fromAddrs) == 0 {
		log.Infof("%s do not find any need collect from address", c.coinName)
		return nil
	...
	if mchId == 1 {
		if strings.ToUpper(toAddrs[0]) != "CDW-W5J4-C5JE-XPZS-69VCH" {
			log.Infof("%s error address", c.coinName)
			return nil
		}
	}
	...
	for _, from := range fromAddrs {
		...
		if err == nil {
			...
			orderReq.Fee = fee.Shift(int32(coin.Decimal)).String()
			//发送交易
	...
	log.Infof("归集完成，总共归集笔数：%d，成功笔数：%d，失败笔数：%d", total, success, fail)
	return nil
}

func (c *CollectMwJob) walletServerCreateHot(orderReq *transfer.MwOrderRequest) (string, error) {
