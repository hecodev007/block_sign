...
type CollectKaiJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectKaiJob(cfg conf.Collect2) cron.Job {
	return CollectKaiJob{
		coinName: "kai",
		cfg:      cfg,
	}
}

func (c CollectKaiJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectKaiJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	kaiCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(kaiCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					//填充参数
					orderReq := &transfer.KaiOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(kaiCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  kaiCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(kaiCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(kaiCoins[0].Decimal)).String(),
						Decimal:      int64(kaiCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectKaiJob) walletServerCreateHot(orderReq *transfer.KaiOrderRequest) (string, error) {
	...
	result := transfer.DecodeKaiTransferResp(data)
	if result == nil {
...
func (c *CollectKaiJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectKaiJob) inArray(coin string) bool {
...
func (c *CollectKaiJob) getChainBalance(address, contract string) (string, error) {
