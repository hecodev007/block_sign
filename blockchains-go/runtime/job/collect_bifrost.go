...
type CollectBncJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectBncJob(cfg conf.Collect2) cron.Job {
	return CollectBncJob{
		coinName: "bnc",
		cfg:      cfg,
	}
}

func (c CollectBncJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	log.Infof("*** %s collect task stBnct***", c.coinName)
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stBnct).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectBncJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	BncCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(BncCoins) == 0 {
		return errors.New("do not find ksm coin")
	}
	//只有一个coin
	coin := BncCoins[0]
	//获取有余额的地址
	...
	if mchId == 1 {
		for _, av := range toAddrs {
			if av != "d8HTtHNL6LBXkKRt2RBCmY2aUN2s1UK8NNEDgk6rncMKCxh" {
				if av != "bnFFXy4vQDsEkhqtm8guvDrhCm2Hou8hfJ1BNCzMxrJFNGz" {
					if av != "cBM28wCFQ1STQgLhNj1KmLwUmn2DK24BufEHTLV7Ep1SH3e" {
...
func (c *CollectBncJob) walletServerCreateHot(orderReq *transfer.BncOrderRequest) (string, error) {
