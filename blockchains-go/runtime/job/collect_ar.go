...
type CollectARJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectARJob(cfg conf.Collect2) cron.Job {
	return CollectARJob{
		coinName: "ar",
		cfg:      cfg,
	}
...
func (c *CollectARJob) collect(mchId int, mchName string) error {
	//获取归集的目标冷地址
	...
	//获取币种的配置
	ARCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(ARCoins) == 0 {
		return errors.New("do not find AR coin")
	}
...
func (c *CollectARJob) walletServerCreateHot(orderReq *transfer.AROrderRequest) (string, error) {
