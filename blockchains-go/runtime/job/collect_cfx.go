...
type CollectCfxJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCfxJob(cfg conf.Collect2) cron.Job {
	return CollectCfxJob{
		coinName: "cfx",
		cfg:      cfg,
	}
}

func (c CollectCfxJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	for _, tmp := range mchs {
		go func(mch *entity.FcMch) {
			wg.Add(1)
			defer wg.Done()
			if err := c.collect2(mch.Id, mch.Platform); err != nil {
...
func (c *CollectCfxJob) collect(mchId int, mchName string) error {
	//获取归集的目标冷地址
	...
	for _, from := range fromAddrs {
		...
		if err == nil {
			//填充参数
			orderReq := &transfer.CfxOrderRequest{}
...
func (c *CollectCfxJob) walletServerCreateHot(orderReq *transfer.CfxOrderRequest) (string, error) {
...
date： 2020-11-09
*/
func (c *CollectCfxJob) collect2(mchId int, mchName string) error {
	// 1. 先查找代代币的coin配置
	cfxCoin, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(cfxCoin) == 0 {
			...
			if err == nil && has {
				...
				for feeAddr, needFee := range pendingFeeTx {
					...
					if err == nil {
						...
						orderReq.Amount = needFee.Shift(int32(cfxCoin[0].Decimal)).String()

						//发送交易
						createData, _ := json.Marshal(orderReq)
						orderHot := &entity.FcOrderHot{
							ApplyId:      int(appId),
							ApplyCoinId:  cfxCoin[0].Id,
							...
							FromAddress:  orderReq.FromAddress,
							ToAddress:    orderReq.ToAddress,
							Amount:       needFee.Shift(int32(cfxCoin[0].Decimal)).IntPart(), //转换整型
							Quantity:     needFee.Shift(int32(cfxCoin[0].Decimal)).String(),
							Decimal:      int64(cfxCoin[0].Decimal),
							CreateData:   string(createData),
							Status:       int(status.UnknowErrorStatus),
							CreateAt:     time.Now().Unix(),
							UpdateAt:     time.Now().Unix(),
...
func (c *CollectCfxJob) needTransferFee(address string, minAmount float64) bool {
