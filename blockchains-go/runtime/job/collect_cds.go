...
type CollectCdsJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCdsJob(cfg conf.Collect2) cron.Job {
	return CollectCdsJob{
		coinName: "cds",
		cfg:      cfg,
	}
}

func (c CollectCdsJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectCdsJob) collect(mchId int, mchName string) error {
	// //获取归集的目标冷地址
	...
	// }
	// 获取币种的配置
	cdsCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(cdsCoins) == 0 {
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(cdsCoins[0].Decimal)).String()

					// 发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  cdsCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(cdsCoins[0].Decimal)).IntPart(), // 转换整型
						Quantity:     feeAmount.Shift(int32(cdsCoins[0].Decimal)).String(),
						Decimal:      int64(cdsCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectCdsJob) walletServerCreateHot(orderReq *transfer.CdsOrderRequest) (string, error) {
...
func (c *CollectCdsJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectCdsJob) inArray(coin string) bool {
...
func (c *CollectCdsJob) getChainBalance(address, contract string) (string, error) {
