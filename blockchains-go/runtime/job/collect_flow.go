...
type CollectFlowJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectFlowJob(cfg conf.Collect2) cron.Job {
	return CollectFlowJob{
		coinName: "flow",
		cfg:      cfg,
	}
}

func (c CollectFlowJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectFlowJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	flowCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		log.Print(err.Error())
		return err
	}
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					...
					//地址需要残留0.001个flow, 目前flow主链交易不需要手续费
					remain := decimal.NewFromFloat(0.001)
...
func (c *CollectFlowJob) walletServerCreateHot(orderReq *transfer.FlowOrderRequest) (string, error) {
...
func (c *CollectFlowJob) inArray(coin string) bool {
...
func (c *CollectFlowJob) getChainBalance(address, contract string) (string, error) {
