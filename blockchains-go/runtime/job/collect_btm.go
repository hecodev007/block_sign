...
type CollectBtmJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectBtmJob(cfg conf.Collect2) cron.Job {
	return CollectBtmJob{
		coinName: "btm",
		cfg:      cfg,
	}
}

func (c CollectBtmJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectBtmJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	btmCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(btmCoins) == 0 {
	...
	for _, coin := range coins {
		...
		fmt.Printf("toAddrs: %v\n", toAddrs)

		var to string
		var index int
		var tempAmount int64
		for i, addr := range toAddrs {
			chainAmountStr, err := c.getChainBalance(addr, coin.Token)
			if err != nil {
				continue
			}
			dBalance, err := decimal.NewFromString(chainAmountStr)
			if err != nil {
				continue
			}
			if dBalance.IntPart() < tempAmount {
				index = i
			}
			tempAmount = dBalance.IntPart()
		}
		to = toAddrs[index]
		//判断该币种是否需要归集到多个地址
		...
		log.Printf("fromAddrs: %v", fromAddrs)
		if err != nil {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				log.Infof("数据库金额: %s, 链上金额: %s", dbAmount.String(), chainAmountStr)
				if err != nil {
				...
				if !chainAmount.Equal(dbAmount) {
					log.Errorf("[%s]数据库金额[%s]与链上金额[%s]不符，不进行归集,币种[%s]", from.Address, dbAmount, chainAmount, coin.Name)
					failNum++
					continue
				}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(btmCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  btmCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(btmCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(btmCoins[0].Decimal)).String(),
						Decimal:      int64(btmCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectBtmJob) walletServerCreateHot(orderReq *transfer.BtmOrderRequest) (string, error) {
...
func (c *CollectBtmJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectBtmJob) inArray(coin string) bool {
...
func (c *CollectBtmJob) getChainBalance(address, contract string) (string, error) {
...
//	}
//	if len(fromAddrs) == 0 {
//		log.Infof("没有找到满足条件的归集地址，商户ID=[%s]", mchName)
//		return nil
...
//		//查找公钥
//		fromSig, err := entity.FcGenerateAddressList{}.Find(builder.Eq{
...
//			"address":     from.Address,
//		})
//		if err != nil || len(fromSig) != 1 {
//			return err
//		}
//		signPubkey := fromSig[0].CompatibleAddress
//		//生产归集订单
//		cltApply := &entity.FcTransfersApply{
//			Username:   "Robot",
//			Department: "blockchains-go",
...
//			orderReq.FromAddress = from.Address
//			orderReq.ToAddress = to
//			orderReq.ChangeAddress = to
//			orderReq.SignPubKey = signPubkey
//			orderReq.Amount = -8 //-8 表示归集
//			orderReq.Quantity = "-8"
...
//
////创建交易接口参数
//func (c *CollectBtmJob) walletServerCreate(orderReq *transfer.BtmOrderRequest) error {
//
//	data, err := util.PostJsonByAuth(c.cfg.Url+"/btm/create", c.cfg.User, c.cfg.Password, orderReq)
//	if err != nil {
//		return err
//	}
//	dd, _ := json.Marshal(orderReq)
