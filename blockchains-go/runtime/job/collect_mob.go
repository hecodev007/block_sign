...
type CollectMobJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectMobJob(cfg conf.Collect2) cron.Job {
	return CollectMobJob{
		coinName: "mob",
		cfg:      cfg,
	}
}

func (c CollectMobJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectMobJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	MobCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(MobCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				chainAmountStr, err := c.getChainBalance(mchName, coin.Name, from.Address)
				if err != nil {
				...
				if err == nil {
					//填充参数
					orderReq := &transfer.MobOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(MobCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  MobCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(MobCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(MobCoins[0].Decimal)).String(),
						Decimal:      int64(MobCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectMobJob) walletServerCreateHot(orderReq *transfer.MobOrderRequest) (string, error) {
	...
	result := transfer.DecodeMobTransferResp(data)
	if result == nil {
...
func (c *CollectMobJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectMobJob) inArray(coin string) bool {
...
func (c *CollectMobJob) getChainBalance(mchName, coinName, address string) (string, error) {
	log.Infof("mob查询余额的商户名:%s,币名%s,地址%s", mchName, coinName, address)
	log.Info(fmt.Sprintf("%s/v1/%s/balance", c.cfg.Url, c.coinName))
	params := make(map[string]interface{})
	params["coin_name"] = coinName
	params["mch_name"] = mchName
	params["address"] = address
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/%s/balance", c.cfg.Url, c.coinName), c.cfg.User, c.cfg.Password, params)
	if err != nil {
