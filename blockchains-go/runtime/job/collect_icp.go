...
type CollectIcpJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectIcpJob(cfg conf.Collect2) cron.Job {
	return CollectIcpJob{
		coinName: "icp",
		cfg:      cfg,
	}
}

func (c CollectIcpJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectIcpJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	icpCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		log.Print(err.Error())
		return err
	}
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if !chainAmount.Equal(dbAmount) {
					log.Errorf("[%s]数据库金额[%s]不等于链上金额[%s]，不进行归集,币种[%s]", from.Address, dbAmount, chainAmount, coin.Name)
					failNum++
					continue
				}
				//生产归集订单
				...
				if err == nil {
					//填充参数
					orderReq := &transfer.IcpOrderRequest{}
...
func (c *CollectIcpJob) walletServerCreateHot(orderReq *transfer.IcpOrderRequest) (string, error) {
	...
	result := transfer.DecodeIcpTransferResp(data)
	if result == nil {
...
func (c *CollectIcpJob) inArray(coin string) bool {
...
func (c *CollectIcpJob) getChainBalance(address, contract string) (string, error) {
