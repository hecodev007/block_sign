...
type CollectCdsOldJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCdsOldJob(cfg conf.Collect2) cron.Job {
	return CollectCdsOldJob{
		coinName: "cds-old",
		cfg:      cfg,
	}
}

func (c CollectCdsOldJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectCdsOldJob) collect(mchId int, mchName string) error {
	//获取归集的目标冷地址
...
func (c *CollectCdsOldJob) walletServerCreateHot(orderReq *transfer.CdsOrderRequest) (string, error) {
