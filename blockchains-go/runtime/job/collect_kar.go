...
type CollectKarJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectKarJob(cfg conf.Collect2) cron.Job {
	return CollectKarJob{
		coinName: "kar",
		cfg:      cfg,
	}
}

func (c CollectKarJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stKart).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectKarJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	KarCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(KarCoins) == 0 {
		return errors.New("do not find kar coin")
	}
	//只有一个coin
	coin := KarCoins[0]
	//获取有余额的地址
	...
	if mchId == 1 {
		for _, av := range toAddrs {
			if av != "qHyvEuvKyPFb438tNzQXKAyvGFbhwoGfbjq6CMZnjA4eaaN" {
...
func (c *CollectKarJob) walletServerCreateHot(orderReq *transfer.KarOrderRequest) (string, error) {
