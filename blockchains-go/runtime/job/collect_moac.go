...
type CollectMoacJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectMoacJob(cfg conf.Collect2) cron.Job {
	return CollectMoacJob{
		coinName: "moac",
		cfg:      cfg,
	}
}

func (c CollectMoacJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectMoacJob) collect(mchId int, mchName string) error {
	// //获取归集的目标冷地址
	...
	// }
	// 获取币种的配置
	moacCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(moacCoins) == 0 {
	...
	for _, coin := range coins {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					// 填充参数
					orderReq := &transfer.MoacOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Value = feeAmount.Shift(int32(moacCoins[0].Decimal)).String()

					// 发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  moacCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(moacCoins[0].Decimal)).IntPart(), // 转换整型
						Quantity:     feeAmount.Shift(int32(moacCoins[0].Decimal)).String(),
						Decimal:      int64(moacCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectMoacJob) walletServerCreateHot(orderReq *transfer.MoacOrderRequest) (string, error) {
	...
	result := transfer.DecodeMoacTransferResp(data)
	if result == nil {
		return "", fmt.Errorf("order表 请求下单接口失败，outOrderId：%s", orderReq.OuterOrderNo)
	}
	if result["code"].(float64) != 0 || result["txid"] == nil {
		log.Error(result)
...
func (c *CollectMoacJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectMoacJob) inArray(coin string) bool {
...
func (c *CollectMoacJob) getChainBalance(address, contract string) (string, error) {
