...
type CollectDhxJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectDhxJob(cfg conf.Collect2) cron.Job {
	return CollectDhxJob{
		coinName: "dhx",
		cfg:      cfg,
	}
}

func (c CollectDhxJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stDhxt).Seconds())
	//if global.G_TASK_MARK {
	//	log.Infof("*** %s 任务正在执行，等待下一轮", c.coinName)
	//}
	log.Infof("*** %s 任务执行", c.coinName)
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectDhxJob) collect(mchId int, mchName string) error {

	log.Info("1111111")
	//获取币种的配置
	DhxCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(DhxCoins) == 0 {
		return errors.New("do not find dhx coin")
	}
...
func (c *CollectDhxJob) walletServerCreateHot(orderReq *transfer.DhxOrderRequest) (string, error) {
