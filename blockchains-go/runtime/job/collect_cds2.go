...
var cdsFeeFloat = decimal.NewFromFloat(0.001)

type CollectCdsJob2 struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCdsJob2(cfg conf.Collect2) cron.Job {
	return CollectCdsJob2{
		coinName: "cds",
		cfg:      cfg,
	}
}

func (c CollectCdsJob2) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectCdsJob2) collect(mchId int, mchName string) error {
	//获取归集的目标冷地址
...
func (c *CollectCdsJob2) walletServerCreateHot(orderReq *transfer.CdsOrderRequest) (string, error) {
