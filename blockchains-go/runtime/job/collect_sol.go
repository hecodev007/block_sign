...
type CollectSolJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectSolJob(cfg conf.Collect2) cron.Job {
	return CollectSolJob{
		coinName: "sol",
		cfg:      cfg,
	}
}

func (c CollectSolJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectSolJob) collect(mchId int, mchName string) error {
	//获取归集的目标冷地址
	toAddrs := make([]string, 0)
	var err error
	if mchId == 1 {
		toAddrs = []string{"FgiJTETJS6LJ1mMiuXPxKFCjbGsxA56B9ke2egvPS12s"}
	} else {
	...
	for _, coin := range Coins {
		for _, from := range fromAddrs {
			...
			//扣除手续费
			amount = amount.Sub(decimal.NewFromFloat(c.cfg.NeedFee))
			...
			if err == nil {
				...
				orderReq.MchId = strconv.Itoa(mchId)
...
func (c *CollectSolJob) walletServerCreateHot(orderReq *transfer.SolOrderRequest) (string, error) {
