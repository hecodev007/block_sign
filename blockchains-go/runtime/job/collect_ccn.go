...
type CollectCcnJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectCcnJob(cfg conf.Collect2) cron.Job {
	return CollectCcnJob{
		coinName: "ccn",
		cfg:      cfg,
	}
}

func (c CollectCcnJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectCcnJob) collect(mchId int, mchName string) error {
	////获取归集的目标冷地址
	...
	//}
	//获取币种的配置
	ccnCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(ccnCoins) == 0 {
	...
	for _, coin := range coins {
		...
		//}
		if  len(toAddrs) <= 1 {
			log.Errorf("%s  have only one cold address, continue", mchName)
			continue
		}
		to = toAddrs[rand.Intn(len(toAddrs))]
		for to == "0x6183a35391b4ad16d244b2b307f63d713272b989" {
			to = toAddrs[rand.Intn(len(toAddrs))]
		}
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					//填充参数
					orderReq := &transfer.CcnOrderRequest{}
		...
		if err == nil && has {
			...
			for to, feeAmount := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Amount = feeAmount.Shift(int32(ccnCoins[0].Decimal)).String()

					//发送交易
					createData, _ := json.Marshal(orderReq)
					orderHot := &entity.FcOrderHot{
						ApplyId:      int(appId),
						ApplyCoinId:  ccnCoins[0].Id,
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       feeAmount.Shift(int32(ccnCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     feeAmount.Shift(int32(ccnCoins[0].Decimal)).String(),
						Decimal:      int64(ccnCoins[0].Decimal),
						CreateData:   string(createData),
						Status:       int(status.UnknowErrorStatus),
						CreateAt:     time.Now().Unix(),
						UpdateAt:     time.Now().Unix(),
...
func (c *CollectCcnJob) walletServerCreateHot(orderReq *transfer.CcnOrderRequest) (string, error) {
	...
	result := transfer.DecodeCcnTransferResp(data)
	if result == nil {
...
func (c *CollectCcnJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	// 获取 余额是否足够一笔归集
...
func (c *CollectCcnJob) inArray(coin string) bool {
...
func (c *CollectCcnJob) getChainBalance(address, contract string) (string, error) {
