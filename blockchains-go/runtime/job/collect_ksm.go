...
type CollectKsmJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectKsmJob(cfg conf.Collect2) cron.Job {
	return CollectKsmJob{
		coinName: "ksm",
		cfg:      cfg,
	}
}

func (c CollectKsmJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	log.Infof("*** %s collect task stKsmt***", c.coinName)
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stKsmt).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectKsmJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	KsmCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(KsmCoins) == 0 {
	...
	if mchId == 1 {
		for _, av := range toAddrs {
			if av != "DaqWX6Bt3Q2MUAZGm9DLi33njkTWKesHuas4AthrhkiTsn9" {
				if av != "DQ2tcM92MDZpViW9KPRQV9XGphvGQWnuQVUda6X8eRi7v86" {
					if av != "FX4cVfPy27JfVh7FrjVSaehbjL376NvsTWeHgxaUpMxhC19" {
...
func (c *CollectKsmJob) walletServerCreateHot(orderReq *transfer.KsmOrderRequest) (string, error) {
