...
type CollectFisJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectFisJob(cfg conf.Collect2) cron.Job {
	return CollectFisJob{
		coinName: "fis",
		cfg:      cfg,
	}
}

func (c CollectFisJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
	...
	defer log.Infof("*** %s collect task end, use time : %f s ", c.coinName, time.Since(stFist).Seconds())
	if len(c.cfg.Mchs) != 0 {
...
func (c *CollectFisJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	FisCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(FisCoins) == 0 {
	...
	if mchId == 1 {
		if toAddrs[0] != "32vxvxU5MVGUYg5s3ktaq3un8xapTQn2dsNXsJwhE8S6NrH2" {
			return fmt.Errorf("%s error addr", toAddrs[0])
		}
	}
...
func (c *CollectFisJob) walletServerCreateHot(orderReq *transfer.FisOrderRequest) (string, error) {
