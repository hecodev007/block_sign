...
type CollectAlgoJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectAlgoJob(cfg conf.Collect2) cron.Job {
	return CollectAlgoJob{
		coinName: "algo",
		cfg:      cfg,
	}
}
...
func (c *CollectAlgoJob) collect(mchId int, mchName string) error {
	...
	for _, coin := range coins {
		...
		var err error
		if mchId == 1 {
			toAddrs = []string{"YDRTADVLLGLU76ARI4NU5V4XTXPHDJII7A5MY3UL6II77LGE5SPQUKJWI4"}
		} else {
		...
		if len(collectAddrs) > 0 {
			...
			for _, from := range collectAddrs {
				...
				if err == nil {
					...
					orderReq.Fee = decimal.NewFromFloat(c.cfg.NeedFee).Shift(algoDecimal).String()
					//		判断是否是合约归集
					if strings.ToLower(coin.Name) != c.coinName {
						orderReq.Assert = coin.Token
						orderReq.Value = amount.Shift(int32(coin.Decimal)).String()
					} else {
						//扣除手续费
						fee := decimal.NewFromFloat(c.cfg.NeedFee)
						amount = amount.Sub(fee)
						amount = amount.Sub(decimal.NewFromFloat(0.3)) //需要残留0.1
						if amount.LessThanOrEqual(decimal.Zero) {
							failNum++
							log.Errorf("%s归集金额小于0", from.Address)
							continue
						}
						orderReq.Value = amount.Shift(int32(algoDecimal)).String()
						orderReq.Assert = "0"
					}
	...
	if len(pendingFeeTx) > 0 {
		...
		log.Infof("Algo需要打手续费的笔数为：%d", totalNum)
		//查找手续费地址
		...
		if err == nil && has {
			...
			for to := range pendingFeeTx {
				...
				if err == nil {
					...
					orderReq.ToAddress = to
					orderReq.Value = decimal.NewFromFloat(c.cfg.NeedFee).Shift(algoDecimal).String()
					orderReq.Fee = decimal.NewFromFloat(c.cfg.NeedFee).Shift(algoDecimal).String()
					orderReq.Assert = "0"
					...
					orderHot := &entity.FcOrderHot{
						...
						FromAddress:  orderReq.FromAddress,
						ToAddress:    orderReq.ToAddress,
						Amount:       decimal.NewFromFloat(c.cfg.NeedFee).Shift(int32(bscCoins[0].Decimal)).IntPart(), //转换整型
						Quantity:     decimal.NewFromFloat(c.cfg.NeedFee).Shift(int32(bscCoins[0].Decimal)).String(),
...
func (c *CollectAlgoJob) walletServerCreateHot(orderReq *transfer.AlgoOrderRequest) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%s collect fail,,from=[%s],to=[%s],amount=[%s],err=%v", orderReq.CoinName, orderReq.FromAddress,
			orderReq.ToAddress, orderReq.Value, err)
	}
...
func (c *CollectAlgoJob) needTransferFee(address string, minAmount float64) (bool, decimal.Decimal) {
	minAmount = minAmount + 0.1 //需要残留0.1个
	// 获取 余额是否足够一笔归集
