...
type CollectEtcJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectEtcJob(cfg conf.Collect2) cron.Job {
	return CollectEtcJob{
		coinName: "etc",
		cfg:      cfg,
	}
}

func (c CollectEtcJob) Run() {
	start := time.Now()
...
func (c *CollectEtcJob) collect(mchId int, mchName string) error {
	//获取归集的目标冷地址
	...
	if mchId == 1 && toAddrs[0] != "0xbaa5e1cdb7ecaad63aa7555d09ea0e8ec22b4935" {
...
//创建交易接口参数
func (c *CollectEtcJob) walletServerCreate(orderReq *transfer.EthOrderRequest) error {
