...
type CollectN3neoJob struct {
	coinName string
	cfg      conf.Collect2
}

func NewCollectN3neoJob(cfg conf.Collect2) cron.Job {
	return CollectN3neoJob{
		coinName: "neo",
		cfg:      cfg,
	}
}

func (c CollectN3neoJob) Run() {
	var (
		mchs []*entity.FcMch
		err  error
	)
...
func (c *CollectN3neoJob) collect(mchId int, mchName string) error {
	//获取币种的配置
	n3neoCoins, err := entity.FcCoinSet{}.Find(builder.Eq{"status": 1}.And(builder.Eq{"name": c.coinName}))
	if err != nil {
		return err
	}
	if len(n3neoCoins) == 0 {
...
func (c *CollectN3neoJob) walletServerCreateHot(orderReq *transfer.N3neoOrderRequest) (string, error) {
	data, err := util.PostJsonByAuth(fmt.Sprintf("%s/v1/%s/collectTransfer", c.cfg.Url, c.coinName), c.cfg.User, c.cfg.Password, orderReq)
	if err != nil {
...
func (c *CollectN3neoJob) inArray(coin string) bool {
...
func (c *CollectN3neoJob) getChainBalance(address, contract string) (string, error) {
