...
type FcTransactionsVet struct {
	...
	Amount              string    `json:"amount" xorm:"not null default 0.00000000000000000000 comment('交易量（转出为负数）') DECIMAL(50,20)"`
	Fee                 string    `json:"fee" xorm:"not null default 0.00000000000000000000 comment('手续费（负数）') DECIMAL(40,20)"`
	...
	BlockHash           string    `json:"block_hash" xorm:"not null default '' comment('区块hash') VARCHAR(150)"`
