...
type FcTxClearQtum struct {
	...
	TxId          string    `json:"tx_id" xorm:"not null comment('txid') unique(transaction_id) VARCHAR(100)"`
	TxInAmount    string    `json:"tx_in_amount" xorm:"not null default 0.000000000000000000 comment('交易from总额') DECIMAL(40,18)"`
	TxInNum       int       `json:"tx_in_num" xorm:"not null default 0 comment('交易from笔数') INT(11)"`
	TxOutAmount   string    `json:"tx_out_amount" xorm:"not null default 0.000000000000000000 comment('交易out总额') DECIMAL(40,18)"`
	TxOutNum      int       `json:"tx_out_num" xorm:"not null default 0 comment('交易to笔数') INT(11)"`
	TxFee         string    `json:"tx_fee" xorm:"not null default 0.000000000000000000 DECIMAL(40,18)"`
