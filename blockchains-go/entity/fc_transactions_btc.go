...
type FcTransactionsBtc struct {
	Id                  int       `json:"id" xorm:"not null pk autoincr INT(11)"`
	CoinId              int       `json:"coin_id" xorm:"not null comment('币种ID') INT(11)"`
	Address             string    `json:"address" xorm:"not null comment('地址') index unique(index_name) VARCHAR(100)"`
	...
	Txid                string    `json:"txid" xorm:"not null comment('交易ID(唯一键)') unique(index_name) VARCHAR(100)"`
	Flag                int       `json:"flag" xorm:"not null default 0 comment('1转入，2转出') unique(index_name) TINYINT(4)"`
	...
	AppId               int       `json:"app_id" xorm:"not null default 1 comment('商户ID') TINYINT(4)"`
	...
	PrevPosition        int       `json:"prev_position" xorm:"not null default 0 unique(index_name) INT(11)"`
}
