...
type FcFeeSupplier struct {
	Id              int       `json:"id" xorm:"not null pk autoincr INT(10)"`
	Platform        string    `json:"platform" xorm:"comment('商户名称') unique(coin_name) VARCHAR(20)"`
	AppId           int       `json:"app_id" xorm:"not null default 0 comment('商户ID') TINYINT(4)"`
	CoinName        string    `json:"coin_name" xorm:"not null comment('币种名称') unique(coin_name) VARCHAR(15)"`
	Address         string    `json:"address" xorm:"not null comment('转出手续费的地址') unique(coin_name) VARCHAR(190)"`
