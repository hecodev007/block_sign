...
type FcHooRtExchangeOrig struct {
	Id           int       `json:"id" xorm:"not null pk autoincr comment('自增ID') INT(11)"`
	HooId        int64     `json:"hoo_id" xorm:"not null comment('hoo那边的id') BIGINT(20)"`
	Channel      int       `json:"channel" xorm:"not null default 0 comment('兑换通道,（1火币，2OK等）') TINYINT(2)"`
