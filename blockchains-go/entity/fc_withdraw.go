...
type FcWithdraw struct {
	...
	Txid         string    `json:"txid" xorm:"not null default '0' VARCHAR(200)"`
	Address      string    `json:"address" xorm:"not null index VARCHAR(200)"`
	...
	MuAddressId  int       `json:"mu_address_id" xorm:"not null comment('后端转出地址ID') index INT(11)"`
	...
	CurConfirm   int       `json:"cur_confirm" xorm:"default 0 comment('当前确认数') TINYINT(3)"`
}
