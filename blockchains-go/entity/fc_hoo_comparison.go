...
type FcHooComparison struct {
	Id           int    `json:"id" xorm:"not null pk autoincr comment('è‡ªå¢žID') INT(11)"`
	TradeId      string `json:"trade_id" xorm:"not null comment('äº¤æ˜“IDï¼Œå”¯â¼€äº¤æ˜“ID') unique(tradeid_wallettype) VARCHAR(64)"`
	WalletType   string `json:"wallet_type" xorm:"not null default '' comment('é’±åŒ…ç±»åž‹ï¼Œä¸ªäººé’±åŒ…(wallet)ï¼Œ å…±ç®¡(multiwallet)') unique(tradeid_wallettype) VARCHAR(18)"`
	Status       int    `json:"status" xorm:"not null comment('äº¤æ˜“çŠ¶æ€ï¼Œ1ï¼ˆæˆåŠŸï¼‰ 2ï¼ˆå†»ç»“ï¼‰') TINYINT(2)"`
	SeqCode      string `json:"seq_code" xorm:"not null comment('范奈斯交易编号') index VARCHAR(50)"`
	...
	Address      string `json:"address" xorm:"not null default '' comment('交易地址，每笔交易对应的地址') index VARCHAR(255)"`
	...
	TradeTime    int    `json:"trade_time" xorm:"not null default 0 comment('äº¤æ˜“æ—¶é—´ï¼ˆæ—¶é—´æˆ³ï¼‰ï¼Œå•ä½ç§’') index INT(11)"`
	Memo         string `json:"memo" xorm:"not null default '' comment('EOSçš„ Memo') VARCHAR(100)"`
	Txid         string `json:"txid" xorm:"not null default '' comment('é“¾ä¸Šäº¤æ˜“ID') unique VARCHAR(80)"`
	...
	ComparisonStatus int       `json:"comparison_status" xorm:"not null default 0 comment('是否比对 1未比对 2数据错误') TINYINT(2)"`
