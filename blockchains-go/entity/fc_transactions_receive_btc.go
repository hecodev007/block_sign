...
type FcTransactionsReceiveBtc struct {
	Id          int       `json:"id" xorm:"not null pk autoincr INT(11)"`
	CoinId      int       `json:"coin_id" xorm:"not null comment('币种ID') INT(11)"`
	FromAddress string    `json:"from_address" xorm:"not null comment('来源地址') index unique(idx_unique) VARCHAR(100)"`
	ToAddress   string    `json:"to_address" xorm:"not null comment('接收地址') index unique(idx_unique) VARCHAR(100)"`
	...
	Status      int       `json:"status" xorm:"not null default 0 comment('确认数') TINYINT(4)"`
