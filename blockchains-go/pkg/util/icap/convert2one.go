...
func Bech32ToAddressHrp(b32 string, bench32AddrHRP string) (addr T, err error) {
	var hrp string
	err = parseBech32Addr(b32, &hrp, &addr)
	if err == nil && hrp != bench32AddrHRP {
		err = errors.Errorf("%#v is not a %#v address", b32, bench32AddrHRP)
	}
	return
}
