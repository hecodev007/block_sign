...
func (rpc *RpcClient) CallWithAuthBtm(raw string) (string, error) {
	result, err := rpc.callbtm(raw)
	if err != nil {
		fmt.Println("CallWithAuth", err.Error())
		return "", err
	}
...
func (rpc *RpcClient) callbtm(raw string) (string, error) {
	request := requestbtm{
		RawTransaction: raw,
	}
