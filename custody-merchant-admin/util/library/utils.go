...
// EncryptSha256Password 盐加密加密
func EncryptSha256Password(password string, salt string) string {
	return encryptSha256(encryptSha256(password) + encryptSha256(salt))
}
...
// RSADecryptPassword 密码解密
func RSADecryptPassword(password string) (string, error) {
	getwd, err := os.Getwd()
	if err != nil {
		return "", err
	}
	decodeString, err := base64.StdEncoding.DecodeString(password)
	if err != nil {
		return "", err
	}
	str, _ := RSA_Decrypt(decodeString, getwd+"/private.pem")
