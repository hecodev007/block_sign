...
func GetMemcachedStore() *MemcachedStore {
	if MemStore == nil {
		MemStore = NewMemcachedStore([]string{Conf.Memcached.Server}, time.Minute)
	}
	return MemStore
}
...
func (c *MemcachedStore) Get(key string, value interface{}) error {
	...
	return util.Deserialize(item.Value, value)
}
