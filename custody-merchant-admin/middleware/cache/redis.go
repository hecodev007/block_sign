...
import (
	cf "custody-merchant-admin/config"
...
func NewRedisCache(host string, password string, defaultExpiration time.Duration) *RedisStore {
	...
	if RedisConn != nil {
		RedisConn = &RedisStore{pool, defaultExpiration}
	}
	return &RedisStore{pool, defaultExpiration}
}

func GetRedisConn() *RedisStore {
	if RedisConn.pool == nil {
		rd := cf.Conf.Redis
		RedisConn = NewRedisCache(rd.AloneAddress, rd.AlonePwd, RedisConn.defaultExpiration)
	}
	return RedisConn
}
...
func (c *RedisStore) Get(key string, ptrValue interface{}) error {
	...
	return util.Deserialize(item, ptrValue)
}
