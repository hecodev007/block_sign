...
func (k *OrderKeeper) pushIfNotExist(param *model.TransferParams) error {
	log.Printf("尝试将订单放入待执行列表outerOrderNo %s", param.OuterOrderNo)
	listLen, err := redis.Client.ListLen(orderKeeperListKey)
	if err != nil {
		log.Printf("从缓存获取待执行列表长度出现异常:%v", err)
		return err
	}
	log.Printf("目前待处理列表数量 %d", listLen)
	if listLen >= k.keeperSize {
		log.Printf("当前待处理订单数已达到最大数量(%d)", k.keeperSize)
	...
	if err != nil {
		log.Printf("执行redis SetNX命令出现异常:%v", err)
		return err
	}
	if !success {
		log.Printf("订单 %s 在待执行列表已存在", param.OuterOrderNo)
	...
	log.Printf("订单推入完成 %s", param.OuterOrderNo)
	return nil
}
