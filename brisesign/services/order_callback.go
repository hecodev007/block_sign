...
func (o *OrderCallback) processCallback() {
	...
	if unmarshalErr != nil {
		log.Printf("processCallback json unmarshal err:%v", unmarshalErr)
	...
	log.Printf("获取到需要执行callback回调的订单数据 %s", reCallback.Data.OuterOrderNo)
	...
	log.Printf("调用blockchain回调接口再次失败，已重试了 %d 次 失败信息:%v", reCallback.RetryCount, sendErr)
	o.rePushToCache(reCallback)
}
...
func (o *OrderCallback) rePushToCache(reCallback *model.ReCallback) {
	...
	if marshalErr != nil {
		log.Printf("[Callback] json Marshal err: %v", marshalErr)
		return
	}
	if err := redis.Client.ListRPush(orderCallbackWaitingReCallbackKey, buf); err != nil {
		log.Printf("Push数据到redis缓存失败:%v", err)
	}
}
...
func (o *OrderCallback) sendPostRequest(params *model.CallbackReqParams) error {
	...
	if err != nil {
		log.Printf("订单%s 回调blockchain出错:%v", params.Data.OuterOrderNo, err)
	} else {
		log.Printf("订单%s 回调blockchain完成:%s", params.Data.OuterOrderNo, string(reqResult))
	}
	return err
}
