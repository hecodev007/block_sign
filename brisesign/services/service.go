...
type IService interface {
	...
	//TransferService(req interface{}) error
	TransferService(req interface{}) ([]byte, error)
	TransferCollectService(req interface{}) (interface{}, error)
...
func (s *Service) GetKeyByAddress(address string) (string, error) {
	...
	privateBytes, err := util.AesBase64Crypt([]byte(encryptKey), []byte(aesKey), false)
	if err != nil {
		return "", err
	}
