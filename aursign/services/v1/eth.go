...
func (bs *BaseService) AURService() *AURService {
	cs := new(AURService)
	cs.BaseService = bs
	// 初始化连接
...
接口创建地址服务
	无需改动
*/
func (cs *AURService) CreateAddressService(req *model.ReqCreateAddressParamsV2) (*model.RespCreateAddressParams, error) {
...
离线创建地址服务，通过多线程创建
	无需改动
*/
func (cs *AURService) MultiThreadCreateAddrService(nums int, coinName, mchId, orderId string) error {
...
签名服务
*/
func (cs *AURService) SignService(req *model.ReqSignParams) (interface{}, error) {
...
热钱包出账服务
*/
func (cs *AURService) TransferService(req interface{}) (interface{}, error) {
	var (
...
func (cs *AURService) TransferWithNonceService(req interface{}) (interface{}, error) {
...
func (cs *AURService) processTransfer(tp model.EthTransferParams, nonce uint64) (interface{}, error) {
	if tp.FromAddress == "" || tp.ToAddress == "" || tp.Amount == "" {
		return nil, fmt.Errorf("params is null,from=[%s],to=[%s],amount=[%s]", tp.FromAddress, tp.ToAddress, tp.Amount)
	}
	if cs.client == nil {
	...
	//余额检查
	balance, err := cs.GetBalance(&model.ReqGetBalanceParams{CoinName: "aur", Address: tp.FromAddress, ContractAddress: tp.ContractAddress})
	if err != nil {
		return nil, fmt.Errorf("get balance error,err=%v", err)
	}
	bBalance := new(big.Int)
	sBalance := fmt.Sprintf("%v", balance)
	bBalance.SetString(sBalance, 10)
	if amount.Cmp(bBalance) > 0 {
		return nil, fmt.Errorf("check balance error,err=%v", "There is not enough balance")
	}
	...
	//测试网地址：0xC2Bba9Ae55bc6A7d13797e3FD393C406702aAF60
	//hexPrivateKey := "837b1cb134e5c6d4d08d56e1fb5000b0de8cc6bb62b8445c60f69c9e425ff1ed"
...
func (cs *AURService) getBuildTxParams(method string, params []interface{}) (*big.Int, error) {
...
func (cs *AURService) getNonce(method, address, tag string) uint64 {
...
创建地址实体方法
*/
func (cs *AURService) createAddressInfo() (util.AddrInfo, error) {
...
*/
func (cs *AURService) makeERC20TransferData(toAddress, transfer_method_id string, amount *big.Int) ([]byte, error) {
	var data []byte
...
*/
func (cs *AURService) ethSign(to string, nonce, gasLimit uint64, value, gasPrice *big.Int, privKey *ecdsa.PrivateKey) (string, bool) {
	// build transaction
	tx := types.NewTransaction(nonce, common.HexToAddress(to), value, gasLimit, gasPrice, nil)
	//tx := types.NewTx(nonce, common.HexToAddress(to), value, gasLimit, gasPrice, nil)
	// to do eth sign
...
*/
func (cs *AURService) ethTokenSign(to, toContractAddr string, nonoce, gasLimit uint64, value, gasPrice *big.Int, privKey *ecdsa.PrivateKey) (string, bool) {
...
*/
func (cs *AURService) sign(tx *types.Transaction, privKey *ecdsa.PrivateKey) (string, bool) {
	signTx, err := types.SignTx(tx, types.NewEIP155Signer(big.NewInt(conf.Config.EthCfg.ChainId)), privKey)
	if err != nil {
...
func (cs *AURService) sign2(tx *types.Transaction, privKey *ecdsa.PrivateKey) (string, bool) {
...
func (cs *AURService) aegisNonce(address string) (uint64, error) {
	// 1. 先获取该地址链上的nonce值
...
func (cs *AURService) isPendingStatus(txid string) bool {
...
func (cs *AURService) GetBalance(req *model.ReqGetBalanceParams) (interface{}, error) {
	var (
		data []byte
		err  error
	)
...
func (cs *AURService) ValidAddress(address string) error {
