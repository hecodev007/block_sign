...
func (c Client) GetAccountInfo(address string, meta *types.Metadata) (types.AccountInfo, error) {
	...
	pubKey := GetPublicFromAddr(address, DOTPrefix)
...
func (c Client) ParseExtrinsic(meta *types.Metadata, blockHash types.Hash, extrinsics []types.Extrinsic, rsp *BlockResponse) error {
	...
	for i, v := range extrinsics {
		...
		if transIndex.SectionIndex == v.Method.CallIndex.SectionIndex && transIndex.MethodIndex == v.Method.CallIndex.MethodIndex {
			...
			// create address
			from, err := CreateAddress(v.Signature.Signer.AsID[:], DOTPrefix)
			if err != nil {
				return err
			}
			...
			to, err := CreateAddress(args.To.AsID[:], DOTPrefix)
			if err != nil {
				return err
			}
			ext.ToAddress = to
...
func (c Client) ParseEventForExtrinsic(er *DotEventRecords, ret *BlockResponse) error {
	...
	if len(er.Balances_Transfer) > 0 {
		...
		for _, ebt := range er.Balances_Transfer {
			...
			from, err := CreateAddress(ebt.From[:], DOTPrefix)
			if err != nil {
				return err
			}
			r.From = from

			to, err := CreateAddress(ebt.To[:], DOTPrefix)
			if err != nil {
				return err
			}
			r.To = to
...
func (c Client) GetEventRecords(blockHash types.Hash) (*DotEventRecords, error) {
