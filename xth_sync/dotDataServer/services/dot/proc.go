...
import (
	...
	"dotDataServer/common/log"
	"dotDataServer/models/bo"
	dao "dotDataServer/models/po/dot"
	"dotDataServer/services"
	rpc "dotDataServer/utils/dot"
	"encoding/json"
	"errors"
	"fmt"
	"strings"
...
func (s *Processor) getBlockTxFromNode(height int64, txid string) (*dao.BlockTx, error) {
	...
	var nodetx *rpc.Extrinsic
	for i, tx := range block.Extrinsics {
		if strings.ToLower(tx.Txid) == strings.ToLower(txid) {
			nodetx = block.Extrinsics[i]
	...
	//	Transactions:      len(block.Extrinsic),
	...
	tmpDaoTx.Txid = nodetx.Txid
	tmpDaoTx.Fromaccount = nodetx.FromAddress
	tmpDaoTx.Toaccount = nodetx.ToAddress
	fee, err := decimal.NewFromString(nodetx.Fee)
	if err != nil {
		log.Error(err.Error())
		return nil, err

	}
	tmpDaoTx.SysFee = fee.Shift(-10).String()
	amount, err := decimal.NewFromString(nodetx.Amount)
	if err != nil {
		log.Error(err.Error())
		return nil, err
	}
	tmpDaoTx.Amount = amount.Shift(-10).String()
	return tmpDaoTx, nil
}
