...
import (
	...
	dao "filDataServer/models/po/fil"
	rpc "filDataServer/utils/fil"
...
func (s *Scanner) ScanHead(height int64) error {
	...
	if CurrentHead.Height < height {
		//log.Info(CurrentHead.Height)
		//dao.ExedRollBack()
		//return errors.New("分叉了")
		return nil
	}
	...
	bc := &dao.BlockChain{
		...
		Timestamp:     time.Unix(CurrentHead.Blocks[0].Timestamp,0),
	...
	for {
		...
		if err != nil {
			...
			for i := 0; i < len(parent); i++ { //查找parent块是否存在
				...
				if err == nil {
					log.Info(CurrentHead.Height,parent[i]["/"])
					break
				}
			}
			//parent块都不存在返回错误
			if err != nil {
				//panic(err.Error())
				dao.ExedRollBack(CurrentHead.Height)
				return err
			}
			//过滤掉不存在的块
...
GetParentReceipts:
	...
	for txindex, tx := range messages {
		...
		if txInfo, err := s.parseBlockRawTX(s.conf.Name, tx.Message, Cids[0]["/"], height); err != nil {
			log.Info(err.Error())
		} else if txInfo != nil {
...
//解析交易
func (s *Scanner) parseBlockRawTX(coinName string, tx *rpc.Transaction, blockhash string, height int64) (*dao.BlockTx, error) {
	if tx == nil {
		return nil, nil
	}
