...
func (s *Processor) RepushTx(userid int64, txid string) error {
	...
	watchaddrs := make(map[string]bool)
	vouts, vins, _ := s.parseContractTX(txinfo, watchaddrs)
	if err != nil {
		return err
	}
	...
	return s.processPush(txinfo.Tx, vouts, vins, watchaddrs, bestBlockHeight)
}
