module atomDataServer

go 1.13

require (
	github.com/BurntSushi/toml v0.3.1
	github.com/DeanThompson/ginpprof v0.0.0-20190408063150-3be636683586
	github.com/bwmarrin/snowflake v0.3.0
	github.com/cosmos/cosmos-sdk v0.34.4-0.20191010193331-18de630d0ae1
	github.com/gin-gonic/gin v1.5.0
	github.com/go-sql-driver/mysql v1.5.0
	github.com/jinzhu/gorm v1.9.12
	github.com/lestrrat/go-file-rotatelogs v0.0.0-20180223000712-d3151e2a480f
	github.com/o3labs/neo-utils v0.0.0-20190806035218-cbe201aea47a
	github.com/onethefour/common v0.0.4
	github.com/shopspring/decimal v0.0.0-20200105231215-408a2507e114
	github.com/streadway/amqp v0.0.0-20200108173154-1c71cc93ed71
	github.com/tendermint/tendermint v0.33.8
	go.uber.org/zap v1.19.0
	golang.org/x/net v0.0.0-20200324143707-d3edc9973b7e
	google.golang.org/appengine v1.6.5
	xorm.io/xorm v0.8.1
)

replace (
	github.com/cosmos/cosmos-sdk => github.com/cosmos/cosmos-sdk v0.37.13
	github.com/tendermint/tendermint => github.com/tendermint/tendermint v0.32.12
)
