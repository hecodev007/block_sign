...
//}
func (s *Processor) getBlockTxInfosFromNode(txid string,height int64) (*dao.BlockTx, error) {
	block,err := s.Block(height)
	if err != nil {
		return nil,err
	}
	var tx *rpc.Transaction
	for k,temptx := range block.Transactions{
		if temptx.Signature == txid {
			tx = block.Transactions[k]
		}
	}
	if tx == nil{
	...
	return s.parseBlockRawTX(tx, block.Hash, height)
}
