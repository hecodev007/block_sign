...
import (
	...
	"telosDataServer/common/rabbitmq"
	"telosDataServer/conf"
	"telosDataServer/models/bo"
	"telosDataServer/models/po"
	"time"
)
	...
	go func() {
		for run {
			...
			case task := <-s.pushTaskList:
				//log.Infof("pushTaskList start ",s.cfg.Enable)
				if s.cfg.Enable {
					...
					if s.cfg.Type == "rabbitmq" {
						...
						if err == nil {
							...
							log.Warn("rabitmq push err:" + err.Error())
						}
					} else {
						...
						if err == nil {
							...
							log.Warn("http push err:" + err.Error())
						}
					}
