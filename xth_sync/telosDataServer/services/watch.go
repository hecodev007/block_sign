...
func NewWatchControl(name string) (*WatchControl, error) {
	...
	//if s.watchContracts[strings.ToLower("eosio.token")]name == "telos" {
	s.watchContracts[strings.ToLower("eosio.token")] = po.ContractInfo{Id: 0, Name: "telos", ContractAddress: "eosio.token", Decimal: 4, CoinType: "telos"}
	//}
	return s, nil
}
