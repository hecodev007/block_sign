...
import (
	...
	dao "telosDataServer/models/po/zec"
	"telosDataServer/utils/btc"
	"time"
)
...
func NewScanner(conf conf.Config, node conf.NodeConfig) common.Scanner {
	log.Info("NewScanner", node.RPCKey, node.RPCSecret)
	return &Scanner{
...
func (s *Scanner) scanBlock2(height, bestHeight int64) (common.ProcTask, error) {
	...
	task := &BtcProcTask{
		irreversible: false, //TODO:bestHeight-block.Height>7
		bestHeight:   bestHeight,
		block: &dao.BlockInfo{
			Height:         block.Height,
			Hash:           block.Hash,
	...
	if len(block.Txs) > 0 {
		for _, tx := range block.Txs {
			if txInfo, err := parseBlockRawTX(s.conf.Name, tx, block.Hash, height); err == nil {
				...
				log.Info("parseBlockRawTX", err.Error())
			}
		}
	}
...
func (s *Scanner) batchParseRawTx(jobs <-chan string, results chan<- int, blockhash string, height int64, task *BtcProcTask) {
	...
	for i := 0; i < count; i++ {
		...
		case txid := <-jobs:
			...
			tx, err := s.GetRawTransaction(txid)
			if err != nil {
				log.Errorf("GetRawTransaction txid: %s , err: %v", txid, err)
				continue
			}

			txInfo, err := parseBlockRawTX(s.conf.Name, &tx, blockhash, height)
			if err != nil {
				log.Errorf("parseBlockTX %v", err)
				continue
			}
