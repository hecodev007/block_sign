...
func (proxy *ResponseBlock) toBlock() *Block {
	block := Block{
		...
		ParentHash: proxy.BlockMeta.Header.LastBlockID.Hash,
		ChainID:    proxy.BlockMeta.Header.ChainID,
		Timestamp:  proxy.BlockMeta.Header.Time,
	}
	block.Height, _ = utils.ParseInt64(proxy.BlockMeta.Header.Height)
