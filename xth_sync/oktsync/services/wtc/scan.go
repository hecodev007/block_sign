...
import (
	...
	"oktsync/common/conf"
	"oktsync/common/log"
	dao "oktsync/models/po/yotta"
	"oktsync/services"
	"oktsync/utils"
	rpc "oktsync/utils/wtc"
	"strings"
	"sync"
	"time"
...
func parseBlockTX(watch *services.WatchControl, rpc *rpc.RpcClient, tx *rpc.Transaction, block *dao.BlockInfo) ([]*dao.BlockTx, error) {
	...
	if watch.IsContractExist(tx.To) || len(tx.Input) > 75 && (watch.IsWatchAddressExist("0x"+tx.Input[34:74])||watch.IsWatchAddressExist(tx.From)) {
