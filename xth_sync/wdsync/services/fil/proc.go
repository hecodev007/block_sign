...
func (p *Processor) RepushTx(userId int64, txid string, height int64) error {
	if height < 16677{
		return errors.New("高度不允许低于分叉高度97500(16677)")
	}
	//height -= 80823
	nextheight := height + 1
	...
	log.Info(nextblock.Cids[0]["/"],string(str))
	if err != nil {
		return err
	}
	...
	for k, _ := range messages {
		//log.Info(messages[k].Message.Cid["/"])
		if messages[k].Message.Cid["/"] == txid {
			index = k
			break
		}
	}
	...
	//todo 有bug block.Cids可能为0
	blockTx, err := p.parseBlockRawTX("", messages[index], block.Cids[0]["/"], height)
	if err != nil {
		return err
	}
...
//解析交易
func (s *Processor) parseBlockRawTX(coinName string, msg *rpc.Message, blockhash string, height int64) (*dao.BlockTx, error) {
	if msg == nil {
		return nil, nil
	}
	tx := msg.Message
