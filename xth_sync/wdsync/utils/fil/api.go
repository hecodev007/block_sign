...
//区块高度
func (rpc *RpcClient) BlockHeightbak() (int64, error) {
	ret := new(SyncState)
...
func (rpc *RpcClient) BlockHead() (ret *ChainHead, err error) {
	...
	if len(data.ActiveSyncs) < 2 {
		return ret, nil
	}
	if data.ActiveSyncs[1].Target == nil {
		return nil,errors.New("Filecoin.SyncState 接口返回错误")
	}
	ret.Cids = data.ActiveSyncs[1].Target.Cids
	ret.Blocks = data.ActiveSyncs[1].Target.Blocks
	if len(data.ActiveSyncs[1].Target.Blocks) > 0 {
		ret.Height = data.ActiveSyncs[1].Target.Blocks[0].Height
	}
	return ret, err
}
