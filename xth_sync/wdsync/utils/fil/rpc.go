...
func NewRpcClient(url, scan,username, password string, options ...func(rpc *RpcClient)) *RpcClient {
	credentials := ""
	if username != "" {
...
func (rpc *RpcClient) CallWithAuth(method, credentials string, target interface{}, params ...interface{}) error {
	...
	if method == "Filecoin.ChainHead" {
		//log.Info(method, string(result))
	}
	err = json.Unmarshal(result, target)
	if err != nil {
		log.Info(rpc.url,method, string(result), err.Error())
	}
	return err
}
...
func (rpc *RpcClient) call(method, credentials string, params ...interface{}) (json.RawMessage, error) {
	...
	//log.Info("response code:",rpc.url, response.Status, method, string(body))
