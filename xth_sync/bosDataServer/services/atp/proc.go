...
import (
	...
	dao "bosDataServer/models/po/cfx"
	"bosDataServer/services"
	rpc "bosDataServer/utils/algo"
	"encoding/json"
	"fmt"
)
...
func (s *Processor) getBlockTxInfosFromNode(txid string) (*TxInfo, error) {
	...
	block, err := s.GetBlockByHeight(int64(tx.ConfirmedRound))
	if err != nil {
		log.Info(err.Error())
		return nil, err
	}
	return parseBlockRawTX(s.RpcClient, s.watch, &tx, block.Hash, bestHeight)
}
