...
func (s *Processor) processPush(blocktxs []*dao.BlockTx, tmpWatchList map[string]bool, bestHeight int64) error {
	...
	for _, blocktx := range blocktxs {
		contract_address := blocktx.ContractAddress
		if contract_address == "iost" {
			contract_address = ""
		}
...
func (s *Processor) getBlockTxFromNode(txid string) ([]*dao.BlockTx, error) {
	...
	block, err := s.BlockByNumber(tx.BlockNumber.IntPart())
	if err != nil {
		return nil, err
	}
	return parseBlockTxInternal(s.watch, s.RpcClient, tx, block.Hash)
}
