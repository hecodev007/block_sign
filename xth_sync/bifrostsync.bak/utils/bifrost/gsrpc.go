...
func (c *Client) GetBlock(h int64) (*BlockResponse, error) {
	...
	err = c.ParseExtrinsic(meta, hash, block.Block.Extrinsics, &ret)
	if err != nil {
		log.Println(err.Error())
		return nil, err
	}
...
func (c *Client) ParseExtrinsic(meta *types.Metadata, blockHash types.Hash, extrinsics []types.Extrinsic, rsp *BlockResponse) error {
	...
	for i, v := range extrinsics {
		...
		if transIndex.SectionIndex == v.Method.CallIndex.SectionIndex && transIndex.MethodIndex == v.Method.CallIndex.MethodIndex {
			...
			nonce := big.Int(v.Signature.Nonce)
			ext.Nonce = nonce.Int64()
			//log.Println(ext.Txid)
			// create address
			from, err := CreateAddress(v.Signature.Signer.AsID[:], BNCPrefix)
			//log.Println(from)
			if err != nil {
				log.Println(err.Error())
				return err
			...
			args := Args{}
			//log.Println(hex.EncodeToString(v.Method.Args))
			...
			to, err := CreateAddress(args.To.AsID[:], BNCPrefix)
			if err != nil {
				log.Println(err.Error())
				return err
			}
			...
			rsp.Extrinsic = append(rsp.Extrinsic, &ext)
...
func (c *Client) ParseEventForExtrinsic(er *CRustEventRecords, ret *BlockResponse) error {
	res := make([]*EventResult, 0)

	if len(er.Balances_Transfer) > 0 {
		//failed event
		failedMap := make(map[int]bool)
		for _, failed := range er.System_ExtrinsicFailed {
		...
		for _, ebt := range er.Balances_Transfer {
			...
			from, err := CreateAddress(ebt.From[:], BNCPrefix)
			if err != nil {
				return err
			}
			r.From = from

			to, err := CreateAddress(ebt.To[:], BNCPrefix)
			if err != nil {
				return err
			}
			r.To = to
...
func (c *Client) GetEventRecords(blockHash types.Hash) (*CRustEventRecords, error) {
	...
	if err != nil {
		log.Println(key, blockHash, err)
		return nil, err
	}

	events := CRustEventRecords{}
