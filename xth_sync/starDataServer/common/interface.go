...
type ProcTask interface {
	...
	GetBestHeight() int64
	GetConfirms() int64
	GetBlockHash(int2 int) string
	GetTxs() []interface{}
	GetBlock(int2 int) interface{}
}
...
type Processor interface {
	...
	SetPusher(p Pusher)
	RemovePusher()
	RepushTx(userId int64, txid string, height int64) error
...
type Scanner interface {
	...
	ScanBaseBlock(height, bestHeight int64) (ProcTask, error)
	ScanHead(height int64) error
}
