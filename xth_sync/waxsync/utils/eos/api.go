...
func NewAPI(baseURL, baseURL2 string) *API {
	api := &API{
...
func (api *API) GetTransactionFromThird(id string) (out *TransactionThirdResp2, err error) {
	err = api.call2("v1/history/get_transaction", M{"id": id}, &out)
	return
}
...
func (api *API) call(baseAPI string, body interface{}, out interface{}) (err error) {
	...
	defer func() {
		if err != nil {
			log.Info(BaseURL,err.Error())
		} else {
			log.Info(BaseURL,"success")
		}
	}()
	...
	BaseURL = api.BaseURL
	if api.BaseURL2 != "" && rand.Int()%3 == 0{
		BaseURL = api.BaseURL2
	}
	//log.Info(BaseURL)
	targetURL := fmt.Sprintf("%s/%s", BaseURL, baseAPI)
...
// See more here: libraries/chain/contracts/abi_serializer.cpp:58...
func (api *API) call2(baseAPI string, body interface{}, out interface{}) (err error) {
	defer func() {
		if err != nil {
			log.Info(err.Error())
		}
