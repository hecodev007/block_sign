...
import (
	...
	sdktypes "github.com/okex/okexchain-go-sdk/types"
...
func NewRpcClient(url, user, pwd string) *RpcClient {
	config, err := gosdk.NewClientConfig(url, "okexchain", gosdk.BroadcastBlock, decimal.NewFromInt(100).Shift(-4).String()+"okt", 200000,
		0, "")
	if err != nil {
		panic(err.Error())
	}
