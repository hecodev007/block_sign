...
var (
	GrabMetadataDecoder scalecodec.MetadataDecoder
)
...
func init() {
	...
	GrabMetadataDecoder.Init(utiles.HexToBytes(crabV13))
	_ = GrabMetadataDecoder.Process()

	ksmFile, err := ioutil.ReadFile(fmt.Sprintf("%s.json", "network/crab"))
	if err != nil {
		panic(err)
	}
...
func (c Client) GetAccountInfo(address string, meta *types.Metadata) (types.AccountInfo, error) {
	...
	pubKey := GetPublicFromAddr(address, GrabPrefix)
...
func (c Client) ParseExtrinsic(meta *types.Metadata, blockHash types.Hash, extrinsics []types.Extrinsic, rsp *BlockResponse) error {
	...
	for i, v := range extrinsics {
		...
		if transIndex.SectionIndex == v.Method.CallIndex.SectionIndex && transIndex.MethodIndex == v.Method.CallIndex.MethodIndex {
			...
			// create address
			from, err := CreateAddress(v.Signature.Signer.AsID[:], GrabPrefix)
			if err != nil {
				return err
			}
			...
			to, err := CreateAddress(args.To.AsID[:], GrabPrefix)
			if err != nil {
				return err
			}
			ext.ToAddress = to
	...
	if len(er.GetBalancesTransfer()) > 0 {
		...
		for _, ebt := range er.GetBalancesTransfer() {
			...
			if len(eventParams) == 3 {
				...
				from, err := CreateAddress(fr, GrabPrefix)
				if err != nil {
					return err
				}
				r.From = from
				...
				to, err := CreateAddress(t, GrabPrefix)
				if err != nil {
					return err
				}
				r.To = to

				r.Amount = eventParams[2].Value.(decimal.Decimal).String()
...
func (c Client) GetEventRecords(blockHash types.Hash) (*EventModelRecord, error) {
	...
	option := scaletypes.ScaleDecoderOption{Metadata: &GrabMetadataDecoder.Metadata}
