...
import (
	...
	dao "marsDataServer/models/po/doge"
	"marsDataServer/utils"
	rpc "marsDataServer/utils/doge"
...
retryGetBlockByHeight:
	...
	for index, txid := range block.Txs {
		...
		getTransaction:
			...
			if err == nil {
				if txInfo, err := s.parseBlockRawTX(s.conf.Name, &tx, block.Hash, height); err != nil {
					log.Info(err.Error())
				} else if txInfo != nil {
...
//解析交易
func (s *Scanner) parseBlockRawTX(coinName string, tx *rpc.Transaction, blockhash string, height int64) (*TxInfo, error) {
	...
	GetRawTransaction:
		vintx, err := s.RpcClient.GetRawTransaction(vin.Txid)
		if err != nil {
