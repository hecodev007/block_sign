...
func (s *Processor) processPush(blocktx *dao.BlockTx, txvouts []*dao.BlockTxVout, txvins []*dao.BlockTxVin, tmpWatchList map[string]bool, bestHeight int64) error {
	...
	for _, txvout := range txvouts {
		tmppushTxoutput := &bo.PushTxOutput{
			Addresse: txvout.Address,
			Value:    txvout.Value,
			N:        txvout.VoutN,
		}
		if txvout.AssertId != "" && txvout.AssertName != "" && txvout.AssertValue != "" {
			tmppushTxoutput.Assets = &bo.AssetsInfo{
				Name:       txvout.AssertName,
				AssetId:    txvout.AssertId,
				AssetValue: txvout.AssertValue,
			}
		}
		pushUtxoTx.Vout = append(pushUtxoTx.Vout, tmppushTxoutput)

	}

	for _, txvin := range txvins {
		tmpPushTxInput := &bo.PushTxInput{
			Txid:     txvin.Txid,
			Vout:     txvin.VoutN,
			Addresse: txvin.Address,
			Value:    txvin.Value,
		}
		if txvin.AssertId != "" && txvin.AssertName != "" && txvin.AssertValue != "" {
			tmpPushTxInput.Assets = &bo.AssetsInfo{
				Name:       txvin.AssertName,
				AssetId:    txvin.AssertId,
				AssetValue: txvin.AssertValue,
			}
		}
		pushUtxoTx.Vin = append(pushUtxoTx.Vin, tmpPushTxInput)
	}
...
func (p *Processor) parseContractTX(txs *TxInfo, watchaddrs map[string]bool) (vouts []*dao.BlockTxVout, vins []*dao.BlockTxVin, err error) {
	...
	for k, vin := range txs.Vins {
		if vin.AssertId != "" {
			contract, err := p.watch.GetContract(vin.AssertId)
			if err == nil {
				txs.Vins[k].AssertName = contract.Name
			} else {
				//log.Info(err.Error())
			}
		}
	...
	for k, vout := range txs.Vouts {
		if vout.AssertId != "" {
			contract, err := p.watch.GetContract(vout.AssertId)
			if err == nil {
				txs.Vouts[k].AssertName = contract.Name
			} else {
				//log.Info(err.Error())
			}
		}
...
func (s *Processor) RepushTx(userid int64, txid string, height int64) error {
	...
	bestheigt, err := s.GetBlockCount()
	if err != nil {
		return err
	}
	return s.processPush(txinfo.Tx, txinfo.Vouts, txinfo.Vins, watchaddrs, bestheigt)
}
...
func (s *Processor) getBlockTxInfosFromNode(txid string) (*TxInfo, error) {
	...
	for k, v := range txs {
		if v.Transaction.TransactionIdentifier.Hash == txid {
			return s.parseBlockRawTX(txs[k].Transaction, txs[k].BlockIdentifier.Hash, txs[k].BlockIdentifier.Index)
		}
	}
