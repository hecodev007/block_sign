...
func init() {
	//if err := InitAdaDB(conf.Cfg.DataBases["ada"]); err != nil {
	//	panic(err.Error())
	//}
...
//连接数据库
func InitAdaDB(cfg conf.DatabaseConfig) error {
	//dburl := fmt.Sprintf("%s:%s@tcp(%s)/%s?charset=utf8mb4&parseTime=true", cfg.User, cfg.PassWord, cfg.Url, cfg.Name)
	dburl := fmt.Sprintf("host=%v port=%v user=%v password=%v dbname=%v sslmode=disable",cfg.Url,cfg.Port,cfg.User,cfg.PassWord,cfg.Name)
	fmt.Println(dburl)
	conn, err := initDBConn(cfg.Type, dburl, cfg.Mode)
	if err != nil {
		return err
