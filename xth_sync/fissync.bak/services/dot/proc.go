...
import (
	...
	"fissync/common/log"
	"fissync/models/bo"
	dao "fissync/models/po/dot"
	"fissync/services"
	rpc "fissync/utils/dot"
	"fmt"
	"strings"
	"time"
)
...
func (s *Processor) RepushTx(userid int64, height int64, txid string) error {
	...
	if txinfos, err = s.getBlockTxFromNode(height, txid); err != nil {
		return fmt.Errorf("没找到合法交易 %v", err)
	}
	if txinfos == nil {
...
func (s *Processor) getBlockTxFromNode(height int64, txid string) ([]*dao.BlockTx, error) {
	...
	for _, tx := range block.Extrinsics {
		if strings.ToLower(tx.Hash) == strings.ToLower(txid) && tx.Success == false {
	...
	rawtxs, err := s.RpcClient.GetExtrinsicsByNum(height)
	if err != nil {
		return nil, err
	}
	txs, err := s.parseBlockTX2(block.Extrinsics, rawtxs, blocktx)
	if err != nil {
		return nil, fmt.Errorf("parseActionForBlocktx err: %v ", err)
	}
	ret := make([]*dao.BlockTx, 0)
	for i, tx := range txs {
		if strings.ToLower(tx.Txid) == strings.ToLower(txid) {
			ret = append(ret, txs[i])
		}
	}
