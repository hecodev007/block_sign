	...
	go func() {
		...
		for run {
			...
			default:
				...
				if !ok {
					log.Debug("获取高度失败等待", s.StartHeight, bullet)
					time.Sleep(time.Second * 1)
					break
				}
...
func (s *ProcServer) RepushTx(userid int64, height int64, txid string) error {
	return s.processor.RepushTx(userid, height, txid)
}
