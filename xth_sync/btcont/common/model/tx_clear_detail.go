...
func (fc *FcTxClearDetail) AllTxIdByAddr(coin_type, addr string) ([]*FcTxClearDetail, error) {
	list := make([]*FcTxClearDetail, 0)
	err := db.SyncConn.Where("coin_type=? and addr=?", coin_type, addr).GroupBy("tx_id").Find(&list)
	return list, err
}

func (fc *FcTxClearDetail) AllByAddr(coin_type, addr string) ([]*FcTxClearDetail, error) {
	list := make([]*FcTxClearDetail, 0)
	err := db.SyncConn.Where("coin_type=? and addr=?", coin_type, addr).Find(&list)
	return list, err
}
