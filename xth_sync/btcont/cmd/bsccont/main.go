...
func diff_cold_scan() {
	node := model.NewEthNode(conf.Cfg.BscNode)
	addressAmounts, err := new(model.FcAddressAmount).All("bsc")
	if err != nil {
		log.Fatal(err.Error())
	}
...
func diff_cold_scan_asset() {
	node := model.NewEthNode(conf.Cfg.BscNode)
	addressAmounts, err := new(model.FcAddressAmount).AllAssert("bsc")
	if err != nil {
		log.Fatal(err.Error())
	}
	log.Info(len(addressAmounts))
...
func diff_suit_cold_scan() {
	...
	for _, addressAmount := range addressAmounts {
		...
		if amount.Cmp(addressAmount.Amount) != 0 {
			...
			//log.Info(t, time.Now().Unix())
			if t < time.Now().Unix()-60*60*3 {
				err = new(model.FcAddressAmount).SetMount("bsc", addressAmount.Address, amount.String())
				if err != nil {
					log.Info(err.Error())
					log.Error(err.Error())
				}
...
func diff_txs(addr string) {
	...
	fctxs, err := new(model.FcTxClearDetail).AllTxIdByAddr("bsc", addr)
	if err != nil {
		log.Fatal(err.Error())
	}
...
func AmountBylow(addr string) (decimal.Decimal, error) {
	utxos, err := new(model.FcTxClearDetail).AllByAddr("bsc", addr)
	if err != nil {
...
func test() {
	...
	for _, addressAmount := range addressAmounts {
		...
		txsMap := new(model.Scan).ToMap(txs)
		fctxs, err := new(model.FcTxClearDetail).AllTxIdByAddr("bsc", addressAmount.Address)
		if err != nil {
			log.Fatal(err.Error())
		}
		for _, fctx := range fctxs {
