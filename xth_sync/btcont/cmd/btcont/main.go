...
func main() {
	if len(os.Args) > 1 {
		...
		log.Info("AmountBylow addr", "addr流水余额")
		log.Info("diff_txs addr", "addr 漏推送的交易id")
		log.Info("diff_cold_flow ", "额度与流水不一致的所有地址")
		log.Info("diff_cold_scan", "额度与链上额度不同的所有地址")
		log.Info("diff_cold_scan_asset", "额度与链上额度不同的所有地址(只检查amount>0)")
	}
	log.Info("btcount end!!!")
}

func diff_cold_scan() {
	addressAmounts, err := new(model.FcAddressAmount).All("btc")
	if err != nil {
		log.Fatal(err.Error())
	}
	now := time.Now()
	...
	reBalanceOf:
		amount, t, err := new(model.Scan).BalanceOf(addressAmount.Address)
		if err != nil {
			log.Info(err.Error())
			goto reBalanceOf
		}
		if amount.Cmp(addressAmount.Amount) != 0 {
			//log.Info(t)
			tm := time.Unix(t, 0)
			log.Info(addressAmount.Address, "链上额度不一致", now.Sub(tm).String(), amount.String(), addressAmount.Amount.String())
...
func diff_cold_scan_asset() {
	addressAmounts, err := new(model.FcAddressAmount).AllAssert("btc")
	if err != nil {
		log.Fatal(err.Error())
	}
	now := time.Now()
...
func diff_suit_cold_scan() {
	...
	for _, addressAmount := range addressAmounts {
		...
		if amount.Cmp(addressAmount.Amount) != 0 {
			...
			//log.Info(t, time.Now().Unix())
			if t < time.Now().Unix()-60*60*3 {
				err = new(model.FcAddressAmount).SetMount("btc", addressAmount.Address, amount.String())
				if err != nil {
					log.Info(err.Error())
					log.Error(err.Error())
				}
...
func diff_txs(addr string) {
	...
	fctxs, err := new(model.FcTxClearDetail).AllTxIdByAddr("btc", addr)
	if err != nil {
		log.Fatal(err.Error())
	}
...
func AmountBylow(addr string) (decimal.Decimal, error) {
	utxos, err := new(model.FcTxClearDetail).AllByAddr("btc", addr)
	if err != nil {
...
func test() {
	...
	for _, addressAmount := range addressAmounts {
		...
		txsMap := new(model.Scan).ToMap(txs)
		fctxs, err := new(model.FcTxClearDetail).AllTxIdByAddr("btc", addressAmount.Address)
		if err != nil {
			log.Fatal(err.Error())
		}
		for _, fctx := range fctxs {
