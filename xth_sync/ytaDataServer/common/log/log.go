...
func init() {
	...
	InitLogger(conf.Cfg.Mode, conf.Cfg.Log.Level, conf.Cfg.Log.Formatter, conf.Cfg.Log.OutFile, conf.Cfg.Log.ErrFile)
}
...
func InitLogger(mode, level, format, outfile, errfile string) {
	...
	config := zapcore.EncoderConfig{
		...
		EncodeCaller:   AbusulteCallerEncoder, //zapcore.ShortCallerEncoder,
...
func AbusulteCallerEncoder(caller zapcore.EntryCaller, enc zapcore.PrimitiveArrayEncoder) {
	...
	index := strings.Index(fullPath, "DataServer")
	if index < 0 {
		enc.AppendString(caller.TrimmedPath())
	} else {
		enc.AppendString(fullPath[index+11:])
	}
