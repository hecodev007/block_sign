...
func (s *ScanServer) scanData() (err error) {
	...
	if s.irrevScanHeight == 0 {
		...
		if s.irrevScanHeight < s.bestHeight-2000 {
			s.irrevScanHeight = s.bestHeight - 2000
		}
	...
	if gapCount > s.conf.Confirmations {
		...
		for endHeight := s.bestHeight - s.conf.Confirmations; s.irrevScanHeight <= endHeight; s.irrevScanHeight++ {
			//log.Info(s.conf.MultiScanNum, s.irrevScanHeight)
			workPool.Incr()
			//log.Info("workPool", workPool.Running, s.irrevScanHeight, endHeight)
