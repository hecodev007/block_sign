...
func (proxy *Result) ToTx() (tx *Transaction, err error) {
	...
	tx.Fee, _ = AtomToInt(feeStr.String())
	tx.From = msgs[sendIndex].(*btypes.MsgSend).FromAddress
	tx.To = msgs[sendIndex].(*btypes.MsgSend).ToAddress
	tx.Value, _ = AtomToInt(msgs[sendIndex].(*btypes.MsgSend).Amount.String())
	return tx, nil
