...
func AtomToInt(value string) (int64, error) {
	if value == "" || value == "0" || value == "0ukava,0ukava" {
		return 0, nil
	}
	if strings.HasSuffix(value, "kavas") {
		return 0, nil
	}
	//10uatom = 0.000010
	if len(value) >= 5 && (value[len(value)-5:len(value)] == "ukava") {
		...
		//10atom = 10.000000
	} else if len(value) >= 4 && value[len(value)-4:len(value)] == "kava" {
