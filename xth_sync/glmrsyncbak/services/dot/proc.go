...
import (
	...
	"glmrsync/common/log"
	"glmrsync/models/bo"
	dao "glmrsync/models/po/dot"
	"glmrsync/services"
	rpc "glmrsync/utils/dot"
	"encoding/json"
	"errors"
	"fmt"
	"strings"
...
func (s *Processor) RepushTx(userid int64, height int64, txid string) error {
	...
	if txinfos, err = s.getBlockTxFromScan(height, txid); err != nil {
		return err
	}
	...
	watchaddrses, txinfosmap := s.parseContractTxs(txinfos)
	for txhash, watchaddrs := range watchaddrses {
...
//}
func (s *Processor) getBlockTxFromScan(height int64, txid string) (ret []*dao.BlockTx,err error) {
	blockhash,err := s.GetBlockHash(height)
	if err != nil {
		return nil,err
	}
	block,err := s.RpcClient.DotScanApi.Block(blockhash)
	if err != nil {
		return nil,err
	}
	log.Info(xutils.String(block))
	txidx := -1
	for k,_ := range block.Extrinsics{
		if block.Extrinsics[k].ExtrinsicHash == txid {
			txidx = k
		}
	}
	if txidx < 0 {
	...
	//transaction,err := block.ToTransaction(txidx)
	//if err != nil {
	//	return nil,err
	//}
	txs,err := block.ToTransactions(txidx)
	if err != nil {
		return nil,err
	}
	//if transaction != nil {
...
func (s *Processor) getBlockTxFromNode(height int64, txid string) (*dao.BlockTx, error) {
	...
	log.Info(block.Extrinsics[txIndex])
