...
func init() {
	...
	KsmMetadataDecoder.Init(utiles.HexToBytes(karuraV14))
	_ = KsmMetadataDecoder.Process()

	ksmFile, err := ioutil.ReadFile(fmt.Sprintf("%s.json", "network/karura"))
	if err != nil {
		panic(err)
	}
...
func (c Client) GetAccountInfo(address string, meta *types.Metadata) (types.AccountInfo, error) {
	...
	pubKey := GetPublicFromAddr(address, KARPrefix)
...
func (c Client) ParseExtrinsic(meta *types.Metadata, blockHash types.Hash, extrinsics []types.Extrinsic, rsp *BlockResponse) error {
	...
	for i, v := range extrinsics {
		...
		if transIndex.SectionIndex == v.Method.CallIndex.SectionIndex && transIndex.MethodIndex == v.Method.CallIndex.MethodIndex {
			...
			// create address
			from, err := CreateAddress(v.Signature.Signer.AsID[:], KARPrefix)
			if err != nil {
				return err
			}
			...
			to, err := CreateAddress(args.To.AsID[:], KARPrefix)
			if err != nil {
				return err
			}
			ext.ToAddress = to
	...
	if len(er.GetBalancesTransfer()) > 0 {
		...
		for _, ebt := range er.GetBalancesTransfer() {
			...
			if len(eventParams) == 3 {
				...
				from, err := CreateAddress(fr, KARPrefix)
				if err != nil {
					return err
				}
				r.From = from
				...
				to, err := CreateAddress(t, KARPrefix)
				if err != nil {
					return err
				}
				r.To = to
