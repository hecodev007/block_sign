...
import (
	...
	"hdxsync/common/log"
	"hdxsync/models/bo"
	dao "hdxsync/models/po/dot"
	"hdxsync/services"
	rpc "hdxsync/utils/ksm"
	"strings"
	"time"
)
...
func NewProcessor(conf conf.Config, node conf.NodeConfig, watch *services.WatchControl) common.Processor {
	return &Processor{
		RpcClient: rpc.NewRpcClient(node.Node, node.RPCKey, node.RPCSecret),
		watch:     watch,
		conf:      conf.Sync,
	}
}
...
func (s *Processor) getBlockTxFromNode(height int64, txid string) (*dao.BlockTx, error) {
	...
	var nodetx *models.ExtrinsicResponse
	for i, tx := range block.Extrinsic {
		if strings.ToLower(tx.Txid) == strings.ToLower(txid)  {
			nodetx = block.Extrinsic[i]
