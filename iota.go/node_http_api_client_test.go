...
func TestNodeAPI_OutputIDsByAddress1(t *testing.T) {
	//ed, _ := iotago.ParseEd25519AddressFromHexString("6da71b0d31d08fc6c4cdbace317eb2921390b585d5b2a4e0eb1675371c987938")

	//"fra1x94crz99aznkr22jmx0rvda8qcem83hhhhlx9m8dkj3xs4q53hksmnccan"
	_, addr1, _ := iotago.ParseBech32("fra1x94crz99aznkr22jmx0rvda8qcem83hhhhlx9m8dkj3xs4q53hksmnccan")
	fmt.Println(addr1)

	nodeAPI := iotago.NewNodeHTTPAPIClient("https://api.lb-0.h.chrysalis-devnet.iota.cafe:443")
	rs, err := nodeAPI.MilestoneUTXOChangesByIndex(context.Background(), 2292478)
	fmt.Printf("%+v\n", rs)
	//nodeAPI.
	for _, outputIDHex := range rs.CreatedOutputs {
		utxoInput, err := iotago.OutputIDHex(outputIDHex).AsUTXOInput()
		fmt.Println(err)
		outputRes, err := nodeAPI.OutputByID(context.Background(), utxoInput.ID())
		output, err := outputRes.Output()
		fmt.Println(output.Target())

		tar, _ := output.Target()
		a := fmt.Sprintf("%v", tar)
		ed, _ := iotago.ParseEd25519AddressFromHexString(a)
		fmt.Println(ed.Bech32(iotago.PrefixTestnet))
		fmt.Println(output.Deposit())
	}

	for _, outputIDHex := range rs.ConsumedOutputs {
		utxoInput, err := iotago.OutputIDHex(outputIDHex).AsUTXOInput()
		fmt.Println(err)
		outputRes, err := nodeAPI.OutputByID(context.Background(), utxoInput.ID())
		output, err := outputRes.Output()
	...
	_, addr, _ := iotago.ParseBech32("atoi1qrhacyfwlcnzkvzteumekfkrrwks98mpdm37cj4xx3drvmjvnep6x8x4r7t")

	edAddr, _ := iotago.ParseEd25519AddressFromHexString(addr.String())

	pri := "0abd673641efea713e202e462ecce5293f4893de57fa4ba9719bb4649bca22d4119ffa4f6fba2398b9eb932e4192919ac391d04e44151eb7cad394a83cc5902b"
	identityOne, _ := hex.DecodeString(pri)

	inputAddr := iotago.AddressFromEd25519PubKey(ed25519.PrivateKey(identityOne).Public().(ed25519.PublicKey))
	fmt.Println(inputAddr.Bech32(iotago.PrefixTestnet))
	rsp, err := nodeAPI.BalanceByEd25519Address(context.Background(), &inputAddr)
	fmt.Println(rsp.Balance)
	addrKeys := iotago.AddressKeys{Address: &inputAddr, Keys: ed25519.PrivateKey(identityOne)}
	singer := iotago.NewInMemoryAddressSigner(addrKeys)
	...
	var inputUTXOs iotago.InputToOutputMapping
	for _, id := range resp.OutputIDs {
		inputUTXO1 := id.MustAsUTXOInput()
		builder = builder.AddInput(&iotago.ToBeSignedUTXOInput{Address: &inputAddr, Input: inputUTXO1})
		inputUTXOs = iotago.InputToOutputMapping{
			inputUTXO1.ID(): &iotago.SigLockedSingleOutput{Address: &inputAddr, Amount: 10},
		}
	}
	builder = builder.AddInputsViaNodeQuery(context.Background(), &inputAddr, nodeAPI, nil)
	//outputAddr1, _ := tpkg.RandEd25519Address()

	builder = builder.AddOutput(&iotago.SigLockedSingleOutput{Address: edAddr, Amount: 1000000})
	builder = builder.AddOutput(&iotago.SigLockedSingleOutput{Address: &inputAddr, Amount: rsp.Balance - 1000000})
	...
	semanticErr := payload.SemanticallyValidate(inputUTXOs)
	if semanticErr != nil {
	...
	fmt.Println(iotago.MessageIDToHexString(mess.MustID()))

}
func TestNodeAPI_Trans(t *testing.T) {
	nodeAPI := iotago.NewNodeHTTPAPIClient("http://chrysalis-nodes.iota.cafe")
	_, addr, _ := iotago.ParseBech32("iota1qquu7z83rzd3epj9stgql8d48gerngv79859nna9wl2njywkdfu37a04ac3")

	toAddr, _ := iotago.ParseEd25519AddressFromHexString(addr.String())

	pri := "b7b26ec248ed302cfbd89476ac01f8c60fcb9b5e9feced444e8cc8b5212c080cc013277a9ec17750c4221415d8a902803b1010c559b33970c7b44bab5bec6d5c"
	identityOne, _ := hex.DecodeString(pri)
	...
	//	builder = builder.AddInput(&iotago.ToBeSignedUTXOInput{Address: &inputAddr, Input: inputUTXO1})
	//	//inputUTXOs = iotago.InputToOutputMapping{
	//	//	inputUTXO1.ID(): &iotago.SigLockedSingleOutput{Address: &inputAddr, Amount: 1000000},
	//	//}
	//}
	builder = builder.AddInputsViaNodeQuery(context.Background(), &inputAddr, nodeAPI, nil)
	//outputAddr1, _ := tpkg.RandEd25519Address()

	builder = builder.AddOutput(&iotago.SigLockedSingleOutput{Address: toAddr, Amount: 900000})
	builder = builder.AddOutput(&iotago.SigLockedSingleOutput{Address: &inputAddr, Amount: rsp.Balance - 900000})
	//inputUTXO1 := &iotago.UTXOInput{TransactionID: tpkg.Rand32ByteArray(), TransactionOutputIndex: 0}
