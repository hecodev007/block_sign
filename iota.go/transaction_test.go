...
func TestTransaction_SemanticallyValidate(t *testing.T) {
	...
	a := base58.Encode(ed25519Addr[:])
	fmt.Println(base58.Encode(ed25519Addr[:]))

	_, identityOne, err := ed25519.GenerateKey(nil)

	one := hex.EncodeToString(identityOne)
	two, _ := hex.DecodeString(one)
	if identityOne[7] == two[7] {
		fmt.Println("true")
	}
	...
	fmt.Println(addrKeys.Address.Bech32(iotago.PrefixTestnet))
	//http://nodes.nectar.iota.cafe
	//"https://api.lb-0.h.chrysalis-devnet.iota.cafe:443"
	goshimAPI := client.NewGoShimmerAPI("http://nodes.nectar.iota.cafe")

	rsp, err := goshimAPI.GetAddressUnspentOutputs(a)
	if err != nil {
		fmt.Println(err)
	}

	//goshimAPI.PostTransaction()
	fmt.Println(rsp)
	fmt.Println(1)
	type test struct {
		name       string
