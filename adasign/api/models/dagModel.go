...
func (m *DagModel) Sign(params *validator.SignParams) (txhash string, rawTx string, err error) {
	for k, _ := range params.Ins {
		seed, err := m.GetPrivate(params.MchId, params.Ins[k].FromAddr)
		if err != nil {
			return "", "", err
		}
		params.Ins[k].FromPrivkey = string(seed)
	}
	return util.Sign(params)
}
