...
// 更新确认数
func UpdateConfirmationsHash(hash string, confirmations int64, nextblockhash string) (int64, error) {
	o := orm.NewOrm()
	res, err := o.Raw("update block_info set confirmations = ?, nextblockhash = ? where hash = ?", confirmations, nextblockhash, hash).Exec()
	if err == nil {
		num, _ := res.RowsAffected()
		return num, nil
	}
