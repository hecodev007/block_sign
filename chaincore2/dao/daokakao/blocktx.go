...
func (b *BlockTX) Select(txid string) (bool, error) {
	...
	if err == nil && nums > 0 {
		...
		b.Fee = common.StrToFloat64(maps[0]["gasused"].(string))
		b.Memo = maps[0]["memo"].(string)
		b.FeePayer = maps[0]["feepayer"].(string)
		return true, err
	}
	return false, err
}
...
func (b *BlockTX) Insert() (int64, error) {
	o := orm.NewOrm()
	res, err := o.Raw("insert into block_tx(txid, height, hash, contract_address, fromaddress, toaddress,amount,gaslimit,gasprice,gasused,memo,feepayer) values(?,?,?,?,?,?,?,?,?,?,?,?)",
		b.Txid, b.Height, b.Hash, b.Contract, b.From, b.To, b.Amount, b.GasLimit, b.GasPrice, b.Fee, b.Memo, b.FeePayer).Exec()
	if err == nil {
		num, _ := res.RowsAffected()
		return num, nil
	}
