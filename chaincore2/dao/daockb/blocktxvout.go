...
func (b *BlockTXVout) Insert() (int64, error) {
	o := orm.NewOrm()
	res, err := o.Raw("insert into block_tx_vout(txid, height, hash, vout_n, vout_value, vout_address, codehash, args, invaild, status) values(?,?,?,?,?,?,?,?,?,?)",
		b.Txid, b.Height, b.Hash, b.Voutn, b.Voutvalue, b.Voutaddress, b.CodeHash, b.Args, b.Invaild, b.Status).Exec()
	if err == nil {
		num, _ := res.RowsAffected()
		return num, nil
	}
