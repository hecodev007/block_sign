...
// 删除区块
func DeleteContractTX(height int64) (int64, error) {
	o := orm.NewOrm()
	res, err := o.Raw("delete from contract_tx where height >= ?", height).Exec()
	if err == nil {
...
func (b *ContractTX) SelectByIndex(index int64) error {
	return nil
}

// hash 获取交易数据
func (b *ContractTX) SelectByHash(hash string) error {
	return nil
}

// txid 获取交易数据
func (b *ContractTX) Select(txid string) (bool, error) {
	o := orm.NewOrm()
	var maps []orm.Params
...
// 插入交易数据
func (b *ContractTX) Insert() (int64, error) {
	o := orm.NewOrm()
	res, err := o.Raw("insert into contract_tx(txid, height, hash, contract_address, fromaddress, toaddress,amount,gaslimit,gasprice,gasused,status) values(?,?,?,?,?,?,?,?,?,?,?)",
		b.Txid, b.Height, b.Hash, b.ContractAddress, b.From, b.To, b.Amount, b.GasLimit, b.GasPrice, b.GasUsed, b.Status).Exec()
	if err == nil {
		num, _ := res.RowsAffected()
		return num, nil
	}
