...
func getBlockConfirmationsByHeight(height int64) int64 {
	var confirmation = int64(0)
	o := orm.NewOrm()
	var maps []orm.Params
	nums, err := o.Raw("select confirmations from block_info where height = ?", height).Values(&maps)
	if err == nil && nums > 0 {
		confirmation = common.StrToInt64(maps[0]["confirmations"].(string))
	}
	return confirmation
}
