...
// update
func UpdateBlockAmount(addr, amount string) (int64, error) {
	o := orm.NewOrm()
...
// find and set
func FindSetBlockAmount(addr, amount string) (int64, error) {
...
// 更新资产
func (b *BlockAmount) Update(val string) (int64, error) {
	o := orm.NewOrm()
	res, err := o.Raw("update block_amount set amount = ? where id = ?",
		val, b.Id).Exec()
	if err == nil {
		num, _ := res.RowsAffected()
		return num, nil
...
// 设置资产
func (b *BlockAmount) Set(val string) (int64, error) {
	o := orm.NewOrm()
