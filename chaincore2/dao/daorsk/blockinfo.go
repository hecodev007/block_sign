...
type BlockInfo struct {
	...
	Timestamp      string  `orm:"column(timestamp)" description:"出块时间"`
	Transactions   int64  `orm:"column(transactions)" description:"交易总数"`
	Confirmations  int64  `orm:"column(confirmations)" description:"确认数"`
	Createtime      string  `orm:"column(createtime)" description:"创建时间"`
}
...
func (b *BlockInfo) GetBlockInfoByIndex(index int64) error {
	...
	if err == nil && nums > 0 {
		...
		b.Timestamp =  maps[0]["timestamp"].(string)
	}
	return err
}
...
func (b *BlockInfo) InsertBlockInfo() (int64, error) {
	...
	res, err := o.Raw("insert into block_info(height,hash,previousblockhash,nextblockhash,timestamp,transactions,confirmations,createtime) values(?,?,?,?,?,?,?,?)",
		b.Height, b.Hash, b.FrontBlockHash, b.NextBlockHash, b.Timestamp, b.Transactions, b.Confirmations,timeStr).Exec()
	if err == nil {
		num, _ := res.RowsAffected()
		return num, nil
	}
