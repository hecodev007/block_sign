...
type BlockInfo struct {
	...
	Timestamp      string  `orm:"column(time)" description:"出块时间"`
...
func (b *BlockInfo) GetBlockInfoByIndex(index int64) error {
	...
	if err == nil && nums > 0 {
		...
		b.Timestamp = maps[0]["time"].(string)
	}
	return err
}
...
func (b *BlockInfo) InsertBlockInfo() (int64, error) {
	...
	res, err := o.Raw("insert into block_info(height,hash,previousblockhash,nextblockhash,time,transactions,confirmations) values(?,?,?,?,?,?,?)",
		b.Height, b.Hash, b.FrontBlockHash, b.NextBlockHash, b.Timestamp, b.Transactions, b.Confirmations).Exec()
	if err == nil {
		num, _ := res.RowsAffected()
		return num, nil
	}
