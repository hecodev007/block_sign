...
func (c *RepushTxController) Post() {
	...
	for true {
		...
		if chainx.UserWatchList[uid] == nil {
		...
		var tmpWatchList map[string]bool = make(map[string]bool)
		{
			...
			if err == nil && nums > 0 {
				for i := 0; i < len(maps); i++ {
					...
					b.Fee = chainx.GetValue(common.StrToFloat64(maps[i]["sys_fee"].(string)))
					b.From = maps[i]["fromaccount"].(string)
					b.To = maps[i]["toaccount"].(string)
					b.Amount = chainx.GetValueStr(common.StrToFloat64(maps[i]["amount"].(string)))
		...
		if err == nil {
			chainx.AddPushTask(height, hash, tmpWatchList, pusdata)
		}
