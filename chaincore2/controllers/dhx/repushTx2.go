...
type DhxRepushTx2Controller struct {
	beego.Controller
	client *rpc.Client
}

func (c *DhxRepushTx2Controller) Post() {
	// 返回数据
	...
	for true {
				...
				for _, extrinsic := range block.Extrinsic {
					if extrinsic.Txid == txid {
						if extrinsic.Status == "success" && extrinsic.Type == "transfer" {
							...
							amount := aDec.Shift(-1 * models.DHX_DECIMAL).String()
							var b models.PushAccountTx
							feeBI, _ := new(big.Int).SetString(extrinsic.Fee, 10)

							fee := common.CoinToFloat(feeBI, int32(models.DHX_DECIMAL))
							b.Txid = txid
							b.Fee = fee
							b.From = from
							b.To = to
							...
							if dhx.WatchAddressList[to] != nil {
								tmpWatchList[to] = true
							}
							if dhx.WatchAddressList[from] != nil {
		...
		if err == nil {
			dhx.AddPushTask(height, txid, tmpWatchList, pusdata)
		}
		break
	}
	c.Data["json"] = resp
