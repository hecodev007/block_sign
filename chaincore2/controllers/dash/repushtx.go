...
func (c *RepushTxController) Post() {
	...
	for true {
			...
			if !res || err != nil {
				if i == 0 {
					...
					if _blockInfo.GetBlockCountByHash(blockhash) > 0 {
						...
						blockInfo := dash.Parse_block(result, false)
						if blockInfo == nil {
							continue
						}

						err = dash.Parse_block_tx_todb(0, hash, highindex, tx, blockInfo)
						if err != nil {
							continue
						}
					} else {
						// 解析整个高度
						dash.SyncBlockDataHash(blockhash)
					}
		...
		pushBlockTx.Time = time.Now().Unix()
		{
			o := orm.NewOrm()
			var maps []orm.Params
		...
		for i := 0; i < len(blockvin_list); i++ {
			...
			if blockvin_list[i].Address == "" && blockvin_list[i].Amount == 0 {
				for true {
					...
					if tmpvouts != nil && tmpvouts[blockvin_list[i].VinVoutindex] != nil {
						...
						blockvin_list[i].Amount = int64(vout["value"].(float64) * dash.ValuePrecision)
			...
			value := dash.GetValueStr(float64(blockvin_list[i].Amount))
		...
		for i := 0; i < len(blockvout_list); i++ {
			value := dash.GetValueStr(float64(blockvout_list[i].Voutvalue))
		...
		if err == nil {
			dash.AddPushTask(blocktx.Height, blocktx.Txid, tmpWatchList, pusdata)
		}
