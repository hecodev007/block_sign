...
func (c *UpdateController) Post() {
	...
	for true {
		...
		if err == nil && nums > 0 {
			res, err := o.Raw("update contract_info set name = ?, `decimal` = ? where contract_address = ? and coin_type = ?", name, decimal, contract, coin).Exec()
			if err != nil {
				log.Debug(err)
				break
			}
		...
		set_resp(0, "update contract success")
		c.Data["json"] = resp
		c.ServeJSON()
