...
var ValuePrecision float64 = 1.0

var SearchMode bool = false
var SearchHeight int64 = 0
var SearchStart int64 = 0
var SearchStop int64 = 0
var WriteDB bool = true

func GetValue(value float64) float64 {
	_value, _ := strconv.ParseFloat(fmt.Sprintf("%.0f", value/ValuePrecision), 64)
	return _value
}
...
//
var globalHeight int64 //维护一个临时的全局高度
// 初始化通道
func InitSync() {
	...
	// search mode
	SearchMode = beego.AppConfig.DefaultBool("searchmode", false)
	SearchStart = beego.AppConfig.DefaultInt64("searchstart", 0)
	SearchStop = beego.AppConfig.DefaultInt64("searchstop", 0)
	if SearchMode {
		SearchHeight = SearchStart
		WriteDB = false
	}
}
...
func SyncData() bool {
	...
	if dbblockcount >= (globalHeight - beego.AppConfig.DefaultInt64("delayheight", 1)) {
		// 获取节点区块高度
		var blockCountResult models.GetBlockCountResult
		err := RequestObject("getblockcount", nil, &blockCountResult)
		if err != nil {
			log.Info("err:%s,使用全局高度：%d", err.Error(), globalHeight)
		} else {
			//log.Debug(blockCountResult.Result)
			globalHeight = blockCountResult.Result
			log.Info("更新全局高度：%d", globalHeight)
		}
	}

	//blockcount := blockCountResult.Result
	blockcount := globalHeight
	if dbblockcount >= (blockcount - beego.AppConfig.DefaultInt64("delayheight", 1)) {
		time.Sleep(time.Millisecond * 500)
		return true
	}
	...
	for i := 0; i < oncecount; i++ {
		...
		respdata, err := Request("getblock", []interface{}{tmpval, 1})
		if err != nil {
			beego.Error(err)
			return true
		} else {
		...
		if tmpval >= (blockcount - 1 - beego.AppConfig.DefaultInt64("delayheight", 1)) {
			break
		}
...
func SearchData() bool {
	...
	blockcount := blockCountResult.Result
	if blockcount <= SearchHeight || SearchHeight >= SearchStop {
		time.Sleep(time.Millisecond * 500)
		return true
	}
...
func worker_tx(id int, jobs <-chan string, results chan<- int, hash string, highindex int64, blockInfo *dao.BlockInfo) {
	...
	for i := 0; i < count; i++ {
		...
		case txid := <-jobs:
			...
			// 解析原始交易信息
			respdata, err := Request("getrawtransaction", []interface{}{txid, 1})
			if err != nil {
				beego.Error(err)
				continue
			} else {
			...
			if datas["result"] == nil {
				log.Debug("getrawtransaction not found", txid, "reindex = 1 and txindex = 1 ?")
				continue
			}
...
// 解析区块到数据库 result
func parse_data_todb(blockdata []byte) error {
	if blockdata == nil {
		return nil
	}
	...
	highindex, hash := int64(result["index"].(float64)), result["hash"].(string)
	checkfind := true
	if SearchMode {
		checkfind = false
	}
	blockInfo := parse_block(result, checkfind)
	if blockInfo == nil {
	...
	for i := 0; i < len(txs); i++ {
		...
		// 过滤矿工交易
		if "MinerTransaction" == tx["type"].(string) {
			continue
		}
...
func parse_block(result map[string]interface{}, checkfind bool) *dao.BlockInfo {
	...
	//log.Debug(result)
	block.Height = int64(result["index"].(float64))
	block.Hash = hash
...
func disposs_vout(hash string, highindex int64, txid string, vout map[string]interface{}) (float64, string, *dao.BlockTXVout) {
	...
	txvout.Hash = hash
	txvout.Txid = txid
	txvout.Voutn = int(vout["n"].(float64))
	txvout.Voutvalue = vout["value"].(string)
	//vout_amount += float64(txvout.Voutvalue)
	txvout.AssetId = vout["asset"].(string)
	txvout.Voutaddress = vout["address"].(string)
	if WatchContractList[txvout.AssetId] != nil {
		txvout.AssetName = WatchContractList[txvout.AssetId].Name
	}
	txvout.Invaild = 0
...
func parse_block_tx_todb(id int, hash string, index int64, tx map[string]interface{}, blockInfo *dao.BlockInfo) error {
	...
	// app log
	if "InvocationTransaction" == tx["type"].(string) {
		respdata, err := Request("getapplicationlog", []interface{}{tx["txid"].(string)})
		if err != nil {
			beego.Error(err)
			return err
		} else {
		...
		applog := datas["result"].(map[string]interface{})
		executions := applog["executions"].([]interface{})
		execution := executions[0].(map[string]interface{})
		vmstate := execution["vmstate"].(string)
		if vmstate == "FAULT" {
			return nil
		}
	}
	...
	blocktx.Sysfee = 0
	if tx["sys_fee"] != nil {
		_fee, _ := decimal.NewFromString(tx["sys_fee"].(string))
		_feeval, _ := _fee.Float64()
		blocktx.Sysfee += _feeval
	}
	if tx["net_fee"] != nil {
		_fee, _ := decimal.NewFromString(tx["net_fee"].(string))
		_feeval, _ := _fee.Float64()
		blocktx.Sysfee += _feeval
	}
	if tx["gas"] != nil {
		_fee, _ := decimal.NewFromString(tx["gas"].(string))
		_feeval, _ := _fee.Float64()
		blocktx.Sysfee += _feeval
	}
	...
	if len(tmpWatchList) > 0 {
		...
		for i := 0; i < len(blockvin_list); i++ {
			...
			if blockvin_list[i].Address == "" {
				for true {
					// 解析原始交易信息
					respdata, err := Request("getrawtransaction", []interface{}{blockvin_list[i].Vintxid, 1})
					if err != nil {
						beego.Error(err)
						break
					} else {
					...
					if tmpvouts != nil && tmpvouts[blockvin_list[i].VinVoutindex] != nil {
						...
						if WatchContractList[vout["asset"].(string)] != nil {
							blockvin_list[i].AssetName = WatchContractList[vout["asset"].(string)].Name
						}
			...
			if WatchContractList[blockvin_list[i].AssetId] == nil {
				continue
			}
			...
			pushtx.Vin = append(pushtx.Vin, models.PushTxInput{Txid: blockvin_list[i].Vintxid, Vout: blockvin_list[i].VinVoutindex, Addresse: blockvin_list[i].Address, Value: amount.String(), AssetId: &assets.AssetId, AssetName: &assets.Name})
		}
		for i := 0; i < len(blockvout_list); i++ {
			if WatchContractList[blockvout_list[i].AssetId] == nil {
				continue
			}
			...
			pushtx.Vout = append(pushtx.Vout, models.PushTxOutput{Addresse: blockvout_list[i].Voutaddress, Value: amount.String(), N: blockvout_list[i].Voutn, AssetId: &assets.AssetId, AssetName: &assets.Name})
		}
		pushtx.Fee = blocktx.Sysfee
...
func update_confirmations(frontHash string) {
	...
	for i := int64(0); i < confirmations; i++ {
		respdata, err := RequestStr("getblock", []interface{}{previousblockhash, 1})
		if err != nil {
