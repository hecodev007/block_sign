...
func AddPushTask(height int64, txid string, watchlist map[string]bool, pushdata []byte) {
	beego.Debug("ar AddPushTask 42:", height, txid)
	//beego.Debug(watchlist)
	...
	for k, _ := range watchlist {
		...
		for i := 0; i < len(WatchAddressList[k]); i++ {
			...
			beego.Debug("ar AddPushTask:", k, uid, url)
			if users[uid] == "" {
...
func AddPushUserTask(height int64, pushdata []byte) {
	var UserPushList map[int64]bool = make(map[int64]bool)
	users, _ := dao.SelectWatchHeight(height)
	for i := 0; i < len(users); i++ {
		if UserWatchList[users[i]] == nil || UserPushList[users[i]] == true {
			continue
		}
		result := dao.NewNotifyResult()
			...
			case txdata := <-PushTaskList:
				{
					...
					// push to mq
					pushmqres := false
					if beego.AppConfig.DefaultBool("enablemq", false) {
						if txdata.UserID == 1 && beego.AppConfig.DefaultBool("enabletestmq", false) {
							if err := common.TestPublisher.SendTx(txdata.TxData); err != nil {
								pushmqres = false
								log.Errorf("publish test send err : %v", err)
							} else {
								pushmqres = true
							}
						} else if txdata.UserID == 2 && beego.AppConfig.DefaultBool("enableprodmq", false) {
							if err := common.ProdPublisher.SendTx(txdata.TxData); err != nil {
								pushmqres = false
								log.Errorf("publish prod send err : %v", err)
							} else {
								pushmqres = true
							}
						} else {
					...
					if beego.AppConfig.DefaultBool("enablehttppush", true) && !pushmqres {
						...
						if pushmqres {
							notifyDB.Result = 1
							notifyDB.Content = "push mq success"
							notifyDB.Update()
							if notifyDB.Result == 1 {
								break // success
							}
