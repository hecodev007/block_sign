	...
	}
	{
		...
		if err == nil && nums > 0 {
			for i := 0; i < int(nums); i++ {
				...
				addrInfo.Address = strings.ToLower(maps[i]["address"].(string))
...
func CheckWatchAddress(address string) bool {
	_, exist := WatchAddressList[address]
	return exist
}
