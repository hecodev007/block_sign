...
func InitWatchAddress() {
	...
	}
	{
		...
		if err == nil && nums > 0 {
			for i := 0; i < int(nums); i++ {
				btcAddress := ConvertAdress("stacks", maps[i]["address"].(string))
				addrInfo := new(models.UserAddressInfo)
				addrInfo.UserID = common.StrToInt64(maps[i]["user_id"].(string))
				addrInfo.Address = btcAddress
...
func InsertWatchAddress(uid int64, stacksAddress string, url string) {
	if !beego.AppConfig.DefaultBool("enablewatchaddress", false) {
		return
	}

	address := ConvertAdress("stacks", stacksAddress)
...
func RemoveWatchAddress(stacksAddress string) {
	address := ConvertAdress("stacks", stacksAddress)
	delete(WatchAddressList, address)
}

func RemoveWatchAddressByUserId(uid int64, stacksAddress string) {
	address := ConvertAdress("stacks", stacksAddress)
	for i := 0; i < len(WatchAddressList[address]); i++ {
		if WatchAddressList[address][i].UserID == uid {
