...
type DhxBlock struct {
	Nodeurl string
}

func NewDhxBlock(node string) *DhxBlock {
	dhx := new(DhxBlock)
	dhx.Nodeurl = node
	return dhx
}

func (dhx *DhxBlock) GetblockCount() (int64, error) {
	req := httplib.Post(dhx.Nodeurl) //.SetTimeout(time.Millisecond*60, time.Millisecond*120)
	...
	var dhxB DhxBlockStruct
	err = json.Unmarshal(bytes, &dhxB)
	if err != nil {
		return 0, err
	}

	currentheight := common.StrBaseToInt(dhxB.Result.Block.Header.Number, 16)
	return int64(currentheight), nil
}

func (dhx *DhxBlock) GetBlockData(hash string) (DhxBlockStruct, error) {
	var dhxB DhxBlockStruct

	req := httplib.Post(dhx.Nodeurl)
	if beego.AppConfig.String("rpcuser") != "" && beego.AppConfig.String("rpcpass") != "" {
		req.SetBasicAuth(beego.AppConfig.String("rpcuser"), beego.AppConfig.String("rpcpass"))
	}
	reqbody := map[string]interface{}{
...
// 获取区块数据
func (dhx *DhxBlock) GethashkByHeight(height int64) (string, error) {
	// 操作neo节点
	req := httplib.Post(dhx.Nodeurl) //.SetTimeout(time.Millisecond*60, time.Millisecond*120)
	reqbody := map[string]interface{}{
		"id":      "1",
...
//手续费加法
func (dhx *DhxBlock) GetPaymentQueryFeeDetails(metaData, blockHash string) (decimal.Decimal, error) {
	req := httplib.Post(dhx.Nodeurl) //.SetTimeout(time.Millisecond*60, time.Millisecond*120)
	if beego.AppConfig.String("rpcuser") != "" && beego.AppConfig.String("rpcpass") != "" {
		req.SetBasicAuth(beego.AppConfig.String("rpcuser"), beego.AppConfig.String("rpcpass"))
	}
	reqbody := map[string]interface{}{
		...
		"params":  []interface{}{metaData, blockHash},
	}
	req.JSONBody(reqbody)
	//respdata, err := req.String()
	bytes, err := req.Bytes()
	...
	return dhxB.Result.PartialFee, nil
}
