...
func SyncData() bool {
	...
	resp, err := common.Post("klay_blockNumber", []interface{}{"latest"})
	if err != nil {
		beego.Error(err)
	...
	for i := 0; i < oncecount; i++ {
		...
		respdata, err := common.Post("klay_getBlockByNumber", []interface{}{height, false})
		if err != nil {
			beego.Error(err)
			return true
		} else {
...
func SyncBlockDataHash(blockhash string) {
	respdata, err := common.Post("klay_getBlockByHash", []interface{}{blockhash, false})
	if err != nil {
		beego.Error(err)
		return
	} else {
...
func worker_tx(id int, jobs <-chan string, results chan<- int, hash string, highindex int64, blockInfo *dao.BlockInfo) {
	...
	for i := 0; i < count; i++ {
		...
		case txid := <-jobs:
			...
			respdata, err := common.Post("klay_getTransactionByHash", []interface{}{txid})
			if err != nil {
				beego.Error(err)
				continue
			} else {
...
func Parse_block_tx_todb(id int, hash string, index int64, tx map[string]interface{}, blockInfo *dao.BlockInfo) error {
	...
	if txtype != "TxTypeLegacyTransaction" && txtype != "TxTypeValueTransfer" && txtype != "TxTypeFeeDelegatedValueTransfer" && txtype != "TxTypeSmartContractExecution" && txtype == "TxTypeFeeDelegatedSmartContractExecution" && txtype != "TxTypeValueTransferMemo" {
		//log.Debug("not support txtype", txid, txtype)
		return nil
	}

	// 获取交易凭据
	respdata, err := common.Post("klay_getTransactionReceipt", []interface{}{txid})
	if err != nil {
		beego.Error(err)
		return nil
	} else {
	...
	blocktx.From = tx["from"].(string)

	switch txtype {
	case "TxTypeLegacyTransaction":
		blocktx.To = tx["to"].(string)
	...
	case "TxTypeFeeDelegatedValueTransfer":
		blocktx.To = tx["to"].(string)
		blocktx.FeePayer = tx["feePayer"].(string)

	case "TxTypeValueTransferMemo":
		blocktx.To = tx["to"].(string)
		blocktx.Memo = string(Hex2Bytes(tx["input"].(string)))

	case "TxTypeSmartContractExecution", "TxTypeFeeDelegatedSmartContractExecution":
		if txtype == "TxTypeFeeDelegatedSmartContractExecution" {
			blocktx.FeePayer = tx["feePayer"].(string)
		}
	...
	if len(tmpWatchList) > 0 {
		...
		pushtx.FeePayer = blocktx.FeePayer
		if blocktx.Contract != "" {
...
func update_confirmations(frontHash string, cmpheight int64) {
	...
	for i := int64(0); i < confirmations; i++ {
		respdata, err := common.Post("klay_getBlockByHash", []interface{}{previousblockhash, false})
		if err != nil {
			beego.Error(err)
			return
		} else {
