...
// 处理vout
func disposs_vout(hash string, highindex int64, txid string, vout map[string]interface{}) (float64, string, *dao.BlockTXVout, bool) {
	...
	//	return vout_amount, watchaddr, nil, true
	//}
	...
	if tx["vout"] != nil {
		...
		if len(vouts) > 0 {
			if enablegoroutine && vinvoutgoroutine {
				...
				for j := 0; j < len(vouts); j++ {
					vout := vouts[j].(map[string]interface{})
					tmpamount, watchaddr, blockvout, iscoinbase := disposs_vout(hash, index, blocktx.Txid, vout)
