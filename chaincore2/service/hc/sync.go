...
func SyncData() bool {
	...
	for i := 0; i < oncecount; i++ {
		...
		respdata, err := common.RequestStr("getblock", []interface{}{getBlockHashResult.Result})
		if err != nil {
			beego.Error(err, getBlockHashResult.Error)
			return true
		} else {
...
func parse_block(result map[string]interface{}, checkfind bool) *dao.BlockInfo {
	...
	block.Transactions = len(result["tx"].([]interface{})) + len(result["stx"].([]interface{}))
...
func disposs_vin(hash string, highindex int64, txid string, vin map[string]interface{}) (float64, string, *dao.BlockTXVin) {
	...
	if vin["vout"] != nil {
		txvin.VinVoutindex = int64(vin["vout"].(float64))
	}
...
// 处理合约交易
func disposs_contract_tx(hash string, height int64, txid string) *dao.ContractTX {
	respdata, err := common.RequestUrl("omni_gettransaction", []interface{}{txid}, beego.AppConfig.String("walleturl"))
	if err != nil {
		beego.Error(err)
		return nil
	} else {
	...
	//log.Debug(len(datas["result"].(map[string]interface{})))
	if datas["result"] == nil || len(datas["result"].(map[string]interface{})) == 0 {
		return nil
	}

	omnitx := datas["result"].(map[string]interface{})

	str_propertyid := common.Int64ToString(int64(omnitx["propertyid"].(float64)), 10)
	//log.Debug(str_propertyid)
	if WatchContractList[str_propertyid] == nil {
		return nil
	}
	...
	contracttx.From = omnitx["sendingaddress"].(string)
	contracttx.To = omnitx["referenceaddress"].(string)
	contracttx.ContractAddress = str_propertyid
	contracttx.Amount = omnitx["amount"].(string)
	strfee, _ := decimal.NewFromString(omnitx["fee"].(string))
	_fee, _ := strfee.Float64()
	contracttx.GasUsed = _fee
	if omnitx["valid"].(bool) {
		contracttx.Status = 1
	} else {
		contracttx.Status = 0
	}
...
// 处理vout
func disposs_vout(hash string, highindex int64, txid string, vout map[string]interface{}) (float64, string, *dao.BlockTXVout, *dao.ContractTX, bool) {
	var vout_amount float64 = 0
	var watchaddr string = ""
	if vout == nil {
		return vout_amount, watchaddr, nil, nil, false
	}
	...
	if voutn == 0 && (scriptPubKey["type"].(string) == "nonstandard") && txvout.Voutvalue == 0 {
		return vout_amount, watchaddr, nil, nil, true
	}

	// omni
	var _contracttx *dao.ContractTX
	if scriptPubKey["type"].(string) == "nulldata" {
		_contracttx = disposs_contract_tx(hash, highindex, txid)
	}

	if scriptPubKey["addresses"] == nil {
		return vout_amount, watchaddr, nil, _contracttx, false
	}
	...
	watchaddr = txvout.Voutaddress
	return vout_amount, watchaddr, txvout, _contracttx, false
}
	...
	if tx["vout"] != nil {
		...
		if len(vouts) > 0 {
			if enablegoroutine && vinvoutgoroutine {
				...
				for j := 0; j < len(vouts); j++ {
					vout := vouts[j].(map[string]interface{})
					tmpamount, watchaddr, blockvout, contracttx, iscoinbase := disposs_vout(hash, index, blocktx.Txid, vout)
