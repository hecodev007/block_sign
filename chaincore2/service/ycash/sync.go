...
import (
	...
	dao "github.com/group-coldwallet/chaincore2/dao/daoycash"
...
func SyncData() bool {
	...
	err := common.RequestObject("getblockcount", nil, &blockCountResult)
	if err != nil {
		beego.Error(err)
	...
	for i := 0; i < count; i++ {
		...
		case txid := <-jobs:
			...
			if err != nil || datas["error"] != nil {
				log.Debug(txid, err, datas["error"])
				continue
			}
...
func disposs_vout(hash string, highindex int64, txid string, vout map[string]interface{}) (float64, string, *dao.BlockTXVout) {
	...
	txvout.Txid = txid
	txvout.Voutn = voutn
	//txvout.Voutvalue = vout["value"].(float64)
	txvout.Voutvalue = int64(vout["valueZat"].(float64)) // value 10 ^ 8
...
func Parse_block_tx_todb(id int, hash string, index int64, tx map[string]interface{}, blockInfo *dao.BlockInfo) error {
	...
	// 手续费
	if tx["fee"] == nil {
		blocktx.Sysfee = (vin_amount - vout_amount) / float64(100000000)
		if blocktx.Sysfee < 0 {
			blocktx.Sysfee = 0
		}
	} else {
	...
	if num <= 0 || err != nil {
		...
		if len(tmpWatchList) > 0 {
				...
				if blockvin_list[i].Address == "" && blockvin_list[i].Amount == 0 {
					for true {
						...
						if tmpvouts != nil && tmpvouts[blockvin_list[i].VinVoutindex] != nil {
							...
							blockvin_list[i].Amount = int64(vout["valueZat"].(float64))
							scriptPubKey := vout["scriptPubKey"].(map[string]interface{})
							if scriptPubKey["addresses"] != nil {
								addresses := scriptPubKey["addresses"].([]interface{})
								if len(addresses) == 1 {
			...
			// 重新计算手续费
			_value, _ := strconv.ParseFloat(fmt.Sprintf("%.8f", float64(vin_amount-vout_amount)/float64(100000000)), 64)
			pushtx.Fee = _value
			if pushtx.Fee < 0 {
				pushtx.Fee = 0
			}
			...
			if err == nil {
				AddPushTask(index, blocktx.Txid, tmpWatchList, pusdata)
			}
		}
	}
