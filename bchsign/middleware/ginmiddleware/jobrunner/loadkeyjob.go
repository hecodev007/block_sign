...
import (
	...
	"github.com/group-coldwallet/bchsign/util"
...
//path 文件路径
//加载方法 loadMethod
func LoadKeyJob(cronSpec, path string, loadMethod func(fileA, fileB string)) {
	jobrunner.Start() // optional: jobrunner.Start(pool int, concurrent int) (10, 1)
	jobrunner.Schedule(cronSpec, LoadKey{Path: path, LoadMethod: loadMethod})
}

// Job Specific Functions
type LoadKey struct {
	Path       string
	LoadMethod func(fileA, fileB string)
}
...
func (load LoadKey) Run() {
	...
	if len(fileArr) == len(files) {
		...
		for _, file := range diffFiles {
			...
			//读取文件，加载进入内存
			load.LoadMethod(file, fileBPath)
		}
