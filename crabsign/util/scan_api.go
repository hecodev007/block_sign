...
func NewScanApi(url string, key string) *ScanApi {
	return &ScanApi{
		Url: url,
		Key: key,
		Dec: 12,
...
func (Api *ScanApi) MetaData() (*MetaData, error) {
...
func (block *Block) ToTransactions(i int) (ret []*Transaction, err error) {
	...
	extrinsic := block.Extrinsics[i]
	if !(extrinsic.CallModule == "utility" &&
		extrinsic.CallModuleFunction == "batch_all") {
		return ret, nil
	}
...
func (Api *ScanApi) Block(blockHash string) (*Block, error) {
...
func (Api *ScanApi) Post(path string, params interface{}, ret interface{}) error {
...
func (Api *ScanApi) AccountInfo(address string) (amount decimal.Decimal, nonce int64, err error) {
	...
	if resp.Code != 0 {
		return decimal.Zero, 0, errors.New(resp.Message)
	}
	return resp.Data.Account.Balance.Shift(12), resp.Data.Account.Nonce, nil
}

//var prefix = []byte{0x00}
var prefix = []byte{0x2a}
