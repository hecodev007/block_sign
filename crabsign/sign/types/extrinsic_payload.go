...
func (e ExtrinsicPayloadV4) SignED(signer signature.KeyringPair) (Signature, error) {
	b, err := EncodeToBytes(e)
	if err != nil {
		return Signature{}, err
	}

	sig, err := signature.SignED(b, signer.URI)
	return NewSignature(sig), err
}
func (e ExtrinsicPayloadV4) SignV1(private []byte) (Signature, error) {
	b, err := EncodeToBytes(e)
	if err != nil {
		return Signature{}, err
	}

	sig, err := signature.SignV1(b, private)
	return NewSignature(sig), err
}
