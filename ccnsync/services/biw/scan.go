...
retryGetBlockByHeight:
	...
	for index, txid := range block.Txs {
		...
		go func(txid string, index int) {
			defer workpool.Dec()
			//过滤非转账交易
			//log.Infof("getrawtransaction:", txid, index, len(block.Txs))
		getTransaction:
			tx, err := s.GetRawTransaction(txid)
			if err != nil {
				log.Infof(err.Error(), txid)
				goto getTransaction
			}
			//log.Infof("getrawtransaction success:", txid, index, len(block.Txs))
			...
			//log.Infof("TX index:", workpool.Running, index)
		}(txid, index)
	}
	workpool.Wait()
...
func parseBlockRawTX(RpcClient *rpc.RpcClient, tx *rpc.Transaction, blockhash string, height int64) (*TxInfo, error) {
	...
	for _, vin := range tx.Vin {
		...
		if vin.Txid == "" { //跳过挖矿交易
			log.Infof(tx.Txid, "empty vin.txid")
			continue
		}
		vintx, ok := txcache[vin.Txid]
		if !ok {
			//log.Infof(tx.Txid, vin.Txid)
		GetRawTransaction:
			tmptx, err := RpcClient.GetRawTransaction(vin.Txid)
			if err != nil {
				log.Infof(err.Error(), tx.Txid, vin.Txid)
