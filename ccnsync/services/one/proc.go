...
func (p *Processor) parseContractTxs(txs []*dao.BlockTx) (watchaddrs map[string]bool) {
	...
	for _, tx := range txs {
		...
		from := icap.ToBech32(common2.HexToAddress(tx.FromAddress))
		to := icap.ToBech32(common2.HexToAddress(tx.ToAddress))

		if p.watch.IsWatchAddressExist(from) {
			watchaddrs[from] = true
		}
		if p.watch.IsWatchAddressExist(to) {
			watchaddrs[to] = true
		}
	}
	return
