...
func parseBlockTX(watch *services.WatchControl, rpc *rpc.RpcClient, tx *rpc.Transaction, block *dao.BlockInfo) ([]*dao.BlockTx, error) {
	...
	if strings.HasPrefix(tx.To,"0x") {
		tx.To = icap.ToBech32(common2.HexToAddress(tx.To))
	}

	toaddr := icap.ToBech32(common2.HexToAddress("0x"+tx.Input[34:74]))

	if watch.IsContractExist(tx.To) || len(tx.Input) > 75 && watch.IsWatchAddressExist(toaddr) {
	...
	TransactionReceipt:
		...
		if strings.HasPrefix(ContractAddress,"0x") {
			ContractAddress = icap.ToBech32(common2.HexToAddress(ContractAddress))
		}
		...
		from := icap.ToBech32(common2.HexToAddress("0x" + receipt.Logs[0].Topics[1][26:66]))
		to := icap.ToBech32(common2.HexToAddress("0x" + receipt.Logs[0].Topics[2][26:66]))
	...
	tx.From = icap.ToBech32(common2.HexToAddress(tx.From))
	tx.To = icap.ToBech32(common2.HexToAddress(tx.To))
	//普通转账处理
...
TransactionReceipt:
	...
	for _, tlog := range receipt.Logs {
		tlog.Address = icap.ToBech32(common2.HexToAddress(tlog.Address))
