...
type TraceTransactionInfo struct {
	...
	Calls   []TraceTransactionInfoCalls `json:"calls"`
}
type TraceTransactionInfoCalls struct {
	Type    string                      `json:"type"`
	From    string                      `json:"from"`
	To      string                      `json:"to"`
	Value   string                      `json:"value"`
...
func (rpc *RpcClient) IsMainCoinTx(t *Transaction) bool {
	if t.Value.Cmp(big.NewInt(0)) == 1 {
		return true
	}
	return false
}
...
// GetTraceTransaction returns the information about a transaction requested by transaction hash.
func (rpc *RpcClient) GetTraceTransaction(txHash string) (*TraceTransactionInfo, error) {
	return rpc.getTraceTransaction("debug_traceTransaction", txHash)
}
...
func (rpc *RpcClient) GetTransactionReceipt(hash string) (*TransactionReceipt, error) {
	...
	log.Debugf("hash %s GetTransactionReceipt used time : %f 's", hash, time.Since(startTime).Seconds())
...
func (rpc *RpcClient) getTraceTransaction(method string, txHash string) (*TraceTransactionInfo, error) {
	result, err := rpc.RawCall(method, "", txHash, map[string]string{"tracer": "callTracer"})
	if err != nil {
		return nil, err
	}
	if bytes.Equal(result, []byte("null")) {
		return nil, fmt.Errorf("get getTraceTransaction null")
	}
	response := &TraceTransactionInfo{}
	err = json.Unmarshal(result, response)
	if err != nil {
		return nil, err
	}
