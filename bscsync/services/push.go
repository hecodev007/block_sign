		...
		for run {
			...
			case task := <-s.pushTaskList:
				if s.cfg.Enable {
					...
					if err != nil {
						log.Infof("SelectNotifyResult err : %v", err)
						break
					}
					notifyDB.Result = 0
...
func (s *PushServer) AddPushTask(height int64, txid string, watchlist map[string]bool, pushdata []byte) {
	...
	for k := range watchlist {
		...
		if err != nil {
			log.Infof("don't find user info")
			break
		}
		for _, wui := range was {
			if users[wui.UserID] == "" {
...
func (s *PushServer) AddPushUserTask(height int64, pushdata []byte) {
	...
	if err != nil {
		log.Info("don't have user watch height")
	}
...
func (s *PushServer) httpPush(pt *PushTask) (string, error) {
	...
	if s.cfg.Agent {
		...
		if err != nil {
			log.Infof("url %s ,err : %v", s.cfg.Url, err)
		...
		if err != nil {
			log.Infof("url %s ,err : %v", pt.PushUrl, err)
	...
	if err != nil {
		log.Infof("post %v ,err : %v", req.URL, err)
	...
	resp.Body.Close()
	if err != nil {
		log.Infof("parse body error: %v", err)
...
func (s *PushServer) disconnectMQ() {
	for _, pb := range s.publishers {
		if err := pb.Close(); err != nil {
			log.Infof("producer shutdown err : %v", err)
		}
	}
	if err := s.rmq.Shutdown("disconnect with finish"); err != nil {
		log.Infof("disconnectMQ err : %v", err)
	}
}
