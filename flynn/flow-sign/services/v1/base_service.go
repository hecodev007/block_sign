...
import (
	...
	"github.com/group-coldwallet/flow-sign/services"
...
type generateKeyAndAddress func() (common.AddrInfo, error) //用于生成地址方法
...
func (bs *BaseService) createAddress(req *model.ReqCreateAddressParams, generateKey generateKeyAndAddress) (*model.RespCreateAddressParams, error) {
	...
	addresses, err := common.CreateAddrCsv(conf.Config.FilePath, req.MchId, req.OrderId, req.CoinName, addrInfos)
	if err != nil {
		return nil, err
	}
...
func (bs *BaseService) multiThreadCreateAddress(number int, coinName, mchId, orderId string, generateKey generateKeyAndAddress) (*model.RespCreateAddressParams, error) {
	...
	buildnummap := []int{}
	addressChan := make(chan common.AddrInfo, number)
	if number <= numcpu {
		numcpu = 1
	...
	addresses, err := common.CreateAddrCsv(conf.Config.FilePath, mchId, orderId, conf.Config.CoinType, addrInfos)
	if err != nil {
		return nil, err
	}
	resp := new(model.RespCreateAddressParams)
