...
func (socket *Socket) Connect() {
	...
	if err != nil {
		log.Info("Error while connecting to server ", err)
...
func (socket *Socket) reConnect() error {
	if socket.Conn == nil {
		...
		if err != nil {
			log.Infof("error while connecting to server,err=%v,reConnect num is %d ", err, socket.reConnectNum)
			socket.reConnectNum++
			socket.IsConnected = false
			if socket.OnConnectError != nil {
				socket.OnConnectError(err, *socket)
