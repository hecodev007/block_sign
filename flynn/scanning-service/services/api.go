...
func (bs *BaseService) RepushTx(userId int64, txid string, height int64) error {
	if userId <= 0 {
		return fmt.Errorf("use id is less than 0,userId=%d", userId)
	}
	if txid == "" {
		return errors.New("txid is null")
	}
	//if height<=0 {
	...
	if height == 0 {
		...
		if err != nil {
			return fmt.Errorf("补推错误，请添加高度重试： %v", err)
		}
	}
	log.Infof("补推处理txid： %s", txid)
	...
	if height > 0 {
		...
		if len(blockData.TxIds) > 0 {
			for _, bd := range blockData.TxIds {
				if strings.TrimPrefix(bd, "0x") == strings.TrimPrefix(txid, "0x") {
					...
					if err != nil {
						return fmt.Errorf("parse transaction error,tx id=%s,err: %v", txid, err)
					}
					if txData == nil {
						return fmt.Errorf("没有相关的交易：%s", txid)
					}
					break
				}
			}
		...
		if !hasTx {
			return fmt.Errorf("do not find this txid: %s", txid)
		}
	}
	...
	if txData.IsFakeTx {
		return fmt.Errorf("发现一笔假充值,txid=%s", txid)
	}
	var (
	...
	if len(tmpWatchList) > 0 {
		...
		return fmt.Errorf("没有发现监听的地址，txid=%s", txid)
	}
	//如果成功 把	数据写入数据库
