...
func newStdout(level string) {
	...
	lge = &logger{}
	lge.Logger = newJSONLogger(level)
	lge.Logger.Out = io.MultiWriter(wErr...)
}
...
func init() {
	...
	if maxAgeHour <= 0 {
		maxAgeHour = int64(24 * 90)
	}

	createInfo(level, path, maxAgeHour)
	createError(level, path, maxAgeHour)

}

func createInfo(level, path string, maxAgeHour int64) {
	filePath := path + "/info-%Y%m%d.log"
	linkPath := path + "/info.log"

	fileWriter := createFileWriter(filePath, linkPath, maxAgeHour)
...
func createError(level, path string, maxAgeHour int64) {
	filePath := path + "/error-%Y%m%d.log"
	linkPath := path + "/error.log"
	...
	lge = &logger{}
	lge.Logger = newJSONLogger(level)
	lge.Logger.Out = io.MultiWriter(writers...)
}

func createFileWriter(filePath, linkPath string, maxAgeHour int64) *rotatelogs.RotateLogs {
...
func Error(args ...interface{}) {
	lge.Error(strings.TrimRight(fmt.Sprintln(args...), "\n"))
}

func Fatal(args ...interface{}) {
	lge.Fatal(strings.TrimRight(fmt.Sprintln(args...), "\n"))
}

func Panic(args ...interface{}) {
	lge.Panic(strings.TrimRight(fmt.Sprintln(args...), "\n"))
}
