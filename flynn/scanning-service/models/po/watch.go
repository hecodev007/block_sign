...
func InsertAddressInfo(u *AddressesInfo) error {
	if err := db.UserDB.DB.Create(u).Error; err != nil {
		return err
	}
	return nil
}
...
func FindAddressesNum(cointype string) (int, error) {
	var count int
	err := db.UserDB.DB.Model(&AddressesInfo{}).Where("coin_type = ? and status = 'used'", cointype).Count(&count).Error
	if err != nil {
		return 0, err
	}
	return count, nil
...
func FindAddressIsExist(cointype, address string) bool {
	var as []AddressesInfo
	err := db.UserDB.DB.Where("coin_type = ? and address= ? and status = 'used'", cointype, address).Find(&as).Error
	if err != nil {
		return false
	}
	if len(as) == 0 {
