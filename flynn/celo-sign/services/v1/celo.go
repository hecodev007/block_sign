...
func (cs *CeloService) TransferService(req interface{}) (interface{}, error) {
	...
	log.Printf("gas Price is [%d]",gasPrice)
	//gasPrice := conf.Config.CeloCfg.GasPrice
	var amount *big.Int
	...
	if strings.Compare(deletePrefix(toAddress.String()),deletePrefix(tp.ToAddress)) != 0 {
		return nil, fmt.Errorf("to address is not equal,address1=[%s],address2=[%s]", deletePrefix(toAddress.String()),deletePrefix(tp.ToAddress))
	}
	if tp.ContractAddress == "" {
		...
		if strings.Compare(deletePrefix(to.String()),deletePrefix(tp.ContractAddress)) != 0 {
			return nil, fmt.Errorf("to address is not equal,address1=[%s],address2=[%s]", deletePrefix(to.String()),deletePrefix(tp.ContractAddress))
		}
...
func (cs *CeloService) createAddressInfo() (util.AddrInfo, error) {
	...
	if len(priv) != 32 {
		for true {
			...
			if len(priv) == 32 {
				pub = append(pub, newPrivKey.X.Bytes()...)
				pub = append(pub, newPrivKey.Y.Bytes()...)
				break
			}
		}
	} else {
		pub = append(pub, privkey.X.Bytes()...)
		pub = append(pub, privkey.Y.Bytes()...)
	}
	//according to priv to create pub bytes
...
func deletePrefix(address string)string{
