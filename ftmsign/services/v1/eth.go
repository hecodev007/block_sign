...
func (bs *BaseService) FTMService() *FTMService {
	cs := new(FTMService)
	cs.BaseService = bs
	// 初始化连接
...
接口创建地址服务
	无需改动
*/
func (cs *FTMService) CreateAddressService(req *model.ReqCreateAddressParams) (*model.RespCreateAddressParams, error) {
...
离线创建地址服务，通过多线程创建
	无需改动
*/
func (cs *FTMService) MultiThreadCreateAddrService(nums int, coinName, mchId, orderId string) error {
...
签名服务
*/
func (cs *FTMService) SignService(req *model.ReqSignParams) (interface{}, error) {
...
热钱包出账服务
*/
func (cs *FTMService) TransferService(req interface{}) (interface{}, error) {
	var (
...
func (cs *FTMService) TransferWithNonceService(req interface{}) (interface{}, error) {
...
func (cs *FTMService) processTransfer(tp model.EthTransferParams, nonce uint64) (interface{}, error) {
	if tp.FromAddress == "" || tp.ToAddress == "" || tp.Amount == "" {
		return nil, fmt.Errorf("params is null,from=[%s],to=[%s],amount=[%s]", tp.FromAddress, tp.ToAddress, tp.Amount)
	}
	if cs.client == nil {
	...
	//余额检查
	balance, err := cs.GetBalance(&model.ReqGetBalanceParams{CoinName: "ftm", Address: tp.FromAddress, ContractAddress: tp.ContractAddress})
	if err != nil {
...
func (cs *FTMService) getBuildTxParams(method string, params []interface{}) (*big.Int, error) {
...
func (cs *FTMService) getNonce(method, address, tag string) uint64 {
...
创建地址实体方法
*/
func (cs *FTMService) createAddressInfo() (util.AddrInfo, error) {
...
*/
func (cs *FTMService) makeERC20TransferData(toAddress, transfer_method_id string, amount *big.Int) ([]byte, error) {
	var data []byte
...
*/
func (cs *FTMService) ethSign(to string, nonce, gasLimit uint64, value, gasPrice *big.Int, privKey *ecdsa.PrivateKey) (string, bool) {
	// build transaction
...
*/
func (cs *FTMService) ethTokenSign(to, toContractAddr string, nonoce, gasLimit uint64, value, gasPrice *big.Int, privKey *ecdsa.PrivateKey) (string, bool) {
...
*/
func (cs *FTMService) sign(tx *types.Transaction, privKey *ecdsa.PrivateKey) (string, bool) {
...
func (cs *FTMService) sign2(tx *types.Transaction, privKey *ecdsa.PrivateKey) (string, bool) {
...
func (cs *FTMService) aegisNonce(address string) (uint64, error) {
	// 1. 先获取该地址链上的nonce值
...
func (cs *FTMService) isPendingStatus(txid string) bool {
...
func (cs *FTMService) GetBalance(req *model.ReqGetBalanceParams) (interface{}, error) {
	var (
		data []byte
		err  error
	)
...
func (cs *FTMService) ValidAddress(address string) error {
